...
**Last Updated**: 2026-05-30 (R23 ✅ MERGED to main at `4cb5220` + finalize `f6025e8`; current work: CI-speed branch `ci-speed` — `python/aletheia/` package + `tools/` ruff(ALL)+pylint-10+basedpyright-0/0/0 CLEAN; **`python/tests/` basedpyright-gate expansion (415→**219** via the ruff-clean DBC batch; tests/ pylint **10.00** via known-first-party isort `ddbd10b`) + ruff(ALL) campaign + Fraction-pipeline Chunk A, IN PROGRESS** 2026-05-30; not yet merged).  R22 ✅ merged to main at `3ebfc37` + clang-format follow-up `ce7bbcc`; main pushed to `origin`.
...
**CI-speed gate optimization** (post-R23 user directive, branch `ci-speed`, NOT merged): make the verification gates fast enough that every contributor runs them manually + often.  Core insight — ONE warm `agda --interaction-json` process loads stdlib + interfaces once (vs ~14s per-invocation reload × N), attacking "calling Agda too often".  **Step 9** dead-import gate (was ~hours) → `tools/warm_dead_imports.py`: warm candidate-sieve + brute-force-confirm-*only-candidates* (~64× for a batch; filters the inferred-type-use FP class).  **Steps 1-8** check-properties (629s) → `tools/warm_check_properties.py` + Shakefile `check-properties-warm` (one process for all 45 proof modules, **~13×**: 629s→47.6s; batch-equivalence verified).  **#1 production-usability** done (`1312115`): portable (no hardcoded paths; `tools/` package; `python -m tools.X`) + pylint 10.00 / basedpyright 0/0/0.  **Lint-compliance campaign (2026-05-27)**: all of `tools/` now passes **ruff `select=["ALL"]` + pylint 10.00 + basedpyright 0/0/0, zero suppressions** (~23 commits; root `ruff.toml`; full `python -m` migration; `_common`+`_agda_imports`; `eval`→AST; the 3 `# pylint: disable` removed via refactor; S603 flagged as the lone judgment-ignore).  **`python/aletheia/` root-level slice COMPLETE 2026-05-29** — 6 commits covering the remaining root files: `8431a8d` (`checks_runner.py` 21→0; R0801 break-via-rename `frame` → `can_frame`), `4b5a8ee` (`checks.py` 26→0; 7× D107 + 6× D400/D415), `b2df9cf` (`yaml_loader.py` 40→0; C901 → 4 condition-family helpers), `7a89708` (`excel_loader.py` 45→0; PLR2004 → `_MIN_DBC_SHEET_ROWS = 2`), `741be31` (`cli.py` 82→0; T201×62 → `_emit()` helper routing through `sys.stdout.write`; ERA001×5 → em-dash for colon-confused section headers; FBT001+FBT002 kw-only; CLI smoke-tested), `696990d` (`dsl.py` 264→0; 192 autofixed + 34 scripted D400/D415 + 4 msg-vars; DEFERRED-block provenance stripped per [[feedback_in_source_deferral_notes]]).  Combined with prior commits: `tools/` + entire `aletheia/` package (root + `asyncio/` + `client/`) are ruff(ALL)+pylint-10+basedpyright-0/0/0 clean and free of review-process marks; package pylint baseline 9.99/10 preserved (4 pre-existing R0801 need a dedicated multi-file dedupe commit).  **Review-process-mark sweep** 2026-05-28 (new memory `feedback_no_review_process_marks_in_code`): stripped ~120 marks from already-ruff-clean dirs across 4 commits — asyncio/ `5156c78`, foundation + __init__ `fcbe717`, client/ `148bb29`, tools/ `4680394`; subsequent ruff(ALL) commits on uncleaned dirs fold mark-stripping into the same commit.  **User crash-mitigation directive** 2026-05-28 (new memory `feedback_wsl2_crash_resource_discipline`): WSL2 + long Claude sessions = crashes (4 on 2026-05-28); commit often, serial-by-default, parallelism ≤2-3, budget-check before heavy ops, no `run_ci` / full check-properties / full bench unless asked.  **Post-root-slice (2026-05-29; this UPD crash-recovered) — 15 commits across 3 streams**: (1) `python/tests/` ruff(ALL) groups — `df12204` infra / `394d331` DSL / `dbf7e02` DBC / `44b4a74` YAML / `e4fee05`+`f915ab8` Excel (wide DBC/When-Then row literals → named constants in new `tests/_excel_helpers.py` to dodge C0302+R0801; new memory `feedback_no_object_type_escape` — never annotate `object`/`Any`→`object`).  (2) Fraction-pipeline redesign Chunk A — `b3eddf4`/`4de0c58`/`7459f22`/`241189f`/`e8f0130`: ONE internal numeric type `Fraction` (int/RationalNumber = wire encodings, float = ingress-only), `_enrichment.py` retyped to `LTLFormula`/`SignalPredicate`, and a **weakNext cross-binding parity fix** (Python alone rendered `<unknown>` not `weak_next(...)`; Go/C++ already correct); renderer-proof deferred (`memory/project_rational_renderer_proof.md`); Chunks B (drop `RationalNumber` type) / C (ingress floats) / loader JSONValue clusters pending.  (3) **basedpyright-gate expansion** (user directive: also cover tests/, benchmarks/) — benchmarks/ already gated since 2026-05-09 @ 0 errors; tests/ **415 → 386** strict errors via `aabe860`/`fba5f68`/`7d0a93b`/`7ad8b4a`, ~22 leaf files left; gate flips (wire tests/ into `run_ci.py:631`) only when tests/=0.  877 tests green throughout.  Roadmap: continue with `python/tests/` (big batch) → `python/benchmarks/` → `examples/` → Agda `src/` → Go `go/` → C++ `cpp/`; the 4 pre-existing `aletheia/` R0801s in a separate dedupe commit → wire tools/ into the gate; (warm) wire `run_ci`, land 5 dead imports, steps 10-30; merge.  **2026-05-30**: tests/ basedpyright **386→219** (ruff-clean DBC batch tier2/queries/format_dbc/validator/tier1); ruff installed in venv + `[dev]` (`1736554`); **tests/ pylint 10.00** via `ddbd10b` — `known-first-party=["aletheia"]` isort resolves I001↔C0411 at root + reorders 38 files, plus `mux_signal()`/`_stream_helpers` R0801 de-dup, `run_one_frame_stream` relocated out of conftest (C0411), local import hoisted (C0415); 4 bp-groups remain (fuzz / string-concat / client-check / input_bounds).  Detail: `memory/project_ci_speed_optimization.md` + `memory/feedback_ruff_all_lint_campaign.md` + `memory/feedback_no_suppression_without_approval.md` + `memory/feedback_fix_tool_gate_violations.md` + `memory/feedback_no_review_process_marks_in_code.md` + `memory/feedback_wsl2_crash_resource_discipline.md`.
