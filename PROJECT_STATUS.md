...
**Last Updated**: 2026-06-10 (**`ci/pr-heavy-lanes` ✅ MERGED to main** — PR #16 squash, 2026-06-10: the Phase-2 heavy-lanes workflow [parallel repro/stability/mutation; repro REQUIRED, stability+mutation ADVISORY] + the mutation-to-zero campaign [C++ **0** / Python **215→1** documented-equiv / Go **0**; baselines pinned in `docs/MUTATION_BENCH.yaml`].  Now on branch `agda/e1-isidentstart-hspace-bridge` continuing **E.2**.  Earlier: **`ci-speed` ✅ MERGED to main** — PR #7, merge `bf0b749`, 2026-06-09; the `push:main` full-CI sweep `27206624057` went green.  The `main` ruleset now **requires** the `tools/run_ci.py (all gates)` check, enabled by the user 2026-06-10.  Post-merge cleanup on branch `post-merge-cleanup`: ghcup log-hygiene `chown`, new `docs/development/DEFERRED_ITEMS.md` backlog, venv-convention docs standardized on `python/.venv`.)  R23 ✅ merged to main at `4cb5220` + finalize `f6025e8` (2026-05-26).
...
**Codebase**:
- Agda modules: **266** (261 `--safe --without-K` + 4 `--safe --without-K --no-main` + 1 allowlisted `--without-K`-only `Substrate.Unsafe`; the +2 over R23 are the ci-speed renderer-proof modules `DecRat/RationalSoundness.agda` (linchpin, 264→265) and `DBC/RationalRenderer/Faithful.agda` (faithfulness + canonical-shape theorems, 265→266)).  Per-round evolution detail in `memory/project_review_round{18,19,20,21}.md` and `docs/archive/PROJECT_STATUS_HISTORY.md`.
