...
**Last Updated**: 2026-04-24 (Phase B.3.d pre-gate ✅, layer 1 ✅, layer 2 in_progress — commits 2a `9adbc46` and 2b `4559d5c` shipped; Identifier primitive roundtrip closed; commit 2c + layers 3–4 pending)
...
- **Phase B.3** — Agda-core DBC text parser (R17-DEF-4). **B.3.a/b/c ✅** (corpus baseline, skeletons, incremental constructs — 13 commits `4a086e8..a7f255e`). **Pre-B.3.d infrastructure ✅** (`check-properties` wiring + three WF fixes, commit `0035a4e`). **B.3.d pre-gate ✅ complete (2026-04-24)** — 6-commit ℚ→DecRat migration (`0b7849b` DecRat primitives, `c05083e` text parser/emitter + universal sketch, `6fa29e3` EV_ + Layer-4 closure + `NonTerminatingRational` wired cross-binding, `dd9b770` SG_ + `lookupDecRat` combinator + `mkℚ`-direct `toℚ` runtime optimisation that closed a 9–15% CAN-FD Signal Extraction regression and delivered +16% on CAN 2.0B, `917465b` Attributes + `TextParser.agda` / `TextFormatter.agda` added to `check-properties` walk, `1f175ab` docs/memory).  **B.3.d Layer 1 ✅ complete (2026-04-24, `66afc2d`)** — Option 3a (2-axiom `Substrate/Unsafe.agda` + formatter refactor to `List Char` internals).  **B.3.d Layer 2 ⏳ in_progress (2026-04-24)** — two commits shipped: **2a** (`9adbc46`) lifts `Identifier` from bare `String` to `record { name : String; valid : T (validIdentifierᵇ (toList name)) }` (T3-fixed; 5–10% Signal Extraction regression accepted on C++/Go, Python flat; `project_identifier_eq_revisit.md` for revisit angles); **2b** (`4559d5c`) ships `parseIdentifier-roundtrip` — the first per-primitive theorem at `List Char` level, composed via `bind-just-step` following the `DecRatParse.parseDecRat-roundtrip-+suc` template.  **Commit 2c pending** — remaining primitives (ByteOrder digit, sign flag, mux marker, StringLit, AttrScope/AttrType/AttrValue tags, SignalPresence) follow the same template.  **Layers 3–4 pending**: per-line-construct roundtrips + top-level `parseText ∘ formatText ≡ id` aggregator.  **B.3.e–j pending**: JSON command, Python feature-flag switch, cantools drop, C++/Go APIs, cross-binding parity.
...
**Status**: Phase 5.1 + Phase A + Phase B.1/B.1.x + Phase B.2 + Phase B.3.a/b/c + **Phase B.3.d pre-gate + layer 1** complete; **Phase B.3.d layer 2 in_progress** (Identifier commits 2a + 2b shipped 2026-04-24; commit 2c + layers 3–4 pending); Phase B.3.e–j → Phase C/D per plan.
