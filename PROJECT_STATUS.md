...
**Last Updated**: 2026-04-25 (Phase B.3.d pre-gate ✅, layer 1 ✅, layer 2 ✅; layer 3 in_progress — commits 3a `804c584` + 3b `ad111bf` shipped; **3c precursor work shipped 2026-04-25** — `3a7c86e` refinement-types foundation + `c884e69` parseInt subsumption + `7a44c87` parseIntType/parseHexType subsumption.  3c per-line proofs next; see `.session-state.md`)
...
- **Phase B.3** — Agda-core DBC text parser (R17-DEF-4). **B.3.a/b/c ✅** (corpus baseline, skeletons, incremental constructs — 13 commits `4a086e8..a7f255e`). **Pre-B.3.d infrastructure ✅** (`check-properties` wiring + three WF fixes, commit `0035a4e`). **B.3.d pre-gate ✅ complete (2026-04-24)** — 6-commit ℚ→DecRat migration (`0b7849b`–`1f175ab`); `mkℚ`-direct `toℚ` runtime optimisation closed a 9–15% CAN-FD Signal Extraction regression and delivered +16% on CAN 2.0B; `NonTerminatingRational` parse error wired cross-binding.  **B.3.d Layer 1 ✅ complete (2026-04-24, `66afc2d`)** — Option 3a (2-axiom `Substrate/Unsafe.agda` + formatter refactor to `List Char` internals).  **B.3.d Layer 2 ✅ complete (2026-04-25)** — three commits: **2a** (`9adbc46`) lifts `Identifier` to `record { name : String; valid : T (validIdentifierᵇ (toList name)) }` (5–10% Signal Extraction regression accepted; revisit angles in `project_identifier_eq_revisit.md`); **2b** (`4559d5c`) ships `parseIdentifier-roundtrip`; **2c** (`f315c6f`) closes Tier A (byte-order/sign/scope/string-type) + Tier B (string-literal/mux marker).  **B.3.d Layer 3 ⏳ in_progress (2026-04-25)** — **3a** (`804c584`) shipped Preamble per-line-construct roundtrips (VERSION + BS_ + NS_); **3b** (`ad111bf`) shipped Option C-broad: all four simple-line constructs with formatter pairs land together — `BU_` (node list, 611L) + `VAL_TABLE_` (value table, 790L) + `EV_` (env var, 1,581L) + `CM_` (comment, 2,533L; 5-way `CommentTarget` dispatch via 4-fold `<|>`-chain) — wired through `Properties/{Topology,ValueTables,EnvVars,Comments}.agda` facades.  CM_ heap blowup root-caused and fixed: `buildCANId-rawCanIdℕ` Extended clause `rewrite n+ext∸ext≡n` over a goal containing nested `ifᵀ`s replaced with pointwise `subst` per AGENTS.md line 234; type-checks at `-M16G` (was failing at `-M48G`).  **3c precursor ✅ (this session, 2026-04-25)** — three commits unifying every DBC numeric slot to DecRat ahead of the 3c per-line attribute roundtrip proofs.  **`3a7c86e`** introduces `IntDecRat` / `NatDecRat` refinement records (DecRat + `T (predicateᵇ value)` witness, mirroring Identifier T3-fixed pattern) + `Aletheia/DBC/DecRat/Refinement.agda` (~190 LOC) + `Properties/Attributes/Common.agda` (~190 LOC) foundation for the per-line proofs.  JSON wire / cantools wire formats preserved by converting at boundaries.  **`c884e69`** subsumes `parseInt` into `parseDecRat = parseDecRatFrac <|> parseDecRatBareInt`; internal proofs renamed `parseDecRat-roundtrip-*` → `parseDecRatFrac-roundtrip-*`; new public wrapper via `alt-left-just`.  **`7a44c87`** completes the symmetry: `parseIntDecRat` / `parseNatDecRat` (`parseDecRat >>= ifᵀ predicateᵇ then mkRefined else fail`) replace `parseInt` / `parseNatural` in `parseIntType` / `parseHexType`.  Owed: `parseDecRat-bareInt-roundtrip-suffix` (~100 LOC, in-source TODO) for the AVInt path.  **Commits 3c per-line (TaskList #17–#22) + 3d (messages) + Layer 4 (top-level aggregator) pending.**  **B.3.e–j pending**: JSON command, Python feature-flag switch, cantools drop, C++/Go APIs, cross-binding parity.
...
**Status**: Phase 5.1 + Phase A + Phase B.1/B.1.x + Phase B.2 + Phase B.3.a/b/c + **Phase B.3.d pre-gate + layer 1 + layer 2** complete; **Phase B.3.d layer 3 in_progress** (3a `804c584` + 3b `ad111bf` shipped; 3c precursor `3a7c86e`/`c884e69`/`7a44c87` shipped 2026-04-25; 3c per-line proofs next); Layer 4 + Phase B.3.e–j → Phase C/D per plan.
