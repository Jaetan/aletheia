...
- `src/Aletheia/DBC/Properties.agda`
  ...
  - `DBCSignalsValid` and `dbcSignalsValid?`
  - Integrated into `message-well-formed` and `dbc-well-formed`
...
1. **Proof extraction from validated DBC**: Add lookup functions that return `SignalsDisjoint` proof for any two signals in a validated DBC (eliminates threading disjointness hypotheses)
2. **Property-based testing**: Generate random DBC files and verify validation correctness
3. **Performance**: Profile validation on large DBC files with many signals
