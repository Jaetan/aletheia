...
| **Commutativity** | **`disjoint-signals-commute-unsigned`** | **Signal-level monadic injection commutes** |
...
Remaining work to complete signal commutativity:

1. **`injectSignal-reduces-signed`** - Reduction lemma for signed signals (parallel to unsigned version)
2. **`disjoint-signals-commute-signed`** - Two signed signals commute (uses signed reduction lemma)
3. **`disjoint-signals-commute-mixed`** - One unsigned + one signed signal commute (combines both reduction lemmas)

Other remaining work:

4. **Fix roundtrip proof warnings** - The `rewrite` statements in roundtrip proofs are not applying; fix these warnings
5. **DBC signal overlap validation** - Check if DBC parsing validates that signals are non-overlapping; add the validation if missing
