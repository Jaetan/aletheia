...
**Proof audit complete. Steps 1-3 done.**

9. ✅ Batch/Properties.agda: Documents proof architecture connecting DBCValid → SignalsDisjoint → extract-disjoint-inject

The proof chain is complete:
- DBC validation (DBCValid) proves all signal pairs are disjoint
- lookupDisjointFromDBC extracts SignalsDisjoint for any two signals
- extract-disjoint-inject-* uses SignalsDisjoint to prove batch operations preserve values

## Next Session: Write buildFrame-preserves proof

Write the inductive proof that `buildFrame` (via `injectAll`) preserves all injected signal values:

```agda
buildFrame-preserves : ∀ sigs frame →
  AllPairsDisjoint sigs →
  injectAll frame sigs ≡ just frame' →
  ∀ (sig, v) ∈ sigs → extractSignal frame' sig ≡ just v
```

Structure:
1. Base case: empty list, trivial
2. Inductive case:
   - Inject first signal
   - By IH, rest are preserved through remaining injections
   - By `extract-disjoint-inject`, first signal survives remaining injections
3. Handle `Maybe` threading and byte order cases

Uses: `extract-disjoint-inject-unsigned/signed`, `lookupDisjointFromDBC`

---

Optional future work:

1. **Store DBCValid proof in StreamState**: Change `dbc : Maybe DBC` to `Maybe (Σ DBC DBCValid)` to enable proof extraction at runtime (not needed for correctness, just convenience)
