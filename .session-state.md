...
Remaining work from proof audit:

1. **DBC Proof extraction**: Add `lookupSignalPairValid` and `extractDisjointness` functions
2. **extract-disjoint-inject proof**: Prove extraction unaffected by injecting disjoint signal
3. **Batch operation proofs** (optional): extractAllSignals-complete, buildFrame-non-corruption
