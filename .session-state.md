...
**Status:** **Layer 4A+4B signature COMPLETE, proof body has hole at Properties:1105**
...
**Issue**: The `inspect` idiom causes unification problems with ℚ expressions.

**Solution**: Use helper lemmas instead of `inspect`:

1. **`injectSignal-result`**: Characterize the exact frame' returned by injectSignal
   when bounds-ok, removeScaling succeeds, and raw fits.

2. **`extractSignal-on-injected`**: Show extractSignal on characterized frame' returns just value.

3. Combine using `signal-roundtrip-unsigned` and `swapBytes-involutive`.

**Available helper lemmas in proof**:
- `remove-eq : removeScaling value factor offset ≡ just (+ n)`
- `fits-check : (n <? 2^bl) ≡ yes n<2^bl` (uses `dec-yes-irr`)
- `bounds-ok` (hypothesis)

**Hole location**: Properties.agda:1105
