...
### Cluster Y — Smart-fallback Rational renderer (cross-binding) ✅ CLOSED
- `GO-D-19.1` [FIX] — promoted from cluster R's Phase-6 framing per user redirect 2026-05-15.  Pretty-printer Rational rendering now classifies on the reduced denominator: terminating decimals (denom divides `2^a · 5^b` after GCD-reduction) keep the existing `:g` / `%g` rendering; non-terminating Rationals emit literal reduced `N/D`.  All three bindings updated symmetrically:
  - **Go** `go/aletheia/enrich.go`: new `formatRational(Rational) string` helper + `gcdInt64` + `absInt64` private helpers; nine call sites in `formatPredicate` flipped from `formatValue(v.Value.Float64())` to `formatRational(v.Value)` (Equals, LessThan, GreaterThan, LessThanOrEqual, GreaterThanOrEqual, Between min, Between max, ChangedBy, StableWithin).  Comment block at the function head documents the dispatch rule.
  - **C++** `cpp/src/enrich.cpp`: `format_value(const Rational&)` body replaced with the smart dispatch (GCD-reduce, strip 2s and 5s, classify); new `<numeric>` (for `std::gcd`) and `<cstdlib>` (for `std::abs`) includes.  All `format_value(v.value.get())` / `format_value(v.min.get())` / etc. call sites in `format_predicate` automatically pick up the new behaviour via overload resolution — no consumer-side changes needed.
  - **Python** `python/aletheia/client/_enrichment.py`: new `_format_rational(v: object) -> str` helper (handles `Fraction` directly via `numerator`/`denominator`; falls through to `_coerce_to_float(v):g` for `int` / `float` paths).  `_format_predicate` flipped from `_coerce_to_float(...):g` formatting to `_format_rational(...)`; the changedBy sign-discriminator still calls `_coerce_to_float` for the comparison but emits via `_format_rational` for both branches.
- **Reduction matters**: Go and C++ `Rational` direct-construction does NOT enforce lowest-terms (only the wire-JSON serializer in `cpp/src/json_serialize.cpp:53` reduces).  The renderer GCD-reduces internally so `Rational{2, 6}` renders as `"1/3"` (not `"2/6"`); Python `Fraction` is always lowest-terms by construction.  Defensive `denominator <= 0` early-return falls through to `:g` to avoid a div-by-zero in the GCD; the `Rational` constructor invariants make this branch unreachable in practice.
- **Test additions** (15 new cases across 3 bindings):
  - `go/aletheia/enrich_test.go`: `TestFormatFormula_NonTerminatingRational` (7 cases — equals 1/3, less_than 2/7, greater_than -1/3, between 1/3..2/3, reduced 2/6→1/3, changed_by 1/3, stable_within 1/7) + `TestFormatFormula_TerminatingRationalExact` (4 cases — 23/2, 7/8, 42/1, -23/2 — regression guard for the dominant DBC-source case).
  - `cpp/tests/unit_tests_check.cpp`: 5 new `TEST_CASE` blocks tagged `[enrich][rational]` (non-terminating equals, signed non-terminating, reduction-before-N/D, terminating-decimal regression guard, between with both bounds non-terminating).
  - `python/tests/test_enrichment.py`: 4 new tests in `TestFormatPredicate` (`test_non_terminating_rational_renders_as_fraction`, `test_non_terminating_rational_signed`, `test_terminating_rational_uses_decimal`, `test_between_non_terminating`) — exercises the user-facing `Signal("S").equals(Fraction(1, 3))` path so the DSL → renderer flow is covered end-to-end.
- **Wire format unchanged**: JSON `{"numerator": N, "denominator": D}` (R19P2 cluster 17) is the wire form; this commit only changes the *display* path.  Cross-binding integration tests hit the wire format and remain green.
- **No benchmark needed**: pretty-printer fires on violation enrichment, not per-frame; the `formatPredicate` / `format_predicate` / `_format_predicate` call graph is reachable only from `format_formula` / `BuildDiagnostic` / `format_enriched_reason`, none of which are on the streaming hot path.  Per [[feedback-hot-path-refactor-benchmark]], a refactor that's structurally off the hot path doesn't require the throughput run.
- **CHANGELOG**: `Changed` entry under `[2.0.0] — Unreleased` documents the rendering shift (terminating unchanged; non-terminating `1/3` now exact instead of `0.333333` truncation), positioned above the BREAKING entries since this is observable but additive in precision.
- New memory `memory/project_smart_rational_renderer.md` records the design (terminating-classification algorithm + sign handling + reduction discipline + cross-binding parity invariant).
...
- 2026-05-15 — Cluster Y shipped (smart-fallback Rational renderer, GO-D-19.1 promoted from cluster R Phase-6 framing per user redirect).  All three pretty-printers now classify by reduced denominator: terminating decimals via `:g` / `%g` (no regression for the dominant DBC-source case where `Rational{23, 2}` → `"11.5"`); non-terminating Rationals as literal reduced `N/D` (e.g. `Rational{1, 3}` → `"1/3"` instead of the truncated `"0.333333"`).  Per-binding scope: Go `formatRational` + `gcdInt64` + `absInt64` helpers added in `enrich.go`, 9 call sites flipped in `formatPredicate`; C++ `format_value(const Rational&)` body replaced with the smart dispatch, `<numeric>` and `<cstdlib>` includes added; Python `_format_rational` helper added in `_enrichment.py`, `_format_predicate` flipped from `_coerce_to_float(...):g` to `_format_rational(...)`.  GCD-reduces internally because Go and C++ Rational construction does not enforce lowest-terms; Python `Fraction` already lowest-terms.  15 new test cases across 3 bindings (7 Go + 5 C++ + 4 Python).  Wire JSON shape `{"numerator": N, "denominator": D}` unchanged — display-path-only refinement.  CHANGELOG `Changed` entry under `[2.0.0] — Unreleased`.  No benchmark needed (pretty-printer is off the streaming hot path per [[feedback-hot-path-refactor-benchmark]]).
