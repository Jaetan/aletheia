...
561. `[FIX]` GO-D-19.1 [HIGH] — ✅ Cluster Y: smart-fallback Rational renderer shipped across all three bindings.  The original FP rationale (cluster R, line 1155) identified "exact-when-terminating" as the Phase-6 alternative path; user redirect 2026-05-15 promoted that alternative into R20 scope.  All three pretty-printers now classify the reduced denominator: terminating-decimal Rationals (denom divides `2^a · 5^b` after GCD-reduction) keep the existing `:g` / `%g` rendering, while non-terminating Rationals emit literal `N/D` from the reduced form.  Common DBC-source case unchanged — `Rational{23, 2}` still → `"11.5"`; user-constructed `Rational{1, 3}` now → `"1/3"` instead of `"0.333333"`.  Wire JSON shape unchanged (`{"numerator": N, "denominator": D}` per R19P2 cluster 17).  See cluster Y closure block + 2026-05-15 progress-log entry for full per-binding scope and gate posture.
...
- `GO-A-3.5` ✅ DEFER-end-of-round closed (see line 128 disposition).
...
- `GO-D-19.1` [FP-then-FIX] — initially FP-verified at cluster R (cross-binding evidence: all three bindings share the `to_double + g-format` pattern; switching to literal `N/D` for the common terminating-decimal case would be a readability regression).  Cluster R left a Phase-6 enhancement on the table: a smart "exact-when-terminating, N/D-when-not" renderer that preserves the common case while fixing the `Rational{1, 3}` → `"0.333333"` precision loss.  User direction 2026-05-15 promoted that enhancement into R20 scope — closed in cluster Y (see closure block below).
...
### Cluster Y — Cross-binding-identical Rational renderer + DSL float-input parity ✅ CLOSED (2 commits)

**Cluster Y shipped in two commits**: `c6a7e73` first delivered the dispatch (terminating → `:g`/`%g`/`{:g}`, non-terminating → reduced `N/D`).  Pre-merge audit surfaced a residual cross-binding asymmetry: the three languages' `:g`/`%g`/`{:g}` defaults differ in precision (Python and C++ truncate to 6 sig figs; Go uses round-trip-shortest) and in scientific-notation thresholds, and Python's DSL `Fraction(float)` produces the exact IEEE 754 binary fraction (giant denom) while Go's `floatToRational` and C++'s `Rational::from_double` both round-and-scale via 10^9.  The user redirected to "option A: pin all bindings to the same display, do not let people be bitten by language-dependent representation," which expanded cluster Y to cover both renderer (A1) and DSL (A2) — shipped as a follow-up commit.

- `GO-D-19.1` [FIX] — promoted from cluster R's Phase-6 framing per user redirect 2026-05-15, then expanded again to cover the DSL construction asymmetry per user redirect 2026-05-15.  Final algorithm (cross-binding identical):

  **A1 — Renderer**: GCD-reduce; if the reduced denominator divides `2^a · 5^b`, compute the exact decimal via integer arithmetic (no floating-point) — pad the numerator into a `k = max(a, b)` digit-stream of length `≥ k+1`, split at the decimal point, trim trailing zeros from the fractional half.  Otherwise emit reduced `N/D`.  Pathological case `k > 18` falls back to `:g` / `%g` formatting (int64 multiplier overflow safety; typical DBC predicate values stay well under k=10).

  **A2 — Python DSL**: `to_predicate_fraction(value)` helper added in `python/aletheia/client/_helpers.py` that mirrors Go `floatToRational` (`client.go:196`) and C++ `Rational::from_double` (`types.cpp`) exactly: float inputs go through `Fraction(round(value * 10^9), 10^9)`.  `int` and existing `Fraction` inputs flow through unchanged.  All 9 DSL methods (`equals`, `less_than`, `greater_than`, `less_than_or_equal`, `greater_than_or_equal`, `between` min/max, `changed_by`, `stable_within`) flipped from `Fraction(value)` to `to_predicate_fraction(value)`.

- **Per-binding renderer scope**:
  - **Go** `go/aletheia/enrich.go`: `formatRational(Rational) string` body now exact-decimal (was `:g`-dispatch in `c6a7e73`); `gcdInt64` + `absInt64` private helpers retained from `c6a7e73`.
  - **C++** `cpp/src/enrich.cpp`: `format_value(const Rational&)` body replaced with exact-decimal algorithm (was `:g`-dispatch in `c6a7e73`); `<numeric>` + `<cstdlib>` includes retained.
  - **Python** `python/aletheia/client/_enrichment.py`: `_format_rational` body replaced with exact-decimal algorithm (was `:g`-dispatch in `c6a7e73`).

- **DSL change rationale**: Python `Signal("S").equals(0.1)` previously stored `Fraction(0.1)` = `Fraction(3602879701896397, 36028797018963968)` (the exact IEEE 754 binary fraction).  Without A2, the new exact-decimal renderer would emit a 56-character string for what the user thinks of as "0.1".  After A2, it stores `Fraction(1, 10)` and renders as `"0.1"` in all three bindings.

- **Reduction matters** (preserved from `c6a7e73`): Go and C++ `Rational` direct-construction does NOT enforce lowest-terms (only the wire-JSON serializer in `cpp/src/json_serialize.cpp:53` reduces).  The renderer GCD-reduces internally so `Rational{2, 6}` renders as `"1/3"` not `"2/6"`.  Python `Fraction` is always lowest-terms by construction.

- **Test additions** (cluster Y cumulative — `c6a7e73` plus this commit):
  - `go/aletheia/enrich_test.go`: `TestFormatFormula_NonTerminatingRational` (7 cases) + `TestFormatFormula_TerminatingRationalExact` (now 9 cases, extended in this commit with large-integer / 9-sig-fig / small-decimal / reduces-trims / zero coverage).
  - `cpp/tests/unit_tests_check.cpp`: 9 new `TEST_CASE` blocks tagged `[enrich][rational]` (5 from `c6a7e73` + 4 from this commit covering large integer / 9-sig-fig / no-scientific-for-small / reduces-trims).
  - `python/tests/test_enrichment.py`: 7 new tests in `TestFormatPredicate` (4 from `c6a7e73` + 3 from this commit: `test_dsl_float_0_1_renders_as_decimal`, `test_dsl_large_integer_no_scientific`, `test_dsl_high_precision_decimal_exact` — exercises the DSL float-input → renderer pipeline end-to-end).

- **Test corpus updates from A2 surface change**:
  - `python/tests/test_dsl_predicates.py::test_comparison_with_float`: assertion changed from `inner_pred['value'] == 12.6` to `== Fraction(63, 5)` because `Fraction(12.6)` (IEEE 754 noise) and `Fraction(63, 5)` (exact 12.6) compare unequal.  The 12.6 user input now produces the exact rational, not the IEEE-754-noise rational.
  - `python/tests/test_dsl_composition.py::test_fractional_values`: same adjustment for 12.65 → `Fraction(253, 20)`.

- **Wire format unchanged**: JSON `{"numerator": N, "denominator": D}` (R19P2 cluster 17) is the wire form; this commit only changes the *display* path AND the Python construction path.  Cross-binding integration tests hit the wire format and remain green.

- **No benchmark needed**: renderer is off the streaming hot path (display-only — `format_formula` / `BuildDiagnostic` / `format_enriched_reason`); DSL float→Fraction conversion happens once per predicate construction, not per frame.  Per [[feedback-hot-path-refactor-benchmark]].

- **CHANGELOG**: cluster Y `Changed` entry under `[2.0.0] — Unreleased` rewritten to reflect the cross-binding-identical algorithm + the DSL change as a separate `Changed` sub-entry.

- New memory `memory/project_smart_rational_renderer.md` rewritten to document the A1 algorithm + A2 DSL change + cross-binding parity invariant.
...
- 2026-05-15 — Cluster Y shipped (smart-fallback Rational renderer, GO-D-19.1 promoted from cluster R Phase-6 framing per user redirect).  Initial commit `c6a7e73`: dispatch terminating → `:g`/`%g`/`{:g}`, non-terminating → reduced `N/D`.  Pre-merge audit surfaced a residual cross-binding asymmetry (the three languages' `:g`/`%g`/`{:g}` defaults differ in precision and scientific-notation thresholds; Python's DSL `Fraction(float)` produces the exact IEEE 754 binary fraction while Go/C++ round-and-scale via 10^9).  User redirect: "option A — pin all bindings to the same display, do not let people be bitten by language-dependent representation."  Follow-up commit covers both renderer (A1: cross-binding-identical exact-decimal algorithm) and DSL (A2: `to_predicate_fraction` helper mirroring Go `floatToRational` exactly).  After A1+A2: same Rational value renders to byte-identical output across all 3 bindings; same `Signal.equals(0.1)` user call produces structurally identical Rationals across all 3 bindings.  22 new test cases cumulative (16 in `c6a7e73` + 6 in follow-up); 2 existing tests adjusted for the DSL exact-rational change (`test_comparison_with_float` and `test_fractional_values` now assert against the exact reduced Fraction rather than the IEEE-754-noise float).  Wire JSON shape unchanged.  No benchmark needed.
