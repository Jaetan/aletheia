...
#
# Self-location needs bash's BASH_SOURCE or zsh's $0; POSIX sh (dash) cannot find
...
_aletheia_here="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" >/dev/null 2>&1 && pwd)"
export ALETHEIA_LIB="${_aletheia_here}/lib/libaletheia-ffi.so"
unset _aletheia_here
