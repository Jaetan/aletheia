...
- B.3.d ⏳ Roundtrip theorem — **universal form is the target**: `∀ d → parseText (formatText d) ≡ inj₂ d`. Per-fixture point verifications (the "corpus-shape" approach attempted earlier and reverted) are not an acceptable substitute — the done-criterion is a theorem that quantifies over the `DBC` domain, not a finite conjunction of concrete equalities. Corpus-based coverage belongs to B.3.j (binding-layer parity against cantools snapshots); B.3.d is the Agda-side property.
  ...
  * **Commit 3d.4 + JSON-mirror IN PROGRESS (uncommitted WIP on branch `b3d-3d4-json-detaint`, 2026-04-27).**  Architectural pivot 2026-04-27: original 3d.4 plan (Point 3 from architectural review 2026-04-26) had a load-bearing gap.  Post-3d.4, with `Identifier.name : List Char`, the JSON-side per-construct roundtrip lemma `validateIdent (nameStr i) ≡ inj₂ i` reduces to `mkIdentFromChars (toList (fromList chars))` — closing this requires `toList∘fromList` axiom.  Pre-3d.4 the witness type alignment was definitional; 3d.4 broke that for the JSON side in exchange for de-tainting the text-parser path.  The original "axioms only at outer parseText/formatText wrap" promise missed that JSON has multiple String boundaries (one per JString site).

    **User decision (2026-04-27): Option 2 — mirror 3d.4 fix on JSON side.**  Three steps land together as one mega-commit (entanglement: any subset breaks roundtrip lemmas with `toList∘fromList` or `fromList∘toList` axioms):

    1. **Step 1 — JSON spine.**  `Aletheia/Protocol/JSON/Types.agda`: `JString : List Char → JSON` (was `String → JSON`).  Tokenizer drops the `fromList` (`Parse.agda:158: pure (JString chars)`).  `Format.agda` inserts `fromList` at wire-output only.  `Lookup.agda` adds `getChars`/`lookupChars` returning chars directly, alongside existing `getString`/`lookupString` (kept axiom-safe via internal `fromList` for kind-discriminator-only paths).  Add `JStringS = JString ∘ toList` ergonomic helper for literal-string JSON values.  ~140 cascade sites across formatters/parsers.
    2. **Step 2 — DBC AST text fields → List Char.**  Retype `DBCSignal.unit`, `DBCComment.text`, `DBC.version`, `AVString` payload, `AttrDef/Default/Assign.name`, `ATEnum.values`, `ValueTable.entries.description` from `String` to `List Char`.  Accessors mirror the `nameStr` pattern: `unitStr`, `commentTextStr`, `versionStr`, existing `attr*NameStr` re-derived via `fromList ∘ field`.  `ParseError.detail` carve-out (diagnostic only, not roundtripped, no axiom site).
    3. **Step 3 — Identifier de-tainting (original 3d.4).**  Refactor `Aletheia/DBC/Identifier.agda` from `record { name : String; valid : T (validIdentifierᵇ (toList name)) }` → `record { name : List Char; valid : T (validIdentifierᵇ name) }`.  Lexer drops `pure (fromList chars)` + `toList∘fromList` axiom dependency.  Carries through ~50% from prior session WIP (Identifier + Substrate.Unsafe + Lexer + Types accessors + TextFormatter sub-modules + Validator/Checks/SignalExtraction/Formatter partial).

    **Status this session (uncommitted WIP):**
    - ✅ Step 1 spine: JSON.{Types,Parse,Format,Lookup}.agda updated.
    - ✅ Step 2 spine: DBC/Types.agda field retypes + accessors added.
    - ⏳ Step 3 source-side: ~50% from prior session WIP carries through; ~30 files pending mechanical `Identifier.name X` → `nameStr X` for String-context consumers.
    - ⏳ Cascade Steps 1+2 through formatters/parsers: `DBC/Formatter.agda` (~48 JString), `DBC/JSONParser.agda` (~35 lookupString), `LTL/JSON/Format.agda` (23 JString), `Protocol/ResponseFormat.agda` (21 JString), `DBC/Formatter/MetadataRoundtrip.agda` (15 JString), `Protocol/JSON/Properties.agda` (15 JString), `JSONParser/{MessageWF,SignalWF}.agda`, `LTL/JSON.agda` (11 lookupString), etc.
    - ⏳ Per-construct JSON roundtrip proofs: re-prove axiom-free at List Char level (`Formatter/MetadataRoundtrip.agda` keystone `validateIdent-roundtrip`, `MessageRoundtrip*`, `SignalRoundtrip`, `WellFormed`, outer composer `Formatter/Properties.agda`).
    - ⏳ Text parser proof reshape (`Properties/Primitives.agda` `parseIdentifier-roundtrip` keystone simplification — drops `mkIdentFromCharsUnsafe-on-valid` path; `Preamble/Namespace.agda` parseIdentifier-consumes simplification; ~10 per-construct downstream proofs).
    - ⏳ Module flag invariants: `Shakefile.hs` allowlist down to `{Substrate.Unsafe}`; `CLAUDE.md` 132/4/48 → 179/4/1.
    - Cannot land partially per pre-commit scope gate (`feedback_pre_commit_scope_check.md`).  See `.session-state.md` for the per-file checklist and resumption strategy.

    **Net effect:** `136 / 184 modules use --safe` becomes `183 / 184 modules use --safe`.  Per-proof LOC reduction ~10–15% across Layer 3.  Substrate.Unsafe.agda's only remaining job becomes the outer parseText/formatText wrap.  All JSON-side per-construct roundtrips axiom-free; zero JSON-roundtrip modules dropped to `--without-K`-only.

    **Gates:** `cabal run shake -- check-properties` green; `check-invariants` allowlist (single Substrate.Unsafe still); all bindings tests green (Python/C++/Go) — JSON wire format preserved (formatter still emits the same UTF-8 bytes via `fromList chars` at `Format.agda` wire-output); benchmark signal-extraction hot path within ±5%, frame-building within ±5% (text-typed AST field retype `Text → [Char]` may shift MAlonzo runtime cost — `unit` not on per-frame extraction path; `comment.text`/`version` loaded once); wire-format diff capture (e.g. `python/tests/fixtures/dbc_corpus/kitchen_sink.dbc` → DBC AST → format JSON → bytes) against `main` HEAD baseline = empty diff.

    **Risks:** MAlonzo `String → List Char` representation change (`Data.Text.Text` → `[Char]`) on text-typed AST fields could regress JSON encode by `O(n)` per field — measure during commit gate; mitigation if regression >5%: cache String accessor at constructor OR keep String at field with `toList`-only formatter path.  Cross-binding API: bindings consume JSON wire bytes; not affected by AST-internal type change.

    **Estimate:** ~3-4 weeks (was ~1w for original 3d.4 alone; JSON-mirror sub-steps add ~2-3w).  Post-merge: text parser proof keystone reshape (`Properties/Primitives.agda` `parseIdentifier-roundtrip`) simplifies substantially since axiom uses around the Identifier round-trip drop out.
