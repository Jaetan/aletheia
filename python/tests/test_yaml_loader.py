...
class TestLoadFromFile:
    ...
    def test_string_path_now_treated_as_inline_yaml(self, tmp_path: Path) -> None:
        ...
        Dispatch is strict by parameter type (``Path`` → file, ``str`` →
        inline YAML), closing the path-confusion vector.  A string that
        happens to name an existing file is no longer opened; it is fed
        to ``yaml.safe_load`` as a YAML scalar.  Callers who want file
        behaviour wrap in ``pathlib.Path``.
        """
...
class TestLoaderHardening:
    """Symlinked YAML refused outright.

    Mirrors ``aletheia::detail::validate_loader_path`` in C++.  Inline YAML
    strings are unaffected — the symlink check only applies to ``Path``
    inputs.
    """

    def test_symlink_path_rejected(self, tmp_path: Path) -> None:
        """Symlinked .yaml is refused outright by the loader hardening."""
