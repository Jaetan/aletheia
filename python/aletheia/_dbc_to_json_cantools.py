"""Cantools-backed DBC parser — slated for removal in B.3.g.

This module isolates every ``cantools`` type reference and helper used by
``dbc_to_json(parser="cantools")``. The default Agda-backed path lives in
``dbc_converter.py`` and never touches cantools, so importing
``aletheia.dbc_converter`` no longer requires the cantools extra.

When B.3.g lands (~8 weeks after B.3.j) this whole file disappears together
with the ``parser="cantools"`` dispatch arm.
"""
...
def dbc_to_json_cantools(dbc_path: str | Path) -> DBCDefinition:
    """Convert a .dbc file to JSON via cantools.

    This is the legacy path; ``parser="agda"`` (the new default) routes
    through the verified Agda text parser instead and does not require the
    cantools dependency. Slated for removal in B.3.g.
    """
    try:
