...
These mirror module-private CLI internals so benchmark scripts and external
test harnesses can exercise the same code path the CLI runs without reaching
into private symbols.  Stability guarantees match ``aletheia.cli`` itself
(implementation surface, not a long-term public API) — minor releases may
adjust signatures with documented migration notes.
...
contents (CLI re-exports) are independent of that decision.
"""
