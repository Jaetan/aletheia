...
**2026-06-07 — pre-merge item 1 (AletheiaError) DONE + full Python namespace-organization review DONE (6 commits)**: `aletheia` is now the sole public package; domain logic in clean sub-namespaces; top-level stays convenience-complete (Tier 2=A, 50 names).  `06343c0` item 1 (`aletheia.client`→internal, no public re-exports; guard test locks it) · `738fc28` Tier 1 (`__all__` on can_log/dsl/excel_loader/checks/yaml_loader) · `0bc4a59` Tier 3a (`dbc_converter`+`dbc_queries`→`aletheia.dbc` pkg) · `5e00dec` Tier 3c (`error_codes`+`issue_codes`→`aletheia.codes` pkg) · `1287459` Tier 3b (`protocols`→`aletheia.types`, 83-file rename).  Each gates-green (ruff(ALL)/bp-0/pylint-10/4827p/doc-harness/mutation/changelog); example+benchmark imports re-verified.  A first over-aggressive pass (also demoting dbc/codes/dsl/loaders) was REVERTED — meaningful sub-namespaces are good; only smell = private-def + redundant public facade.  **Pre-merge program: item 1 ✅; NEXT = item 2 Fraction B/C** [[project_object_sweep]].  Detail: `memory/project_ci_speed_optimization.md` (2026-06-07).
