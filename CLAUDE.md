...
**R22 A-1.1 batches 7-25f (2026-05-18 → 2026-05-20)** — batches 7-9 landed on `review-r21` pre-merge; batches 10-25f on `review-r22` post-merge.  Per-batch narrative + commit hashes consolidated in `memory/project_review_round21.md`; summary:
...
**Cumulative R21/R22 A-1.1 closure: 135 files / ~1646 candidates**.  Scanner v2.6 at `~/.local/bin/scan_aletheia_dead_imports.py` (persistent across WSL restarts; not committed to git per WSL-resilience-via-out-of-repo-PATH rule; rebuild from `feedback_unicode_brackets_scanner.md` + b23 commit body's v2.6 multi-line fix if ever lost).  **Scanner FP rate climbs with depth into proof tree, stabilises in long-tail** (b25b 3.3% → b25e 26.9% → b25f 25.5%): mixfix `_≡_` at theorem signatures + every `_X_` operator at heavy-mixfix parser surfaces are highest-recurrence FP forms.  **Known scanner FPs**: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉`/`-[1+_]` outside ident class; (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` body tokens); (3) `public` re-exports (v2.4 line-skip).  **Known FN classes**: heavy `using (...)` clauses, `import M as N` module aliases, multi-line `using\n  (`, `_X_` mixfix-with-args (`∣_∣`, `ifᵀ_then_else_`, `-[1+_]`, `_≈ᵇ_`, `_≡_`, `_<|>_`, `_∷_`, `_+_`, `_≤ᵇ_`, `_⊎_`, `_*>_`, `_>>=_`, `_>>=ₑ_`), **`module X` literal with qualified `X.field` body** (b25e), `renaming (X to Y)` clauses (scanner doesn't process at all).  **Workflow**: per-name `grep -cP` audit after scanner pass + `simplify` skill quality-review agent post-prune (catches residual `renaming` + within-using-clause items).

**Gates fresh at `f042b31`** (R22 A-1.1 batch 25f landing, 2026-05-20): build clean **2m03s** / check-properties clean **12m06s** "All proof modules type-checked successfully" / count-modules **263** (unchanged across b19-b25f).  Per-batch gate timings b25b/c/d/e/f: build 3m05s/2m32s/2m36s/2m31s/2m03s · check-properties 11m11s/9m02s/13m11s/11m50s/12m06s.  Binding tests NOT re-run for b25-series — dead-import pruning is MAlonzo-neutral.  Last green binding posture at `dd8bb81`: pytest 870p+1s (6.27s), Go -race ok 7.343s, C++ ctest 10/10 (22.78s).  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 long-tail OR pivot to other R22 carry-overs.  Top remaining per fresh v2.6 scan post-b25f (676 dead / 183 files): Receivers/Roundtrip (9), JSON/Parse (8 — 8 stuck mixfix FPs), Handlers (8), Validator/Checks (8), TextParser (8), ValueDesc (8), Dispatcher (8), Dispatcher/Attribute/Foundations (8), Format (8), JSONParser (8).  Long-tail batches should budget 25-30% scanner FP rate; per-name `grep -cP` audits mandatory for mixfix and `module X` clauses.  **AGDA-D-12.1 follow-ups REQUIRED** unchanged: per-warning enumerant + cross-binding parity + check-runbook + PROTOCOL.md (in-source block at `Adequacy/StreamingWarm.agda:392-406`).  **Pre-close blocker** unchanged: 18 `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` (see in-source `R22-PRE-CLOSE` block).  **Push gap**: local `main` 152 commits ahead of `origin/main` — R20+R21 merges held local per `feedback_review_branch_workflow.md`.
