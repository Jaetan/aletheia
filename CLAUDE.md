...
**Gates fresh at `0b360fd`** (R22 AttrLine FIX landing, 2026-05-18): 12 Agda gates clean (build 3m04s / check-properties **10m47s** All proof modules type-checked successfully / check-invariants / check-no-properties-in-runtime / check-erasure 1m45s / check-fidelity 17/17 (1m52s) / check-ffi-exports 45 entries / check-bound-enforcement 7 BoundKind ctors / count-modules **263** / check-runbook 15 events / check-changelog / check-limits-parity 14+7 in parity).  Binding tests (pytest / Go / C++) NOT re-run — AttrLine FIX is purely proof-side restructure (sibling Builders.agda not reached by Main.agda runtime closure; `.so` rebuild identical to prior commit; check-no-properties-in-runtime confirms no runtime leakage).  Five split patterns demonstrated this round: facade-mediated re-export (Primitives via Properties.agda, Line/Alt5 via Properties/Attributes), Shake walk-root (RegressionTests), parent-imports-back (AttrDef + Assign), external-consumer-redirect (ValueDescriptions + AttrLine/Builders).  **All 8 AGDA-D-15.1 candidates closed** — AttrLine FIX scope-deviation lesson saved as `feedback_advisor_not_user_proxy.md` (advisor approval is technical review not user authorization; ask user before deviating from agreed scope).  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline; not re-run for R22 since none of the touched paths are streaming hot path.

**Next**: `--no-ff` merge of `review-r21` to `main` per R18/R19/R20 precedent.  AttrLine.agda FIX landed at `0b360fd` 2026-05-18; AGDA-D-15.1 fully closed.  R22 carry-over scope (suggested): continue AGDA-A-1.1 dead-import sweep (~620 candidates / ~130 files remaining; scanner v2.2 procedure stable).  Optional AGDA-D-12.1 follow-ups (per-warning `endstream.uncached_atom` LogEvent + check-runbook entry + PROTOCOL.md write-up; all documented in `StreamingWarm.agda` DEFER block — none blocking).
