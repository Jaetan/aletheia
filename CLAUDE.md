...
**✅ DISTRIBUTION BUNDLE COMPLETE ✅ MERGED 2026-07-17 (#198 keyless release · #200 env constructors · #201 payload `0bb7f2ac`).** A `v*` tag → `shake dist` → **keyless-sign** (Sigstore / GitHub Actions ambient OIDC — NO key/passphrase in CI) → **self-verify vs the workflow OIDC identity, publish gated** → DRAFT Release. `shake dist` now ALSO bundles all four language wrappers (`bindings/{python,cpp,go,rust}`, library files via `git archive HEAD` — no tests, no `go.work`) + `env.sh`/`env.fish` (self-locating, ABSOLUTE `ALETHEIA_LIB`) + `install.sh`/`install.fish` (print per-shell/per-lang wiring, never edit an rc file) + `LICENSE`; all four bindings read `ALETHEIA_LIB` (Go `NewFFIBackendFromEnv` / C++ `make_ffi_backend_from_env` added #200). Gated by a dist **self-check** (proven teeth: fails on a dropped binding / leaked `go.work`) + a release-workflow **functional smoke** (unpack → source env.sh → load the `.so` from the bundled Python pkg). **Distribution = GitHub Release + bundle, NOT pip** (`--bignum=native` not needed; libgmp system-linked, BSD-2-clean). 2 adversarial rounds + 6 Copilot + 1 self-review fix (quoted emitted paths / fish `set -gx` / zsh-`%x` robust env.sh). Detail: PROJECT_STATUS § Phase 6 + `memory/project_r26_product_gaps.md` + git #198/#200/#201.
