...
**Active branch (2026-05-16): `review-r20` — READY TO MERGE**. Forked from `main` at `2e79ed8` 2026-05-12 (post R19 merge `41f6ba6` + post-merge tidy `2e79ed8`; pushed to remote). Live commit count via `git rev-list --count main..HEAD` per `feedback_self_referential_count_drift.md`. Module count **250** (R20 per-cluster drift recorded in `memory/project_review_round20.md`; clusters X and GO-A-3.5 are non-Agda; cluster Y stage 2 added `Aletheia.DBC.RationalRenderer` + Properties submodule; Phase C1 / R20-GO-A-4.10 / R5-B1+R6-B7.1 / R20-AGDA-B-26.3+GA9.1 — the five post-Y items landed this session — touched LTL/Protocol/DBC/Encoding modules but added no new modules).

**R20 review round** (started 2026-05-12, **DEFER queue empty 2026-05-16**) — 17 review agents → **671 raw findings** (`3571702`); cluster split (`3fa8e65`) into Clusters A-V + DEFER-end-of-round; clusters W, X, Y added during pass-2; GO-A-3.5 sub-finding closed in pass-2; five additional user-redirected closures landed post-Y (Phase C1, R20-GO-A-4.10, R5-B1/R6-B7.1 JSON path, R5-B1/R6-B7.1 text-path follow-up, R20-AGDA-B-26.3+GA9.1). **Clusters A-Y ✅ CLOSED + GO-A-3.5 ✅ CLOSED + Phase C1 ✅ + R20-GO-A-4.10 ✅ + R5-B1/R6-B7.1 ✅ (both parse surfaces) + R20-AGDA-B-26.3 + GA9.1 ✅** + regen safety fix + N/P/S follow-ups + DEFERRALS.md audit-trail flips (R5-A11, R6-P1.1, R6-B9.1, R6-B7.3).  R20-AGDA-B-18.3 stays DEFER with prominent in-source DO-NOT-RE-RAISE block. Per-cluster commit hashes, scope, dispositions, lessons distilled: `memory/project_review_round20.md`. WSL2 jitter empirical: median 1.70%, max 4.58% inter-batch CV (`feedback_wsl2_variance_stance.md`).
...
**Phase C1** (`92e7eaa`) — **R6-B7.2 — Metric* operators lift ℕ → `Timestamp μs`**. The original DEFERRALS.md NO-FIX rationale ("window is a frame count rather than microseconds") was factually wrong — `Aletheia.LTL.Coalgebra.stepL` does microsecond-vs-microsecond comparison `metricElapsed s curr ≤ᵇ tsValue w`. `MetricEventually` / `MetricAlways` / `MetricUntil` / `MetricRelease` now take their `window` parameter as `Timestamp μs` (a wrapped `ℕ` with microsecond dimension tag) rather than a bare `ℕ`. The JSON wire shape is unchanged — the LTL JSON parser wraps incoming `ℕ` via `mkTs` at the boundary, and the formatter unwraps via `tsValue`. `startTime` slot stays a suc-encoded `ℕ` because the `until` / `release` ADTs need the suc-form for structural recursion. Audit-trail pass in the same commit flipped four other DEFERRALS.md NO-FIX items to ✅ / 🟢: R5-A11 (`M.map` closure already rewritten in `bf238b3`), R6-P1.1 (rejected unbounded `*` reasoning — combinators structurally recurse on `length input`), R6-B9.1 (`Char ≟ Char` already optimised via `bool_decide_eq_char`), R6-B7.3 (cardinality bound on `compatibleSignalsPV` — held as 🟢 not-required since handler-boundary now enforces). 16 files / +298 / -158. Module count unchanged at 250.

**R20-GO-A-4.10** (`7cb131a`) — **check-limits-parity gate** (Agda Limits SSOT vs Go mirror). New `tools/check_limits_parity.py` (291 LOC) parses `src/Aletheia/Limits.agda` Limits records (`max-identifier-length`, `max-attribute-name-length`, `max-attribute-value-length`, `max-string-payload-length`, `max-comment-length`, `max-nesting-depth`, `max-atom-count`, `max-dbc-input-bytes`) and asserts they match the constants in `go/aletheia/limits.go` (one-shot import-time check). New `phony "check-limits-parity"` in `Shakefile.hs`; `tools/run_ci.py` step 14 wired in (always-on step count 27 → 28; header step count updated in run_ci.py docstring; steps 14-27 renumbered to 15-28). Originally tracked as cluster Q `GO-A-4.10` DROP ("CI value-parity check is a tooling task"); user redirect 2026-05-15 promoted it to a real gate. 5 files / +363 / -20.

**R5-B1 + R6-B7.1, JSON path** (`aa308c8`) — **LE bitLength=0 → parse error** (BE-LE parity completion). Closes the asymmetry where `PhysicallyValid.pv-BE` carried a `1 ≤ bitLength` constraint but `pv-LE` did not. `Aletheia.DBC.JSONParser.physicalGate` now checks `ifᵀ 1 ≤ᵇ bl then ... else inj₁ (ParseErr SignalBitLengthZero)` for BOTH byte orders (BE was already rejected since 2026-04-08). `pv-LE` constructor gains a `1 ≤ SignalDef.bitLength sd` argument parallel to pv-BE's existing `len-pos`. Affected proof sites: `JSONParser/SignalWF.agda::postPresence-wf×pv` LE case, `Formatter/SignalRoundtrip.agda::signal-roundtrip-LE` (gains `len-pos` parameter), `TextParser/Properties/Topology/Resolve.agda::convert-unconvert-id`. Cross-binding test updates: Python `TestBitLengthZero` reworked into byte-order-explicit pair (LE + BE both expect `ProtocolError` with `code = parse_signal_bit_length_zero` from `validate_dbc`); C++ added LE parity test + replaced "bypasses physicalGate" test with "rejected at parse" (notes the `parse_dbc` → `ErrorKind::Protocol` vs `validate_dbc` → `ErrorKind::Validation` asymmetry); Go's MockBackend-based parse-error tests are byte-order-agnostic. 10 files / +180 / -74.

**R5-B1 + R6-B7.1 follow-up, text path** (`35e306f`) — **LE bitLength=0 → parse error, text-parser surface** (closes advisor-flagged unilateral scope deferral on `aa308c8`). User said "promote it in Agda" (full scope); `aa308c8` shipped JSON only and documented the text parser as "a separate finding" — exact pattern `feedback_no_unilateral_deferral.md` warns against; advisor on the doc sweep caught it and forced a 4th commit. `TextParser/Topology/SignalLine.agda::buildSignal` now wraps the record construction in `if 1 ≤ᵇ bl then just <record> else nothing`; the `nothing` propagates through `buildAllRaw` → `resolveSignalList` → `buildMessage` (parser `fail`) and surfaces at the top level as `DBCTextParseError.ParseFailure` (wire code `dbc_text_parse_failure`). Text-parser error vocabulary intentionally coarser than the JSON parser's typed `parse_signal_bit_length_zero` — `DBCTextParseError` has three constructors and expanding it to mirror JSON's per-cause codes is out of scope. Functional outcome identical: zero-length LE signals are rejected at parse time on every external entry point. `Properties/Topology/Resolve.agda`: new `physicallyValid-len-pos` accessor extracts `1 ≤ bitLength` from `PhysicallyValid`; `buildSignal-fields-recover` now equates the full `if … then just record else nothing` LHS to `just (clearVds sig)` via rewrite chain `startBit-recovers → bitLength-mod-id → T→true (≤⇒≤ᵇ physicallyValid-len-pos) → presence-eq`; `buildSignal-roundtrip-Always/-When` drop `cong just` wrapper. Python test `test_zero_length_le_signal_rejected_at_parse` parameterises a DBC template over `length` — `length=1` parses to `status=success`, `length=0` parses to `dbc_text_*` error, so bl=0 is the only differentiator (advisor catch on the doc sweep: without the success arm, any incidental parse failure satisfies the error assertion without exercising the new gate — new feedback `memory/feedback_test_guard_parameterise_over_diff.md`). 5 files / +148 / -74.

**R20-AGDA-B-26.3 + GA9.1** (`f478bb5`) — **`injectHelper` lifted to top-level + Bool fast-path via `Reflects`**.  User-directed re-examination 2026-05-16 of R19 cluster D + F's RE-DEFER framing.  Hypothesis was "this is probably a proof architecture issue — lemmas that are local instead of global, piling 'with', breaking abstraction levels" — right in spirit but needed refinement: encapsulation alone (cluster D approach 2's `mkBoundedBitVec` with `with ... in eq`) hit the same trap inside the smart constructor's reduction lemma.  The structural fix is to swap the dispatch mechanism INSIDE the smart constructor from `with ... in eq` (proof-equation witness) to stdlib's `Reflects` (data witness via `<ᵇ-reflects-<` + the constructor pattern `ofʸ`/`ofⁿ`).  Three coordinated changes: (1) `injectHelper` lifted from where-bound to top-level in `Aletheia.CAN.Encoding`; new top-level reduction lemmas `injectSignal-bounds-{true,false}` dispatch the outer `inBounds` guard via single-line `rewrite`; (2) new smart constructor `Aletheia.Data.BitVec.Conversion.mkBoundedBitVec : (n bl : ℕ) → Maybe (BitVec bl)` using two-with `Reflects` form (`with n <ᵇ bl | <ᵇ-reflects-< n bl`) — two-with required under `--without-K` because single-with on `Reflects A (n <ᵇ bl)` hits SplitError on the open Boolean index; (3) `injectHelper`'s Dec dispatch (`_<?_`) swapped for `mkBoundedBitVec` (MAlonzo output confirmed: `_<?_` and Dec constructor gone from `MAlonzo.Code.Aletheia.CAN.Encoding`).  Proofs in `Encoding/Properties/Roundtrip.agda` and `Encoding/Properties/Disjoint.agda` decomposed: `injectSignal-reduces-{unsigned,signed}` and `injectSignal-preserves-disjoint-bits{,-physical}` now `trans`-compose a one-line outer-bounds lemma with a 2-deep helper-level lemma (`injectHelper-reduces-*` / `injectHelper-preserves-disjoint-bits*`); the 3-deep `with`-chain mirror is gone.  R20-AGDA-B-18.3 stays DEFER but strengthened: the `... | nothing = nothing` arm on `mkBoundedBitVec`'s result is still structurally required by coverage (provably dead under bound-injection invariant) — added 14-line ALL-CAPS "AGDA-B-18.3 — DO NOT RE-RAISE IN REVIEW" in-source block above the branch documenting the cascade-cost rationale (~30+ call sites would need a `WellFormedSignal` precondition threaded through).  Minimal reproducer at `/tmp/agda-with-in-eq-repro/MinimalRepro.agda` (75 lines, stdlib-only, two sections — Section A reproduces the `[UnequalTerms]` "ill-typed with-abstraction" error with `with ... in eq`, Section B shows the working Reflects two-with form).  Perf: no measurable Frame Building gain over the post-`@0` baseline (R19 cluster D's `@0`-erasure already captured the throughput win; deltas across all 3 bindings within WSL2 session-distant ±10% jitter).  MAlonzo emits `Reflects.fromEquivalence` → `du_of_30` + 2 closures per call — one heap cell, similar to post-`@0` Dec.  The architectural win is proof clarity, not throughput.  `memory/feedback_with_in_eq_outer_abstraction_barrier.md` rewritten: barrier description preserved, "workaround: keep Dec" conclusion corrected — `Reflects` is the structural escape hatch the cluster D + F four-approach probe didn't try.  4 files / +249 / -149 (refactor proper) + 2 files / +102 / -17 (CHANGELOG + DEFERRALS doc sweep, same commit).  Module count unchanged at 250.

**Next**: `--no-ff` merge of `review-r20` to `main` per R18/R19 precedent. DEFER queue empty.

**Gates fresh at every cluster ship**: **12** Agda gates clean (build + check-properties / check-invariants / check-no-properties-in-runtime / check-erasure / check-fidelity / check-ffi-exports / check-bound-enforcement / count-modules / check-runbook / check-changelog / **check-limits-parity** [new this session]); pytest **856p+1s** post-`f478bb5` (env lacks `hypothesis` for the optional `test_property_hypothesis.py` — pre-existing on stashed baseline; pre-`f478bb5` counts were 866p+1s when `hypothesis` was present); Go test -race ok 7.861s on `go/aletheia/`; C++ ctest 10/10 (24.57s); basedpyright 0/0/0; pylint 10.00/10 on `aletheia/ tests/ ../conftest.py`; gofmt + go vet + clang-format + clang-tidy clean on canonical `tools/run_ci.py:506` src scope; doc-fence harness 116/116; check-fidelity 17/17.  Cluster X throughput benchmark vs `cpp_throughput_baseline.json` 2026-05-12: max deviation +8.55% Stream LTL with σ 4.77% — within session-distant ±10% per `feedback_wsl2_variance_stance.md`; CAN-FD lanes ≤ +1.10% with σ ≤ 0.88% confirming the composition refactor is codegen-identical. GO-A-3.5 is a pure source-only identifier rename — generated machine code identical, no benchmark needed per `feedback_hot_path_refactor_benchmark.md`. Cluster Y (all stages) is display-path only (pretty-printer reachable from `format_formula` / `BuildDiagnostic` / `format_enriched_reason`, none on streaming hot path) — no benchmark needed; lazy-load is one-time per process, not per-frame. Phase C1 / R20-GO-A-4.10 / R5-B1+R6-B7.1 are not on the streaming hot path either (Metric* op constructor signature is parse-time, check-limits-parity is a gate not a kernel module, parser bitlength gate is parse-time rejection — no benchmarks needed).  **R20-AGDA-B-26.3+GA9.1** (`f478bb5`) IS on the streaming hot path (frame-build via `injectSignal`); benchmark verified: Frame Building 2.0B deltas Python +4.9% / C++ −3.1% / Go −3.9%, Stream LTL FD deltas −4.4% / −5.9% / −5.3% across bindings — all within WSL2 session-distant ±10% jitter, no codegen regression; the Bool-via-Reflects + post-`@0` net heap cost is structurally similar to post-`@0` Dec.
