...
**Next**: `--no-ff` merge of `review-r20` to `main` per R18/R19 precedent. End-of-round DEFER queue empty; DEFERRALS.md long-term residuals reduced by 3 (R20-AGDA-C-27.1 reclassified DEFER → DROP; R6-B7.3 + R6-B7.4 ✅ RESOLVED). Every remaining DEFER/DROP/FP-VERIFIED item carries an in-source DO-NOT-RE-RAISE block visible at the call site — `3194ea4` closed the last three weak-coverage gaps (`nothing≢just` helper, `StandardID`/typedef asymmetry, lockCh/closeOnce split).
