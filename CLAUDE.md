...
**🔬 E.3 TIGHTNESS + VALIDATOR MIRRORS ✅ MERGED 2026-07-20 (#218 prune `16521fc1` · #219 classification `67df5a7b` · #220 mirrors `50e4bf19`).** Every diagnostic-bearing WF condition behind `wfTextIssues` carries a PROVEN verdict in the checker's module header (tight / merely-bundled / unreachable; the witness oracle = always-strict `format_dbc_text`; review hypotheses overturned on the geometry bounds and the attribute arms) · `format_dbc_text` docs in every binding state the guarantee's exact level (parser-level; load-error-class duplicates round-trip in the proof but the validating load refuses the emitted text) · the structural validator now names the round-trip-fatal mux shapes warning-class at validate/load (CHECK 24/25; deciders SSOT via `WellFormedCheck/Foundations` — validator and formatter cannot disagree; runtime closure grew by exactly that module, snapshot regenerated in-diff; nested-mux now correctly warns, stale "validates clean" tests re-pinned across the bindings) · DEFERRED_ITEMS: E.3 closed + backlog pruned of every DONE tombstone; NEW items J.1 (normalize-vs-refuse JSON geometry boundary) / E.7 (provably-dead diagnostic arms disposition) / E.8 (proof-surplus hypotheses). Detail: CHANGELOG + the `WellFormedCheck.agda` header + git #218-#220.
