...
**🦀 RUST PARITY R3a (check DSL + add_checks) ✅ COMPLETE 2026-06-18** (`main` `e6403d3`, #59) — fluent `check` module (`rust/src/check.rs`): `check::signal("S").never_exceeds/never_below/stays_between/never_equals/equals().always()/settles_between().within(…)` + causal `check::when("A").exceeds/equals/drops_below.then("B").equals/exceeds/stays_between.within(ms)` compile domain checks to LTL `Formula`s + display metadata (`Check` carries name/severity/condition; `named`/`with_severity`). `Client::add_checks(&[Check])` binds them via `set_properties` (property_index = position; metadata client-side). Idiomatic Rust surface: `impl Into<Rational>` (+ `From<i64>`), `u64` ms (negative time unrepresentable), `Result` range/overflow guards; raw LTL combinators stay on `Formula`. `settles_between → MetricAlways` (= Go `AlwaysWithin` = Python `for_at_least`, verified). 2 rows → implemented (rust **28/40**). Copilot: 3 fixes (i128 `rational_le`; rendered when/then range error; non-vacuous conforming test). Next: **R3b** (YAML loader), **R3c** (Excel crate).

**🦀 RUST PARITY R2 (frame construction) ✅ COMPLETE 2026-06-18** (`main` `d50f2c2`, #57) — `Client::build_frame`/`update_frame` encode named signal values into a CAN payload via the binary build/update FFI (output-buffer convention); they take a typed `&DbcMessage` and resolve names→indices positionally — **idiomatic Rust, no client-side DBC cache** (user guidance 2026-06-18: "idiomatic divergence OK if it offers the same feature the idiomatic way"). `send_frames(&[Frame]) → (Vec<FrameResponse>, Option<Error>)` batch (partials preserved, Go-analogue); `extract_signals` confirmed mux-aware. Payload-length-vs-DLC now validated on every data-bearing op. 4 rows → implemented (rust **26/40**). Copilot: 6 fixes (checked `numSignals`, payload validation, doc accuracy). Next: **R3** (Check tier).

**🦀 RUST PARITY R1 (typed DBC document model) ✅ COMPLETE 2026-06-17** (`main` `85bbf22`; #53/#54/#55, 11 rows) — full typed `Dbc` family + `BA_*` attributes, deserialize/serialize vs canonical JSON, parse/format/validate + mux-query/lookup; mirrors Go/Python (hand-mapped serde_json; `message_by_id(CanId)` std/ext). Detail: `memory/project_rust_parity_r1.md`.
...
**⚡ RUNNER-CACHING + DEPENDABOT BATCH ✅ MERGED 2026-06-17** (`main` `ad3ed8c`; all "tooling PRs") — closed the gap where the CI runner got ~nothing from the local incremental build (#37): an ephemeral runner is cold each run, so a re-push cost as much as the first. Five PRs: **#39** split `tools/run_ci.py` 999→616 (off the C0302 1000-line ceiling = DEFERRED_ITEMS **G.4 ✅**), step catalog → new `tools/_ci_steps.py` (`register_all_steps`/`HEAVY_STEPS`/`_run_*`); **#40** build-tree cache (`build/` + `dist-newstyle/`, content-keyed on GHC/Agda/stdlib — cold `build` ~384s → ~23s on a hit; safe because Shake/GHC hash content, defeating `actions/cache`'s mtime reset); **#47** staleness self-test scheduled on build-graph changes (new `--build-staleness {auto,always,never}` + `_ci_steps.should_run_staleness`/`_build_graph_changed`: gate runs only when the diff vs local `main` touches `Shakefile.hs`/`shake.cabal`/`haskell-shim/`/`aletheia.agda-lib`, always on `push:main`, else plain `cabal run shake -- build`) **+ a deterministic `check-fidelity` fix** (ensure the gitignored `haskell-shim/MAlonzo` symlink itself — it was created only inside the `.so` rule's action, which no-ops on a warm build, so a warm-cache fresh checkout failed to resolve MAlonzo; reproduced + fixed, NOT flaky); **#48** `ccache` for C++ (`cpp/CMakeLists.txt` `CMAKE_CXX_COMPILER_LAUNCHER`; `ctest` 615→180s, ubsan 830→217s; no clang-tidy landmine — launcher stays out of `compile_commands.json`); **#49** strengthened the staleness gate (`check_build_incremental.py` now probes **two** structurally-distant runtime modules + a no-op-must-not-relink **incrementality** assertion). Fully-warm `run_ci` on the runner ≈ **13min** (was ~20-23min); code-only PRs additionally skip the ~260s staleness relinks. Then the **Dependabot batch (#41-46)** — upload-artifact v4→v7, mutmut, pylint, pytest-random-order, hypothesis, ruff — merged via `gh` (GitHub-signed web-flow merge commits); the `main` ruleset is **`strict:true`** so the batch is inherently serial (one ~13min CI cycle per merge; `[dev]`-cluster conflicts resolved via `@dependabot rebase`). Detail: `memory/project_ci_speed_optimization.md`. Lesson: `memory/feedback_watch_registration_race.md` (`gh pr checks --watch` false-greens before the check registers — guard the race; caught by GitHub's base-branch policy).
