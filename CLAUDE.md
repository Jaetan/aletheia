...
**Current Status**: ✅ 135 of 181 Agda modules use `--safe`; 1 allowlisted Unsafe module (B.3.d substrate) carries the only postulates project-wide; the remaining 46 `--without-K`-only modules transitively import that substrate (45 under `DBC/TextParser/` + the allowlisted Unsafe substrate itself).

### Module Safety Flag Breakdown

**By flag combination** (181 total — `cabal run shake -- count-modules`):
...
**135 of 181 modules use `--safe`**. No modules require `--sized-types`.
