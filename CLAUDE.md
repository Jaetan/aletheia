...
- **b25b** `b65bae5` (5f / ~101c — Format subtree: AttrDef + Preamble + AttrLine + AttrValue + ValueDescription; 3 mixfix FPs `_<|>_`/`_≢_`/`-[1+_]`; 4 FNs `Σ-syntax` ×3 + `∃-syntax`).
- **b25c** `458f3ba` (4f / 91c — top-heterogeneous: Aggregator/Dispatcher/Attribute/Assign + TextParser/Attributes + Aggregator/Universal + Topology/Signal; 6 mixfix FPs incl. `_≡_` ×2 universal-pattern type signatures).
- **b25d** `2d73f6e` (5f / 57c — long-tail: Topology/Resolve + SignalLine + Comments/Comment + Format/ValueTable + Aggregator/Foundations; 10 mixfix FPs; SignalLine had `_*>_` first-pass-remove-then-restore caught by per-file agda).
- **b25e** `744901a` (5f / 38c — deep long-tail: Def + MetadataRoundtrip + Handlers/ParseDBCText + TopStmt/Default + Assign/BareInt; **27% FP rate** highest of session; **NEW FN class** `feedback_scanner_fn_module_qualified_access.md` — 7 of 10 flags in Assign/BareInt L65-76 `module Trace*` were FPs via `Trace<Name>.pos9` qualified-access body uses; b25c's prior similar removal in Aggregator/Dispatcher/Attribute/Assign was correct because that file routes through `on-AVInt-*` from Bridges instead).
- **b25f** `f042b31` (6f / 41c — top long-tail: Format/SignalGroup + Protocol/JSON/Parse + Main/JSON + Validity/WarningChecks + Refine/ValueDescriptions + Format/SignalLine/Roundtrip; 14 FPs / 25.5% rate; Protocol/JSON/Parse hit 89% file-local FP rate — 8 mixfix-with-args FPs at a heavy-mixfix parser surface; Main/JSON had 3 comment-only dead names — `ParseErr`/`ParseError`/`DispatchError` referenced in doc comments while body code uses `Error` ctors directly; SignalLine/Roundtrip had 6 of 9 names referenced only in commentary blocks documenting the universal `signalLine-roundtrip` decomposition; no new FN class).
- **b25g** `41bad7c` (6f / 38c — top long-tail: Format/Receivers/Roundtrip + Protocol/Handlers + DBC/Validator/Checks + DBC/TextParser + ValueTables/ValueDesc + Aggregator/Dispatcher; 12 FPs + 1 section-FN revert = 13 not-removed / 25.5% rate; **TWO NEW FN classes**: `feedback_scanner_fn_mixfix_section.md` (`_∈?_` used in section `(_∈? names2)` — leading `_` is in standard `(?<![A-Za-z0-9_'])` lookbehind ident class, so both scanner and standard per-name grep miss the body use; caught at per-file `agda --type-check` of Validator/Checks, reverted in same edit cycle) + `feedback_scanner_fn_module_path_segment.md` (ValueTable/SignalGroup in Dispatcher: scanner-LIVE but body uses were only the tail segments of unrelated submodule paths `Properties.ValueTables.ValueTable` / `Properties.SignalGroups.SignalGroup`; module path's dot before the segment isn't in scanner's ident class so the regex's negative lookbehind doesn't reject; +2 FN-found removals merged into the batch); Validator/Checks heaviest mixfix file in batch — 50% file-local FP rate from 4-of-8 mixfix FPs).

**Cumulative R21/R22 A-1.1 closure: 141 files / ~1684 candidates**.  Scanner v2.6 at `~/.local/bin/scan_aletheia_dead_imports.py` (persistent across WSL restarts; not committed to git per WSL-resilience-via-out-of-repo-PATH rule; rebuild from `feedback_unicode_brackets_scanner.md` + b23 commit body's v2.6 multi-line fix if ever lost).  **Scanner FP rate climbs with depth into proof tree, stabilises in long-tail** (b25b 3.3% → b25e 26.9% → b25f 25.5% → b25g 25.5%): mixfix `_≡_` at theorem signatures + every `_X_` operator at heavy-mixfix parser surfaces are highest-recurrence FP forms.  **Known scanner FPs**: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉`/`-[1+_]` outside ident class; (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` body tokens); (3) `public` re-exports (v2.4 line-skip).  **Known FN classes**: heavy `using (...)` clauses, `import M as N` module aliases, multi-line `using\n  (`, `_X_` mixfix-with-args (`∣_∣`, `ifᵀ_then_else_`, `-[1+_]`, `_≈ᵇ_`, `_≡_`, `_<|>_`, `_∷_`, `_+_`, `_≤ᵇ_`, `_⊎_`, `_*>_`, `_>>=_`, `_>>=ₑ_`), **`module X` literal with qualified `X.field` body** (b25e), **`_X_` mixfix sections `(_X y)` / `(x X_)`** (b25g — `_∈?_`), **single-type module-path tail segments** (b25g — `ValueTable`/`SignalGroup`), `renaming (X to Y)` clauses (scanner doesn't process at all).  **Workflow**: per-name `grep -cP` audit after scanner pass + `simplify` skill quality-review agent post-prune (catches residual `renaming` + within-using-clause items); for `_X_` flagged dead also check left-section / right-section / free-combinator forms; for single-CamelCase types from `Types` modules coexisting with submodule imports terminating in same identifier, add `.` to lookbehind to disambiguate module-path segments.

**Gates fresh at `41bad7c`** (R22 A-1.1 batch 25g landing, 2026-05-20): build clean **2m03s** / check-properties clean **8m37s** "All proof modules type-checked successfully" / count-modules **263** (unchanged across b19-b25g).  Per-batch gate timings b25b/c/d/e/f/g: build 3m05s/2m32s/2m36s/2m31s/2m03s/2m03s · check-properties 11m11s/9m02s/13m11s/11m50s/12m06s/8m37s.  Binding tests NOT re-run for b25-series — dead-import pruning is MAlonzo-neutral.  Last green binding posture at `dd8bb81`: pytest 870p+1s (6.27s), Go -race ok 7.343s, C++ ctest 10/10 (22.78s).  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 long-tail OR pivot to other R22 carry-overs.  Top remaining per fresh v2.6 scan post-b25g (640 dead / 183 files): JSON/Parse (8 — 8 stuck mixfix FPs from b25f, untouched), Dispatcher/Attribute/Foundations (8), Format (8), JSONParser (8), Topology/SignalList (7), Topology/Message (7), SignalGroups/SignalGroup (7), Assign/Network (7), TopStmt/Def (7), TopStmt/Assign (7).  Long-tail batches should budget 25-30% scanner FP rate; per-name `grep -cP` audits mandatory for mixfix and `module X` clauses + new `_X_` section + module-path-segment checks (b25g lessons).  **AGDA-D-12.1 follow-ups REQUIRED** unchanged: per-warning enumerant + cross-binding parity + check-runbook + PROTOCOL.md (in-source block at `Adequacy/StreamingWarm.agda:392-406`).  **Pre-close blocker** unchanged: 18 `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` (see in-source `R22-PRE-CLOSE` block).  **Push gap**: local `main` 152 commits ahead of `origin/main` — R20+R21 merges held local per `feedback_review_branch_workflow.md`.
