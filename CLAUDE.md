...
193 total modules (`cabal run shake -- count-modules`):
- **188**: `--safe --without-K`
...
**Status (2026-04-28):** Layers 1–2 ✅; Layer 3 through 3d.3 ✅; **3d.4 + JSON-mirror + Path A ✅ shipped 2026-04-28 (commit `320c5a9` on branch `b3d-3d4-json-detaint`)**; **3d.5.a framework core + 3d.5.b parseValueTable gate + 3d.5.c-γ.1+γ.2+γ.3+ε+ζ+η + 3d.5.d parseValueTable production migration ✅ shipped on branch `b3d-3d5-format-dsl`**.
...
3d.4 + JSON-mirror + Path A details: single mega-commit (98 files, +1545/-1314).  Module-flag invariants post-η: **193 modules total** (was 191; +2 new for Format/SignalLine + Format/SignalLine/Roundtrip, all `--safe --without-K`); 188 `--safe --without-K` + 4 `--safe --no-main` + 1 `--without-K`-only (Substrate.Unsafe).  Stream LTL +12-38% across bindings (Path A.5 Bool fast path); Signal Extraction -2-9% / Frame Building -1-7% (Path A structural cost; γ.2 incremental cost folds into the Path A profile, no fresh regression beyond what 320c5a9 already accepted; ε/ζ/η/3d.5.d are proof-only and runtime-neutral on the streaming hot path — both `parseSignalLine` and `parseValueTable` run at DBC config time only).  Baselines NOT refreshed per user "wait and see" 2026-04-28; COMPILE-pragma escape hatch deferred (now requires explicit user approval per `feedback_no_suppression_without_approval`).  Per-commit detail in PROJECT_STATUS.md and `memory/project_b3d_universal_proof.md`.
