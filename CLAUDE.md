...
**Status (2026-04-27):** Layers 1–2 ✅; Layer 3 through 3d.3 ✅ (commits 3a/3b/3c-precursor/3c.0/3c.1/3c.2/3c.3/3c.4/3d.1/3d.2/3d.3a/3d.3b — per-commit detail in PROJECT_STATUS.md and `memory/project_b3d_universal_proof.md`).  **3d.4 + JSON-mirror IN PROGRESS uncommitted WIP on branch `b3d-3d4-json-detaint` 2026-04-27.**  Architectural pivot 2026-04-27: original 3d.4 plan had a load-bearing gap — post-3d.4 with `Identifier.name : List Char`, JSON-side roundtrip needs `toList∘fromList` axiom per Identifier site (the original plan analyzed text-parser-side only).  User picked Option 2 (mirror 3d.4 fix on JSON side); three steps land together as one mega-commit (entanglement: any subset breaks roundtrip lemmas).  Step 1 spine ✅ (JSON.{Types,Parse,Format,Lookup}.agda — `JString : List Char → JSON`); Step 2 spine ✅ (DBC AST text fields → List Char); Step 3 source-side ~50% from prior session.  ~150 cascade sites + proof reshape pending.  Cannot land partially per pre-commit scope gate.  See `.session-state.md` for the WIP map + per-file checklist and `memory/feedback_audit_all_wire_boundaries.md` for the lesson earned this session.
...
1. **3d.4 + JSON-mirror (~3-4w)**: combined mega-commit: (a) `JString : List Char → JSON` spine; (b) DBC AST String fields → List Char (`unit`, `text`, `version`, `AVString`, `AttrDef/Default/Assign.name`, `ATEnum`, `ValueTable.entries`); (c) `Identifier.name : String → List Char` Lexer drops `fromList` + `toList∘fromList` axiom dependency; 47 modules under `DBC/TextParser/` lift from `--without-K` to `--safe --without-K`; `Substrate.Unsafe.agda` retained only for the outer `parseText/formatText` wrap.  JSON-side per-construct roundtrips axiom-free; zero JSON-roundtrip modules dropped to `--without-K`-only.
