...
**🏷️ v5.0.0 RELEASED 2026-07-24 — first tag-triggered release (`v5.0.0` tag → keyless-signed multi-binding bundle + `.deb`/`.rpm` + GHCR image; published + consumer-verified ×4 artifacts against the `@refs/tags/v5.0.0` identity).** MAJOR — breaking binary wire (H.2 detailed extraction reasons #240), JSON wire (I.1 Int64 bound #229 + geometry refuse-at-entry #222), Rust `#[non_exhaustive]` #211. Bundled the post-v4.0.0 arcs (detail in each memory file + git): C.2 Dec₀ self-certifying hot-path Bools #243 [[project_c2_dec0]] · H.2 binary detailed reasons #240 [[project_h2_detailed_reasons]] [[feedback_wire_capability_over_yagni]] · RTS/exactness I.1/streaming/K.1 #229/#232/#233 [[project_k1_rts_ssot]] [[feedback_value_linear_stdlib_cost]] · geometry refuse-at-entry #222 · E.3 tightness #218-#220 [[project_e3_tightness]] · distribution hardening #213-#216 + wire-codes SSOT #211. The tag path was distribution-hardening's remaining acceptance test — it CAUGHT + fixed two release-path defects before go-live: (1) `.deb`/`.rpm` Python install must use `PYTHONPATH`, not `pip install /opt/...` (read-only source → setuptools `egg_info` denied) #246; (2) a pre-existing GHCR package needs the repo granted **Write** under *Manage Actions access* (RELEASE.md had documented only the visibility flip). The dispatch dry-run (`gh workflow run release.yml`) exercises the whole pipeline minus the two publish steps — it caught (1); the real tag caught (2). Detail: [[project_v5_release]] + CHANGELOG [5.0.0] + git tag v5.0.0.

**📦 Distribution hardening ✅ COMPLETE 2026-07-19 (#213-#216; Phase 6 item 1 closed).** Dist-staging gate · SBOM coverage gate · bundle validation runs the installers' recipes · nfpm `.deb`/`.rpm` + keyless-signed GHCR image. Tag-triggered acceptance test PASSED via v5.0.0 (caught two release-path fixes; [[project_v5_release]]). Detail: PROJECT_STATUS § Phase 6 + git #213-#216.
