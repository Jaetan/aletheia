...
- **b25-series** (b25b → b25i, 8 batches 2026-05-19 → 2026-05-20; commits `b65bae5` / `458f3ba` / `2d73f6e` / `744901a` / `f042b31` / `41bad7c` / `f2ed4f1` / `6b50d50`): 41 files / ~395 dead removed across Format subtree → Aggregator/Dispatcher/Attribute siblings → deep long-tail → top long-tail tier → crash-recovery + Aggregator/Dispatcher/Attribute padding.  FP rate trajectory: b25b 3.3% → b25e 26.9% → b25f/g 25.5% → b25h 34.0% (heavy-DSL-mixfix peak) → b25i 20.0%.  **FOUR new FN-class lessons captured this series**: b25e `feedback_scanner_fn_module_qualified_access.md` (`module X` body via `X.field`), b25g `feedback_scanner_fn_mixfix_section.md` (`_X_` sections `(_X y)` / `(x X_)`), b25g `feedback_scanner_fn_module_path_segment.md` (single-CamelCase tail segments of `M.X` paths), b25h `feedback_scanner_fn_mixfix_free_combinator.md` (`_X_` as higher-order argument).  b25i side-observation: 4 of top-5 remaining Assign/* siblings (String, Frac, Bridges, BareInt) are all-`module Trace*` FPs — deprioritised for next batch.  Per-batch narrative + commit hashes + per-file FP/FN inventory in `memory/project_review_round21.md`.

**Cumulative R21/R22 A-1.1 closure: 152 files / ~1759 candidates**.  Scanner v2.6 at `~/.local/bin/scan_aletheia_dead_imports.py` (persistent across WSL restarts; not committed to git per WSL-resilience-via-out-of-repo-PATH rule; rebuild from `feedback_unicode_brackets_scanner.md` + b23 commit body's v2.6 multi-line fix if ever lost).  **Scanner FP rate climbs with depth into proof tree, stabilises in long-tail then ticks up on heavy-DSL-mixfix surfaces** (b25b 3.3% → b25e 26.9% → b25f 25.5% → b25g 25.5% → b25h 34.0% → b25i 20.0%): mixfix `_≡_` at theorem signatures + every `_X_` operator at heavy-mixfix parser surfaces are highest-recurrence FP forms.  **Known scanner FPs**: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉`/`-[1+_]` outside ident class; (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` body tokens); (3) `public` re-exports (v2.4 line-skip).  **Known FN classes**: heavy `using (...)` clauses, `import M as N` module aliases, multi-line `using\n  (`, `_X_` mixfix-with-args (`∣_∣`, `ifᵀ_then_else_`, `-[1+_]`, `_≈ᵇ_`, `_≡_`, `_<|>_`, `_∷_`, `_+_`, `_≤ᵇ_`, `_⊎_`, `_*>_`, `_>>=_`, `_>>=ₑ_`), **`module X` literal with qualified `X.field` body** (b25e), **`_X_` mixfix sections `(_X y)` / `(x X_)`** (b25g — `_∈?_`), **single-type module-path tail segments** (b25g — `ValueTable`/`SignalGroup`), **`_X_` as free combinator at higher-order argument site** (b25h — `_≢_` in `AllPairs _≢_ args`), `renaming (X to Y)` clauses (scanner doesn't process at all).  **Workflow**: per-name `grep -cP` audit after scanner pass + `simplify` skill quality-review agent post-prune (catches residual `renaming` + within-using-clause items); for `_X_` flagged dead, run FOUR checks: infix `a X b`, left section `(_X y)`, right section `(x X_)`, AND free combinator `_X_` literal (b25h); for single-CamelCase types from `Types` modules coexisting with submodule imports terminating in same identifier, add `.` to lookbehind to disambiguate module-path segments.

**Gates fresh at `6b50d50`** (R22 A-1.1 batch 25i landing, 2026-05-20): build clean **1m59s** / check-properties clean **9m19s** "All proof modules type-checked successfully!" / count-modules **263** (unchanged across b19-b25i).  Per-batch gate timings b25b/c/d/e/f/g/h/i: build 3m05s/2m32s/2m36s/2m31s/2m03s/2m03s/3m34s/1m59s · check-properties 11m11s/9m02s/13m11s/11m50s/12m06s/8m37s/12m01s/9m19s.  Binding tests NOT re-run for b25-series — dead-import pruning is MAlonzo-neutral.  Last green binding posture at `dd8bb81`: pytest 870p+1s (6.27s), Go -race ok 7.343s, C++ ctest 10/10 (22.78s).  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 long-tail OR pivot to other R22 carry-overs.  Top remaining per fresh v2.6 scan post-b25i (583 dead / 183 files): JSON/Parse (8 — 8 stuck mixfix FPs from b25f, untouched), Assign/String + Assign/Frac + Assign/Bridges (7 each — **all-`module Trace*` FPs confirmed via grep this session; skip**), Assign/BareInt (7 — likely same-pattern; re-audit `TraceXXX\.` before pursuing), DecRatParse (7), CAN/SignalExtraction (7), CAN/Encoding/{Roundtrip,Disjoint,Arithmetic/Rational} (7 each), CAN/Batch/Properties/Capstone (7).  Strong candidates for b25j: DecRatParse + the CAN tree.  Long-tail batches should budget 25-34% scanner FP rate; per-name `grep -cP` audits mandatory for mixfix and `module X` clauses + new `_X_` section + module-path-segment + free-combinator checks (b25g + b25h lessons).  **AGDA-D-12.1 follow-ups REQUIRED** unchanged: per-warning enumerant + cross-binding parity + check-runbook + PROTOCOL.md (in-source block at `Adequacy/StreamingWarm.agda:392-406`).  **Pre-close blocker** unchanged: 18 `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` (see in-source `R22-PRE-CLOSE` block).  **Push gap**: local `main` 152 commits ahead of `origin/main` — R20+R21 merges held local per `feedback_review_branch_workflow.md`.
