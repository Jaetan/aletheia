...
**Status (2026-05-01):** Layers 1–2 ✅; Layer 3 ✅ (all per-line constructs migrated to Format DSL); **Layer 4a ✅ (`70766cd`, SignalGroup DSL migration + 4 char-class disjointness lemmas) + Layer 4b ✅ (`7b17da6`, polymorphic `many-η-roundtrip` helper + 5 list-level lifts: SignalGroups / ValueTables / EnvVars / Comments / Messages)**.  **Layer 4c WIP** — foundations + body bridge + `many-η-roundtrip-with-lift` extension (3 modules, ~490 strict-LOC) all type-check, **uncommitted in working tree** until universal aggregator closes (per `feedback_no_unilateral_deferral.md`).  Active branch: `b3d-3d5-format-dsl`.  Path A.4 (3d.4 + JSON-mirror, commit `320c5a9` on `b3d-3d4-json-detaint`) lifted the prior 47-module `--without-K`-only cluster to `--safe --without-K` and shipped Path A.5 Bool fast path on `Cache.lookupEntries` / `DBCHelpers.findSignalInList`.
...
- 4c WIP — three modules type-check (uncommitted): `Properties/Aggregator/Foundations.agda` (~150L: `rawOf` + `WFAttribute` + `TopStmtTyped` + `liftTopStmt` + `emitTopStmt-chars` + `toTopStmtsTyped`), `Properties/ManyRoundtrip.agda` extension (`many-η-roundtrip-with-lift`, ~110L added — strict generalization with lift `L : I → O` so parser may yield `O` while emitter consumes `I`), `Properties/Aggregator/BodyBridge.agda` (~230L: body bridge via 5 foldr-distributivity steps + 6 per-section emit-map bridges).  Architecture **option α** (typed shadow + lift): TopStmtTyped mirrors TopStmt's 6 payload constructors; lift routes attributes through `rawOf defs`.  Universal target REFINED to `∀ d → WellFormedDBC d → parseText (formatText d) ≡ inj₂ d` (option a: WF precondition; matches 4b's MessageWF/SignalGroupWF precedent).  Remaining: dispatcher (5 simple sections + 1 attribute heavy via existing 31 `parseAttrLine-roundtrip-*` slims with typed→raw byte bridges) + apply many-η-with-lift + partitionTopStmts categorical preservation + refineAttributes inverse + universal aggregator.
