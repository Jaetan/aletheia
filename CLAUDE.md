...
**✅ E.2 ROUTE (b) — always-strict `format_dbc_text` ✅ MERGED (#181/#183/#187 + review #188, UPD #189).** Returns `.dbc` text that provably re-parses to the input DBC, or a typed `handler_text_roundtrip_failed` refusal — never silently-lossy; machine-checked (`formatDBCTextResult-sound`), across all four bindings. Two heavier routes stay deferred (lossless emission of lossy constructs; a stronger validator). Detail: [[project_e2_reexamination]] + docs/development/E2_ROUTE_B.md + git.

**🏷️ 3.0.0 RELEASED 2026-07-09 (#171, tag `v3.0.0`, squash `74db395a`)** — first major since v2.0.0 (GPG-signed tag; signed dist cosign+Rekor; breaking across all 4 bindings). Earned a build-integrity fix: the `.so` was located by `find … | head -1`, non-deterministic across cabal's per-version `dist-newstyle` trees → a version bump could ship the STALE prior `.so`. Fixed with version-scoped fail-loud `.so` select + dist stale-tree prune ([[feedback_release_stale_dist_tree]]). Detail: PROJECT_STATUS + git #171.
