...
**🧾 G.x CI-PROGRAM CLOSE-OUT ✅ MERGED 2026-06-17** (`main` `615483c`, #51) — the two trailing CI-speed items. **G.2**: `check_changelog.py` now fails on build/CI/tooling changes (`Shakefile.hs`/`shake.cabal`/`aletheia.agda-lib`/`haskell-shim/`/`tools/`/`.github/workflows/`) too, not just the public-API surface (user-scoped: Build+CI+tooling, enforce-all); Agda `src/` excluded (reaches users via the bindings), `.md`-anywhere + `/tests?/` exempt so a README/fixture can't sour it; pure `watched_files()` seam + first-ever tests (`python/tests/test_check_changelog.py`, 28: matcher partition incl. the `.md`-under-watched-dir discriminator + hermetic temp-git-repo e2e both polarities); distinct AGENTS.md Universal Rule (off UR-1, which alone carries migration-guidance); failure msg names the "internal — no behavior change" escape. **Dogfooded** (the PR's own `tools/` edit required its CHANGELOG line). Consequence: dependabot `github-actions` bumps + `gen-ffi-modules` cabal regens now need a CHANGELOG line/escape note (merging human adds it). **G.3**: BUILDING.md honest-graph + `gen-ffi-modules`/`iwyu` + staleness-gate doc pass + corrected timings; CI_LOCAL.md `--build-staleness` section + de-numbered stale step counts; CLAUDE.md cold-build "~60s"→"~2m" (measured 2m16s — the cross-doc contradiction settled by measuring). DEFERRED_ITEMS **G.2 ✅ / G.3 ✅**. Copilot (4, residual public-API framing) fixed+resolved. Detail: `memory/project_ci_speed_optimization.md`.
