...
**R22 A-1.1 batches 7-18 (2026-05-18)** — batches 7-9 landed on `review-r21` pre-merge; batches 10-18 on `review-r22` post-merge. Per-batch index:
...
**Cumulative R21/R22 A-1.1 closure: 80 files, ~686 of 864 candidates (~79%)**. Scanner v2.4 at `/tmp/scan_dead_imports.py` (backup `.v24.bak`) — patches v2.3 to skip `open import ... public` lines (Lexer FP class — full build is the only catch). Known blindspots: (1) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` ↔ token-bounded `if` in body) — ~2-3% per batch caught by per-file agda type-check; (2) `public` re-exports — addressed in v2.4 by line-skip.

**Gates fresh at `4e02d27`** (R22 A-1.1 batch 18 landing, 2026-05-18): build clean **3m44s** / check-properties clean **12m16s** All proof modules type-checked successfully / count-modules **263** (unchanged).  Recent batch gate timings: b18 3m44s+12m16s, b17 2m03s+10m01s, b16 3m38s+10m53s, b15 3m56s+12m30s, b14 3m49s+12m34s, b13 2m03s+10m28s.  Build-time variance ~2-4m driven by which subtree was edited (Identifier.agda and DBC.Formatter trigger more downstream cascade than leaf Format/* modules).  Other invariant gates (check-invariants / check-no-properties-in-runtime / check-erasure / check-fidelity / check-ffi-exports / check-bound-enforcement / check-runbook / check-changelog / check-limits-parity) NOT re-run for batches 10-18 — they only flip on structural changes, none here.  Binding tests (pytest / Go / C++) NOT re-run — dead-import pruning is MAlonzo-neutral (touched names weren't in runtime path; runtime-path edits like Handlers.agda b10, Identifier.agda b14, JSON/Parse.agda b16 affect build-time elaboration only).  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 — ~178 candidates / ~63 files remaining (scanner v2.4 at `/tmp/scan_dead_imports.py` + backup at `/tmp/scan_dead_imports.py.v24.bak`; rebuild after WSL restart — patterns documented in `.session-state.md`).  Current efficiency: 3-candidate tier (4-cand exhausted at b17), each batch nets ~12-15 candidates / ~25-30 min including check-properties.  Optional AGDA-D-12.1 follow-ups (per-warning `endstream.uncached_atom` LogEvent + check-runbook entry + PROTOCOL.md write-up; documented in `StreamingWarm.agda:394-409` DEFER block — none blocking).  **Pre-close blocker** (added 2026-05-18, b15 side-observation + `74a72fa` in-source marker): 18 pre-existing `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` — stale `using (...)` re-exports of submodule names (`parseRawAttrAssign-after-keyword-Network` etc.).  Must be resolved before R22 → main merge — see in-source `R22-PRE-CLOSE` block for disposition options + `.session-state.md` Pre-close checklist.  **Push gap**: local `main` is 152 commits ahead of `origin/main` — R20 + R21 merges not yet pushed; user-gated per `feedback_review_branch_workflow.md`.
