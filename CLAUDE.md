...
**🔬 r25 ADVERSARIAL 4-BINDING API REVIEW — Point 3 COMPLETE; remaining B5b/B6/B7/B8** (spec `.archive/reviews/r25/FINAL_report.md` §3/§4; cadence = focused PRs, each SHA-verified green). **Shipped #80-111**: B1-B4 + a Dependabot batch + gate-fixes + the user's 3-point sequence (Point 1 cargo-doc gate #103; Point 2 renderer-vocal ×4 #104-107; **Point 3 render-delegation ×4** — Py #109 / Go #110, C++ already all-FFI, Rust done) + **throughput Lever A** (#111). **Point 3**: every binding renders Check descriptions + extraction values as exact rationals via the kernel `formatℚ` — Go `SignalValue.Value` float64→Rational (shared build/update INPUT + extraction OUTPUT), lazy `ConditionDesc()`/`condition_desc` (`(string,error)` in Go), binary+JSON decoders carry exact num/den + reject den≤0, CLI `extract --json` emits bare-int-or-`{num,den}`. **Lever A** (#111, `ci(mutation):`): the `mutation testing` lane is diff-scoped per binding — a single-binding PR runs only that engine (go-only ≈ 24→16min, since gremlins is the slow ~12.5min engine; py/cpp-only win more); fails SAFE to the full run (git-absent / no-`main`-ref / global-path / empty-diff / `ALETHEIA_MUTATION_NO_DIFF_SCOPE=1`); whole-binding-dir scope (source+tests+config); coverage-neutral; verified in prod on #110 (`diff-scope → [go]`). **Throughput levers**: A done; **B (pipeline PRs) = STANDING way of working** (overlap the next PR's implementation with the current PR's GitHub-CI wait — `memory/feedback_pipelining_prs.md`); C (fast pre-push) DROPPED — the warm local pre-push is ~110s (a non-issue); the real ~16-24min cost is the GitHub PR-CI we babysit. **Attribution policy REVERSED 2026-06-27**: commits get `Co-Authored-By: Claude Opus 4.8`, PR bodies get the Claude Code footer (`memory/feedback_commit_workflow_signed.md`). Detail + lessons: `memory/project_r25_binding_review.md` + `memory/project_mutation_to_zero.md` + CHANGELOG + git #80-111.
