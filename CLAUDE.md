...
**📦 DISTRIBUTION HARDENING ✅ COMPLETE 2026-07-19 (#213 `10f3f547` · #214 `d92fe6b1` · #215 `6b1770eb` · #216 `37843e5a`; PROJECT_STATUS Phase 6 item 1 closed).** Each slice shipped with empirically proven teeth: **always-on staging gate** `check-dist-staging` (Shakefile stageBinding pathspecs must resolve incl. `:(exclude)` inner globs — git archive passes dead excludes SILENTLY; packaging scripts tracked + `bash -n`/`fish -n`, fish installed in CI, FAST tier) · **SBOM bills the bundled binding sources** (manifest parsers over the STAGED tree, zero external SBOM tooling; `check_sbom_coverage` fails dist on any missing dependency + always-on `--parse-only` rot tripwire; go.sum `h1:` dirhash rides a property, never a SHA-256 hashes entry) · **real-workflow bundle validation** (`bundle_validate` executes the installers' own captured recipes; one consumer per compiled binding through a real LTL scenario; gates publish; first `schedule:` workflow re-validates weekly — HEAD dist + published-Release replay; `--self-test` corruption teeth, all-skipped = COULD-NOT-CHECK) · **native packages + GHCR image** (nfpm one-manifest `.deb`/`.rpm`, payload = the bundle at `/opt/aletheia` byte-identity-proven; rpm deps on the libgmp SONAME (resolves across rpm distros); self-verify loop over EVERY signed artifact; Dockerfile bundle/worktree skew fixed + BuildKit verify stages — image cannot build unless every binding builds against its own bytes; digest keyless-signed; user's one-time GHCR visibility flip documented). First tag-triggered release = the full-path acceptance test. Acceptance run of `bundle-validation.yml` green in full (dispatch 2026-07-19). Detail: PROJECT_STATUS § Phase 6 + CHANGELOG + git #213-#216.

**🔒 WIRE-CODE SSOT + COMPLETENESS GATES ✅ MERGED 2026-07-19 (#211 `862d3840`; the wire-code-parity and Rust-enum-growth backlog items closed, both since removed from DEFERRED_ITEMS).** `docs/WIRE_CODES.yaml` pins the kernel's two wire vocabularies (validation issue codes + error codes); `tools/check_wire_codes.py` (run_ci step `check-wire-codes`) anchors YAML↔kernel formatter arms (reciprocal set equality + declaration order, fail-closed on vacuous input); per-binding completeness tests anchor every binding's vocabulary to the YAML (Rust: named-variant decode + lossless `Error::Core` passthrough, typed lifts SSOT-checked); `test_error_code_sync.py` retired. The enum-growth half: Rust `IssueCode`/`Error` now `#[non_exhaustive]` (BREAKING Rust-only; `Unknown(String)` stays as the version-skew channel; `as_str()` skew idiom documented; `IssueSeverity`/`Verdict`/LTL AST stay exhaustive by design). **USER RULE 2026-07-19**: prose about code carries no hard-coded counts and no line-number references — describe structure qualitatively or name the SSOT; measurements (benchmarks, run evidence), spec constants, versions/dates, and machine-re-derived numbers keep their digits. `memory/feedback_self_referential_count_drift.md` (rewritten) + AGENTS/docs.md cat 14. Detail: CHANGELOG + git #211.
...
**✅ E.2 CLOSED 2026-07-18 (route (b) shipped #181/#183/#187; item removed).** `format_dbc_text` returns `.dbc` text that provably re-parses to the input DBC, or a typed `handler_text_roundtrip_failed` refusal — machine-checked (`formatDBCTextResult-sound`), ×4 bindings. Universal `validity ⇒ WellFormedTextDBCAgg` is structurally impossible (warning-class walls by design) — the terminal form is reached; the why lives in the `Properties.WellFormedFromValidity` module header. Strategy docs deleted; the design plan lives on as `EXTENDED_MUX_DESIGN.md` (A.1/A.3). [[project_e2_reexamination]].
