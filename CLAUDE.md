...
**Latest (2026-04-23):** Phase A (feature matrix + structural gates) + Phase B.1 (DBC metadata Tier 1) + Phase B.1.x (DBC metadata Tier 2 + signal receivers + message senders) + Phase B.2 (mux query helpers — closed via audit) + **Phase B.3.a → B.3.c (DBC text parser corpus + Agda skeletons + incremental construct implementation)** landed. **B.3.d pre-gate Commit 1/6** (DecRat primitives) shipped as `0b7849b`. **B.3.d pre-gate Commit 2/6** (DecRat parser + emitter + universal roundtrip) shipped as `c05083e`. **B.3.d pre-gate Commit 3/6** (EV_ EnvironmentVar `ℚ → DecRat` migration) — `RationalRoundtrip.agda` Layers 1–4 all closed; proves the universal `fromℚ?-after-toℚ : ∀ d → fromℚ? (toℚ d) ≡ just d` via `stripFactor-peel` + `canonicalizeDecRat-id` + `↥/↧ₙ-toℚ-canonical` + the `peel-2`/`peel-5`/`fromℚ?-raw-canonical-shape` composition. EV_ migration mechanics landed (Types/JSONParser/Formatter/TextParser-EnvVars/TextFormatter-EnvVars/MetadataRoundtrip); `check-properties` Shake phony extended to walk RationalRoundtrip; `NonTerminatingRational` parse error added cross-binding; pylint config consolidated (pyproject.toml authoritative, pylint ≥4.0 pin, `.pylintrc` removed); `ErrorCode` enum split into `aletheia/error_codes.py`. Work follows the [cross-binding parity roadmap](docs/development/PARITY_PLAN.md) locked after R17.
...
- **Pre-B.3.d infrastructure** (`0035a4e`) — `check-properties` Shake phony extended to walk `Aletheia/DBC/TextParser/Properties.agda`; three WF proof-completeness fixes in `Formatter/SignalRoundtrip.agda` (`List⁺.toList vs` unification), `JSONParser/MessageWF.agda` (missing `senders` inversion step), `JSONParser/Properties.agda` (missing `nodes`/`comments`/`attributes` bind-step inversions).
