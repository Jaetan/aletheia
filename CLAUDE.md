...
**⚡ INCREMENTAL + STALENESS-SAFE BUILD ✅ MERGED 2026-06-15** (PR #37 squash → `main` `e80a101`; a "tooling PR") — fixed the dependency graph the `rm -rf`/always-dirty-phony/`touch` sledgehammer was masking, so `libaletheia-ffi.so` builds incrementally: no-op build **~0.1s** (was a full ~280-module recompile EVERY invocation), one-module edit **~12s**. Two graph bugs: (1) the Shake `.so` rule now depends on the `.agda` SOURCES (content-hash) + shim + `.cabal`, with `Main.hs` an ordering edge only (dropped `need malonzoFiles`/the `create-symlink` phony/`touch`/`rm -rf`); (2) `haskell-shim/aletheia.cabal` `other-modules` now LISTS all 433 MAlonzo modules — cabal's up-to-date check was blind to them (`-Wmissing-home-modules` suppressed) → it skipped GHC → stale `.so`. Regenerate the list with `cabal run shake -- gen-ffi-modules`; `-Werror=missing-home-modules` is the drift gate (verified it fires + names the module). New tooling: staleness gate `tools/check_build_incremental.py` (run_ci `build` prereq; verified it exits 1 on a stale build), an `AgdaVersion` Shake oracle + `need aletheia.agda-lib` (toolchain tracking), `cabal run shake -- iwyu` (regenerate relevant `.agdai` via Agda's own interface cache + run IWYU, no full rebuild), and a content-aware `install_hooks.py` whose pre-push hook now runs `run_ci --parallel`. CoPilot: 3 comments, all valid, fixed+replied. Detail (incl. why `produces` was wrong + the symlink red herring): `memory/project_build_so_idempotency.md` (now RESOLVED). Backlog: DEFERRED_ITEMS **G.2** (broaden check-changelog), **G.3** (BUILDING.md doc pass), **G.4** (run_ci.py at 999/1000 lines). Lesson: `memory/feedback_no_git_checkout_in_revert_traps.md`.
