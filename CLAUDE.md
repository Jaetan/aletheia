...
**Current Status**: ✅ 135 of 171 Agda modules use `--safe`; 1 allowlisted Unsafe module (B.3.d substrate) carries the only postulates project-wide; the remaining 36 `--without-K`-only modules transitively import that substrate (35 under `DBC/TextParser/` + the allowlisted Unsafe substrate itself).

### Module Safety Flag Breakdown

**By flag combination** (171 total — `cabal run shake -- count-modules`):
...
- **36 modules**: `--without-K` only (no `--safe`) — all under `Aletheia/DBC/TextParser/`. Exactly **1** is the allowlisted Unsafe module: `Aletheia/DBC/TextParser/Properties/Substrate/Unsafe.agda`, hosting the two `String ↔ List Char` bridging axioms (`toList∘fromList`, `fromList∘toList`) needed to compose the B.3.d universal roundtrip theorem (mirrors stdlib's `Data.String.Unsafe`, proven by `trustMe` under `--with-K`; structurally unprovable in `--safe --without-K` because the Agda String primitives reduce only on closed terms). The other **35** transitively import that substrate via `DBC/TextParser/Lexer.agda` (which uses the bridging axioms to lift `parseIdentifier` from `List Char` to `String`); Agda's `--safe` propagation forces them to the same flag set.  Allowlisted by name in `Shakefile.hs` `check-invariants`; any other Unsafe module or `^postulate` line is rejected.

**135 of 171 modules use `--safe`**. No modules require `--sized-types`.
...
- **Layer 3 Commit 3c.1 ✅** (this session, 2026-04-26, `12175ac`): `parseAttrDef-roundtrip` (5 standard scopes via `WfAttrDef-NotRel` dispatch) + `parseAttrDefRel-roundtrip` (2 rel scopes via `WfAttrDef-Rel` dispatch).  Three new modules totaling ~2,647 LOC: `Properties/Attributes/Type.agda` (~1,176L per-tag `parseAttrTypeDecl-roundtrip-AT*` for STRING/INT/FLOAT/ENUM/HEX with cross-keyword failure helpers + parseEnumLabels cons-case induction), `Properties/Attributes/Def.agda` (~1,428L; 9-step `parseAttrDef-after-keyword` + 10-step `parseAttrDefRel-after-keyword` carrying `isRelScope scope ≡ true` discriminator + 7 case dispatch + per-case 4-step `++ₗ-assoc` input reshaping + 4-step `advancePositions-++` position folds + `emitAttrDef-NotRel-shape` / `emitAttrDef-Rel-shape` with-clause unfolders + inline `parseRelScope-AS*-eq` lemmas via `alt-left-just`/`alt-right-nothing` + `string-*>-success`), `Properties/Attributes.agda` (43L facade re-exporting Common — previously orphaned, now reachable per `feedback_check_properties_aggregator_walks.md` — plus Def).  Auxiliary: `DecRatParse/Properties.agda` lifts `headOr` from `private` to public.  Module count 168 → 171 (+3).  **3c.2+ remaining: parseRawAttrDefault, parseRawAttrAssign+parseRawAttrRel, parseAttrLine 5-way composer.**  Commits 3d (messages: BO_ + inner SG_ many) pending.
...
**Module count:** 171 Agda modules — see Module Safety Flag Breakdown above.  Net +5 vs 166 post-3b: `Aletheia/DBC/DecRat/Refinement.agda` (`--safe --without-K`) + `Properties/Attributes/Common.agda` + `Properties/Attributes/Type.agda` + `Properties/Attributes/Def.agda` + `Properties/Attributes.agda` (last 4 all `--without-K` only — transitively import `Substrate/Unsafe.agda` via `Lexer.agda`).
