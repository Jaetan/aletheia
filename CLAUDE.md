...
**🎯 FLOAT-PRINCIPLE SWEEP ✅ MERGED 2026-07-01 (#130-136, ultracode)** — closed THE FLOAT PRINCIPLE for every rational/decimal value (**the principle: a value is an exact rational/DecRat everywhere — no float internal, none at UI-input — degrading to floating-point ONLY at print-out to the user**; the lone remaining UI-input float, timestamp input, is deferred to Phase 6 — see **Open** below). **#130** Python rejects floats at the rational wire fields (field-aware `reject_inexact` / `reject_formula_inexact`); **#131** decimal-test dedup + demo-gate hardening (surfaced+fixed 2 real demo bugs: `warnings`-vs-`issues` field, stale multiplex shape); **#132** Go JSON-decoder dead native-type arms removed (sole entry `parseResponse` uses `UseNumber` → only `json.Number`); **#133** dead float member removed from C++/Rust `LogValue` (NO binding logs a float; both now the symmetric set `{str,i64,u64,bool}`); **#134/#135/#136 = the render-rationals-exactly sweep** (r25 direction-call #2, all 4 bindings): #134 C++ `extract --json` lossy `double`→exact `{num,den}`; #135 CLI human-text (Go/C++/Python) `%g`/`to_double`→kernel `format_rational`; #136 Go/Rust validation-error messages→`format_rational` (+ exact `num/den` fallback; **reordered `format_rational` to check RTS before `library()` dlopen** — a validation error now fails fast with no I/O when RTS down). A whole-tree grep confirms **NO lossy user-facing rational render remains**. Dead-code sweep (task #85): the 6 Go `deadcode`-flagged exports are library-API keepers (`deadcode`'s reachability lens is wrong for a library); C++/Rust dead-float log variants removed. Every PR: an adversarial-verify workflow (caught byte-identical-vs-parser-equivalent, a Go/C++ test-coverage gap, the #136 dlopen regression) + all Copilot comments validated/fixed/resolved. **Open**: #86 (Go decoder reject-branch coverage tests — pre-existing gap), the rational-log-values "design rich later" proposal ([[project_log_rational_values]]), timestamp-float-input → Phase 6. Detail: `memory/project_r25_binding_review.md` + `memory/project_log_rational_values.md` + `memory/project_b6e_decrat_ffi.md` + CHANGELOG + git #130-136.
