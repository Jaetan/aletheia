...
- **Stage 2 (`.agdai` reader) stays opt-in** — a source-verified DESIGN, not a built tool; recompile-confirm is already SOUND, so Stage 2 buys only speed + provider-redundancy/full-IWYU (recompile fundamentally can't flag a used-but-wildcard-redundant import) at the cost of linking Agda-as-library (version-locked `.agdai` blob).  Detail: `memory/project_agda_iwyu.md` (2026-06-04) + `memory/feedback_test_interface_via_di.md` (callable-DI lesson).
