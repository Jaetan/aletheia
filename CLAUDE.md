...
231 total modules (`cabal run shake -- count-modules`):
- **226**: `--safe --without-K`
...
**Status (2026-05-02):** Layers 1–2 ✅; Layer 3 ✅ (all per-line constructs migrated to Format DSL); **Layer 4a ✅ (`70766cd`, SignalGroup DSL migration + 4 char-class disjointness lemmas) + Layer 4b ✅ (`7b17da6`, polymorphic `many-η-roundtrip` helper + 5 list-level lifts: SignalGroups / ValueTables / EnvVars / Comments / Messages) + Layer 4c-(a) checkpoint ✅ (`cf091d8`, parseTopStmt → 7-bucket head-character pattern dispatch + 5 simple section dispatchers + generic Attribute helper; Foundations + BodyBridge + many-η-roundtrip-with-lift extension committed alongside) + Layer 4c-(b) ✅ (`6c7ade3`, TAT universal attribute roundtrip via β prefix lemma — `parseAttrLine-on-emit-Attribute` and `parseTopStmt-on-emit-typed-TAT` both universal over `DBCAttribute` under `WFAttribute defs a` precondition; 7-case TopStmt scope dispatch 25 min OOM → 1-case via β 13.8s, ≥120× speedup)**.  **Layer 4c remaining**: apply many-η-roundtrip-with-lift (one-liner) + partitionTopStmts categorical preservation + refineAttributes inverse under WF + universal aggregator + finalizeParse closure.  Active branch: `b3d-3d5-format-dsl`.  Path A.4 (3d.4 + JSON-mirror, commit `320c5a9` on `b3d-3d4-json-detaint`) lifted the prior 47-module `--without-K`-only cluster to `--safe --without-K` and shipped Path A.5 Bool fast path on `Cache.lookupEntries` / `DBCHelpers.findSignalInList`.
...
- 4c-(a) ✅ shipped `cf091d8` — parseTopStmt restructured from 10-way `<|>` chain to 7-bucket head-character pattern dispatch (V / BA / BO / SI / SG / EV / CM); 5 simple section dispatchers + generic `parseTopStmt-on-BA-head` attribute helper landed under `Properties/Aggregator/Dispatcher/`.  Empirical wins: TVT 15.7 GB / 273s → 443 MB / 14s (35× memory, 19× speedup); other 4 simples 449-1430 MB / 8-33s; all under `-M16G` budget.  Foundations + BodyBridge + many-η-roundtrip-with-lift extension (previously WIP-uncommitted) shipped alongside.  AltChain.agda + HeadFails.agda deleted (chain-era proofs obsolete).  Module count 209 → 218.  All 760 Python tests pass; full Shake build 1m50s.
- 4c-(b) ✅ shipped `6c7ade3` — TAT universal attribute roundtrip closes task A.  `parseAttrLine-on-emit-Attribute` and `parseTopStmt-on-emit-typed-TAT` are now universal over `DBCAttribute` under `WFAttribute defs a` precondition.  3-way DBCAttribute dispatch (Universal.agda) routes through 31 existing parseAttrLine-roundtrip-* slims; TopStmt lift via NEW **β prefix-lemma strategy**: per-shape `emit*-chars-BA-head` Σ-witness (case-splits on scope/target inside a small `≡`-typed goal) + `parseTopStmt-on-BA-head-via-prefix` (accepts input + propositional prefix witness via `subst`).  Each TopStmt sub-dispatcher (Def/Default/Assign) becomes 1 case, no scope/target pattern-match.  **Empirical**: 7-case dispatch in TopStmt/Def & TopStmt/Assign each 25 min OOM at -M16G → **13.8s each (≥120× speedup)**.  WFAttribute.wfDef strengthened with WfAttrType (ENUM-non-empty) precondition.  13 new files under Properties/Aggregator/Dispatcher/Attribute/ + 3 Shakefile walk roots covering entire Aggregator subtree (cf091d8's aggregator work was not previously covered).  Module count 218 → 231 (+13).  Gates: check-properties 8m18s clean; check-invariants clean; shake build 1m47s; python test_format_dbc.py 13/13 PASS.  Lessons captured: `feedback_beta_prefix_lemma_pattern.md` + `feedback_pipestatus_for_real_exit.md`.
- 4c remaining — (B) apply many-η-roundtrip-with-lift (one-liner) instantiated at `(P, E, L, Stop) = (parseTopStmt, emitTopStmt-chars defs, liftTopStmt defs, TopStmtTypedWF defs)` backed by BodyBridge bytes-equivalence; (C) partitionTopStmts categorical preservation ~150-300 LOC; (D) refineAttributes inverse under WF ~200-400 LOC; (E) universal aggregator + finalizeParse closure + parseText↔parseTextChars bridge ~100-200 LOC.  Architecture **option α** (typed shadow + lift) confirmed in 4c-(a)+(b): TopStmtTyped mirrors TopStmt's 6 payload constructors; lift routes attributes through `rawOf defs`.  Universal target `∀ d → WellFormedDBC d → parseText (formatText d) ≡ inj₂ d` (option a: WF precondition; matches 4b's MessageWF/SignalGroupWF precedent).
