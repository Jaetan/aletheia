...
**🔬 r25 ADVERSARIAL 4-BINDING API REVIEW — IN PROGRESS** (spec `.archive/reviews/r25/FINAL_report.md` §3/§4; cadence = focused PRs, each SHA-verified green). **Shipped #80-99**: B1-B4 + a Dependabot batch (#95, 6 bumps) + a clang-tidy-coverage gate-fix (#98). **B3** (#94): Go `FloatToRational` int64 round-trip guard (arm64/whole-stack-cross-compile deferral → DEFERRED_ITEMS I.1) + Python `end_stream` warning validate-not-cast. **B4 cluster** (C++ parity): neg-denom reject + `IssueCode` wire-code preserve (#96); inline-YAML 64 MiB bound + no-DBC-message guard + bold Excel headers (#97); **BREAKING** `struct SignalError` (#99). **Gate-fix** (#98): C++ clang-tidy now lints ALL of `cpp/src` via `run-clang-tidy` over `compile_commands.json` + a `check_clang_tidy_coverage` orphan guard — closed a non-recursive-glob hole that silently skipped `src/detail/`. **Remaining B5-B8** (B5 = cross-binding BREAKING/semantics, 2 LOCKED §4 direction-calls). Detail + lessons: `memory/project_r25_binding_review.md` + CHANGELOG + git #80-99.
...
**🟢 PHASE 6 — CLI PARITY (C++/Go) ✅ #21** — Go `go/cmd/aletheia` + C++ `aletheia::run_cli` (`aletheia-cli`): 5 subcommands dispatching to the verified core; FEATURE_MATRIX `cli` ×3; real-FFI tests. `check` deferred (needs `can_log_reader`). Lessons: `memory/feedback_command_dribble_file.md`, `feedback_verify_actions_before_claiming.md`.
