...
**🔀 MERGE-QUEUE ARC ✅ 2026-07-06/07 (#157–#160) — #106 + #107 SHIPPED** — four PRs merged serially (the `main` ruleset is `strict:true` → each merge BEHINDs the next PR; choreography: `gh pr merge --squash --auto` + `gh pr update-branch`, ~26 min CI per cycle). **#157** the V2b/freshness UPD. **#158 CLI single-parse (#106 ✅)**: every CLI invocation parses a DBC ONCE (Python `.dbc` via `parse_dbc_text` on the subcommand's own client; the redundant ValidateDBC round-trip dropped in all 3 CLIs; Excel parse-status checked; `run_checks` gains `client=`). **#159 kernel dual-route factoring + C2 (#107 ✅)**: new leaf `Protocol/Handlers/LoadDBC.agda` = the single field-tagged bound cascade + validate-and-load epilogue shared by `parseDBC`/`parseDBCText` (handlers differ only in the context literal; the LoadDBC closure is text-parser-free so Handlers.agda stays under the 16 GiB elaborator cap; modules 277→**278**); **C2**: `validateDBC` runs the cascade → over-bound DBC rejects typed `input_bound_exceeded` (Python lifts via the new SSOT `_response_parsers.lift_input_bound_exceeded`, mirroring `lift_validation_issues`); the text route's bound MESSAGE now names the field (message-level; no new JSON key). Three review revisions: Copilot `checkCard` (bind each observed length once); the required mutation lane caught **6 `validate_dbc` survivors** → 2 correlated and/or-guard equivalents **designed out** by typed extraction + 4 killed by an `err.code` assertion ([[feedback_unkillable_mutant_is_design_signal]] applied); `*bound` splat (R0914). **#160 mutation-cache hygiene** (spun off mid-arc; user sequenced it BEFORE #159): mutmut's persistent `mutants/` work-tree invalidates on SOURCE but NOT test changes → 20 phantom `_converter` survivors on #159's merged tree; `run_python` now erases `mutants/` before every run (warm 29 s→clean 40 s, local-only; CI always checked out fresh) → [[feedback_mutation_cache_hygiene]]. Merge-hygiene lessons: a "clean" auto-merge can be structurally wrong (duplicate `### Changed` headers — `check-changelog` doesn't validate section shape); rebuild+reinstall after every branch switch (the freshness gate bites at pre-push). **Queue: PR D (#101 EOS reshape) → PR E (#102 Rust symbol cache).** Detail: PROJECT_STATUS head + CHANGELOG + git #157–#160.
