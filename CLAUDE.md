...
- **Roadmap**: (lint) continue the whole-tree ruff **package phase** (per-file, keeping the gated package green — do NOT blanket-autofix; `aletheia/` foundation + root deps + `client/` + `asyncio/` now clean, remaining dirs: `cli.py`, `dsl.py`, `excel_loader.py`, `yaml_loader.py`, etc. + `tests/` + `benchmarks/`), folding [[feedback_no_review_process_marks_in_code]] strips per file → **wire tools/ into the gate** (separate `pylint tools/` step + tools/ in basedpyright + ruff steps); (warm) wire `run_ci` (batch authoritative until weeks of agreement), land 5 confirmed dead imports (`++ₗ-assoc`/`concatMap`/`-_`/`sj`/`suc-injective`), steps 10-30; then merge `ci-speed`→main.
