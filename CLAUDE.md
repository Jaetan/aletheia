...
**✅ KEYLESS-SIGNED RELEASE AUTOMATION ✅ MERGED 2026-07-16 (#198).** Tag-triggered `release.yml`: a `v*` tag → `shake dist` → **keyless-sign** (Sigstore / GitHub-Actions ambient OIDC — NO key/passphrase in CI) → **self-verify vs the workflow OIDC identity, publish gated on it** → DRAFT Release. `shake dist` gained a keyless mode (`ALETHEIA_COSIGN_KEYLESS=1`; emits `.sig`+`.crt`); sign-mode resolved once → the sign command + the embedded MANIFEST verify recipe can't disagree. Acceptance dispatch dry-run PASSED (`Verified OK`; sidecars verify, no bundle format needed; publish correctly skipped). **Distribution reframe** (supersedes the manylinux-wheel plan): a GitHub Release + self-contained multi-binding bundle, NOT pip → the `--bignum=native` rebuild is not needed for distribution (libgmp stays system-linked, BSD-2-clean; it stays an optional future hardening item). **PR 1 of the distribution track** (keyless-CI + key-stays-local signing); PR 2 = expand `shake dist` payload to all four bindings + `install.sh`. Detail: CHANGELOG + git #198 + PROJECT_STATUS § Phase 6.
