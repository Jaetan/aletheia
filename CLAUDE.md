...
274 of 275 modules use `--safe`. No modules require `--sized-types`. The per-commit module-count genealogy (which split/extraction added each module across A.2, Path A.4, Tracks D/E, and rounds R18–R23) lives in [PROJECT_STATUS.md](PROJECT_STATUS.md) and the `memory/project_*.md` round files — not duplicated here.
...
**🔁 MERGE-QUEUE DAY + FLAKE HARDENING ✅ 2026-07-02/03 (#143-145 + #125-129)** — the whole open-PR queue drained + the jitter watch closed. **#143** UPD (decoder-sweep doc-sync + the new PARITY_PLAN "No row → invisible to the gate" rule); **#144** r25-**B7 comment-truth sweep ✅** (the user-pinned 3 YAML symlink→`cp` cache comments — ground truth Shakefile.hs `cp`; the cache = cabal's foreign-lib incremental state — plus ~30 binding-comment sites, every finding re-grounded against CURRENT code by a 36-agent find+adversarial-verify workflow: 6 stale review items left alone, 5 proposed rewordings corrected, then 7 Copilot round-2 precision fixes); **#125 RESCUED+MERGED** (B6e residual: C++ `Rational` ctor/`make` reject `double`/`bool`, BREAKING — had sat forgotten-OPEN 3 days after a stability flake; "task completed" ≠ "on main"); **Dependabot #126-129** (serial under strict; actions/cache v6 needed a hand-written CHANGELOG entry — bots don't write them and G.2 watches workflows; #129's PR flag stuck OPEN after its squash landed on main → closed manually with audit comment — when APIs disagree, git is truth); **#145 FLAKE HARDENING** (watch CLOSED, user: "flakiness is our enemy" — retry-once both advisory lanes, throughput re-MEASURES before re-gating, stability variance JSONs upload on EVERY run, `FLAKE-RETRY` tally = warning annotation + step summary + attempt1-artifact count + the ledger in [[project_python_stability_jitter_watch]]; thresholds UNCHANGED). venv → pytest 9.1.1 / ruff 0.15.20 / basedpyright 1.39.9 (tree-wide gates re-verified). **Lesson earned the hard way** ([[feedback_nonfatal_not_silent]]): non-fatal ≠ silent — a silenced `cp` shipped in the flake-evidence PR itself; now every shell-touching diff gets a pre-dribble `rg '\|\| true|2>/dev/null'` + justification pass. Detail: memory + CHANGELOG + git #143-145.

**🧪 CROSS-BINDING DECODER-REJECT SWEEP ✅ MERGED 2026-07-02 (#138-142)** — the cross-binding follow-up to the r25 wire-decoder-reject audit ([[project_xbinding_decoder_reject_audit]]): after Go's reject-branch pass, audited Py/C++/Rust for the same 5 reject classes. **#138** Go — reject tests + dropped 2 unreachable `json.Number` switch arms; **#139** Python — `extract_rational_from_dict` + bound-triple guards (the only tool-CONFIRMED hole); **#140** Rust — reject tests **tool-measured** (`cargo-llvm-cov` 74.6%→81.7%), which CORRECTED the read-based audit (flagged lines already covered; real gaps elsewhere); **#141** C++ — ~18 reject tests **plus the sweep's one real correctness bug**: `Marshal.hs` used Haskell `show` as a JSON encoder for the decimal error envelope's echoed `input`/`msg`/`code` → invalid `\NNN` escapes for non-ASCII (JSON needs `\uXXXX`), so `from_decimal("1.5€")` emitted an envelope NO binding could parse (misreported `Protocol` not `Validation`); fixed at the SSOT (`jsonString` encoder) + 4-binding non-ASCII regression tests; **#142** Rust (BREAKING) — lifted the typed `input_bound_exceeded` triple → `Error::InputBoundExceeded {code,bound_kind,observed,limit}` (was generic `Error::Core`), Go/C++/Python parity, + the missing `input_bound_exceeded_error` FEATURE_MATRIX row (matrix now **43 rows**: py 43/43, go 42/43, cpp 41/43, rust 40/43) — its absence is why the parity gate never caught the divergence. **Lessons**: read-based coverage audit = hypothesis, tool-measurement = evidence (both Rust+C++ read audits were wrong on specifics); "reachable" = USER-reachable; a capability with no matrix row is invisible to the parity gate ([[feedback_matrix_row_or_invisible]]). Detail: `memory/project_xbinding_decoder_reject_audit.md` + `memory/project_b6e_decrat_ffi.md` + git #138-142.

**🎯 FLOAT-PRINCIPLE SWEEP ✅ MERGED 2026-07-01 (#130-136)** — closed THE FLOAT PRINCIPLE (a value is an exact rational/DecRat everywhere — no float internal, none at UI-input — degrading to float ONLY at print-out to the user; timestamp input the lone Phase-6 exception): #130 Python wire-float reject; #131 demo-gate hardening; #132/#133 dead float decode-arms + `LogValue` float removed (Go/C++/Rust); #134-136 render-rationals-exactly (all 4 bindings: `extract --json` → exact `{num,den}`, CLI human-text + validation-error messages → kernel `format_rational`); whole-tree grep = NO lossy user-facing rational render remains. Detail: `memory/project_r25_binding_review.md` + `memory/project_log_rational_values.md` + `memory/project_b6e_decrat_ffi.md` + git #130-136.
...
**🔬 r25 ADVERSARIAL 4-BINDING API REVIEW** — 65 findings across Py/Go/C++/Rust (spec `.archive/reviews/r25/FINAL_report.md`). Shipped #80-136: B1-B5, the B6a-d wire-float arc #114-117, the **B6 decimal-SSOT** arc #119/#120 (kernel `aletheia_parse_decimal`; all bindings parse via it; heuristics deleted; float+bool rejected at every numeric boundary; all-text Excel; demo gate), **B6e UI-converters ✅** + the render-rationals-exactly direction-call #2 (= the float-principle sweep #130-136, top entry). **Still open**: B7 comment-truth (3 `.yml` `symlink`→`cp` comments in benchmark/pr-full-ci/pr-heavy-lanes, user-pinned), B8 dead-code/DRY remainder, r25-B6 logging items (doc.go 16-event / Go MessageByID deep-copy / Rust logging completeness). **Standing way of working**: attribution (`Co-Authored-By: Claude Opus 4.8` + PR Claude Code footer — `memory/feedback_commit_workflow_signed.md`), the single-slot dribble (user GPG-signs commits), Lever B pipeline-PRs, ultracode. Detail: `memory/project_r25_binding_review.md` + `memory/project_mutation_to_zero.md` + CHANGELOG + git #80-136.
