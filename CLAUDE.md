...
**Cluster Y** (2 commits — `c6a7e73` + this commit) — **Cross-binding-identical Rational renderer + DSL float-input parity** (GO-D-19.1, promoted from cluster R Phase-6 framing per user redirect 2026-05-15, then expanded again to A1+A2 per follow-up redirect). `c6a7e73` shipped the dispatch (terminating → `:g`/`%g`/`{:g}`, non-terminating → reduced `N/D`); pre-merge audit surfaced that the three languages' `:g`/`%g`/`{:g}` defaults differ in precision (Python+C++ truncate to 6 sig figs; Go uses round-trip-shortest) AND in scientific-notation thresholds, AND that Python's DSL `Fraction(float)` produces the exact IEEE 754 binary fraction (giant denom) while Go/C++ round-and-scale via 10^9. User redirect: "option A — pin all bindings to the same display." Follow-up commit closes both. **A1 — Renderer**: replaced `:g`/`%g`/`{:g}` dispatch with cross-binding-identical exact-decimal algorithm (GCD-reduce, split denom into 2^pow2 · 5^pow5, scale numerator into a digit stream of length ≥ k+1 where k=max(pow2, pow5), split at decimal point, trim trailing zeros). Pathological case k > 18 falls back to language `:g`/`%g`/`{:g}` (int64 multiplier overflow safety; typical DBC predicate values stay well under k=10). **A2 — Python DSL**: new `to_predicate_fraction` helper in `_helpers.py` mirrors Go `floatToRational` exactly (`Fraction(round(value * 10^9), 10^9)`); all 9 DSL methods flipped from `Fraction(value)` to `to_predicate_fraction(value)`. After A1+A2, `Signal.equals(0.1)` produces `Fraction(1, 10)` and renders as `"0.1"` in all 3 bindings; `Signal.equals(1234567)` renders as `"1234567"` (was lossy `"1.23457e+06"` in Python/C++); `Signal.equals(0.123456789)` renders as `"0.123456789"` (was lossy `"0.123457"`). 22 new tests cumulative; 2 existing tests adjusted for exact-rational semantics. Wire JSON shape unchanged. No benchmark needed (display + one-time DSL conversion paths). CHANGELOG ships two `Changed` entries (renderer + DSL). Memory `memory/project_smart_rational_renderer.md` rewritten. Module count unchanged at 248 (no Agda touched).
...
**Gates fresh at every cluster ship**: 11 Agda gates clean (build + check-properties / check-invariants / check-no-properties-in-runtime / check-erasure / check-fidelity / check-ffi-exports / check-bound-enforcement / count-modules / check-runbook / check-changelog); pytest (853p+1s post-W; cluster Y adds 4 → expect 857p+1s); Go test -race (both `go/aletheia/` and `go/excel/`); C++ ctest 10/10 post-X / post-GO-A-3.5 / post-Y; basedpyright 0/0/0; pylint 10.00/10 on `aletheia/ tests/ ../conftest.py`; gofmt + go vet + clang-format + clang-tidy clean on canonical `tools/run_ci.py:506` src scope (X added `<concepts>` to `types.hpp` + `ltl.hpp`; Y adds `<numeric>` + `<cstdlib>` to `enrich.cpp` for `misc-include-cleaner`); doc-fence harness 116/116. Cluster X throughput benchmark vs `cpp_throughput_baseline.json` 2026-05-12: max deviation +8.55% Stream LTL with σ 4.77% — within session-distant ±10% per `feedback_wsl2_variance_stance.md`; CAN-FD lanes ≤ +1.10% with σ ≤ 0.88% confirming the composition refactor is codegen-identical. GO-A-3.5 is a pure source-only identifier rename — generated machine code identical, no benchmark needed per `feedback_hot_path_refactor_benchmark.md`. Cluster Y is display-path only (pretty-printer reachable from `format_formula` / `BuildDiagnostic` / `format_enriched_reason`, none on streaming hot path) — no benchmark needed.
