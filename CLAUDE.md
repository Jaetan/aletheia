...
- **Layer 4 pending** — top-level aggregator induction over the `DBC` record + the owed char-class-disjointness bridge lemmas (`isIdentStart→¬isHSpace`, `isIdentCont→¬isHSpace`, `isIdentCont→¬isNewlineStart`) plus `showInt-chars-head-non-hspace` (locally provable, ~15L).  Commits 3d.4+ (`manyHelper-parseSignalLine-body` recursion induction + signal-list resolution roundtrip + `parseMessage-roundtrip` outer composer) pending.
