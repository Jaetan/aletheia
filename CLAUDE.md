...
**🧬 MUTATION LANE REPAIRED + CACHED + REQUIRED ✅ 2026-06-20 (#71/#72)** — a user question surfaced the advisory `mutation testing` lane as **crash-dead** (zero mutants since #51 — a baseline-COLLECTION failure, NOT new survivors): mutmut runs pytest from the relocated `python/mutants/` tree where two post-baseline tests broke collection (`test_check_changelog.py` imports repo-root `tools`; `test_excel_loader.py`'s demo_workbook tests use `parents[2]` outside the copied tree). **#71** `--ignore`/`--deselect` + mutmut-3.6 key migration (`source_paths`/`pytest_add_cli_args_test_selection`, zero warnings) + always-on `check_mutation_setup` gate (every `tools`-importing test must be mutmut-ignored) → reconciles to baseline **827/1/828**. **#72** cached the lane (push:[main] seeds Mull + build-tree caches cross-branch; `.so` no-ops for non-Agda PRs; **no ccache** — Mull-plugin stale risk) cutting cold ~33min→~22min + renamed the job; `mutation testing` is now a `main`-ruleset required check (added via gh `PUT`, not PATCH). Detail: `memory/project_mutation_to_zero.md` + CHANGELOG + git #71/#72.

**🦀 RUST PARITY R1–R4 ✅ COMPLETE 2026-06-17/19** — the Rust binding now matches the Check + ergonomics tiers (rust **34/40**). **R1** typed `Dbc` family + `BA_*` attrs + parse/format/validate (#53/#54/#55); **R2** `build_frame`/`update_frame`/`send_frames` + mux extract (#57); **R3** check tier — fluent `check::signal`/`check::when` DSL → LTL + `add_checks` (#59), `load_checks_from_yaml` behind a default-on `yaml` feature (#62), `aletheia-excel` crate + cross-binding Excel column-handling unification (by-name, **absent col ≡ empty**, **STRICT coercion** — text-number rejected for numeric fields, BREAKING Go/C++; #65); **R4** ergonomics — `ClientBuilder` structured logging (callback `Logger`) + RTS-cores config, lazy_streaming→n/a (#67), client-side violation enrichment (#68), runtime-agnostic **`AsyncClient`** (feature `async`: a worker thread owns the `!Send` `Client`; cancellation = drop the future; `Send + Sync` via a Mutex-wrapped job sender; #69). Idiomatic divergence where it offers the same feature the Rust way (no client DBC cache; `impl Into<Rational>`; `u64` ms; future-drop cancellation, no `ctx`/`stop_token`). **R5 (`mock_backend` + `backend_di_seam` → 36/40) is the last in-plan slice; its merge triggers the OWED thorough Rust review** of the whole `rust/` + `rust/excel/` surface (user-requested 2026-06-19; milestone-trigger, not dated). cli/can_log_reader/doc_example_gate → Phase 6. Detail: `memory/project_rust_parity_r1.md` + `memory/project_excel_column_handling.md` + `docs/development/RUST_PARITY_PLAN.md` + CHANGELOG + git #53–69.

**🐹 GO CHECK-BUILDER FLOAT-ERROR FIX ✅ MERGED 2026-06-18** (`main` `5c67c9b`, #61) — cross-binding parity bug spotted during R3b float→rational research: Go's check builders **silently clamped** a bad float (NaN/±Inf/int64-overflow) to `0/1` via `RationalFromFloat`, while Python raises + C++ throws. Fix (uniform-error, user-chosen): `NeverExceeds`/`NeverBelow`/`NeverEquals`/`Always` now return `(CheckResult, error)`; value methods funnel through the error-returning `FloatToRational`; fluent chains carry a `valueErr` (sibling to `rangeErr`) surfaced at `.Within()`. `RationalFromFloat` (literal-construction convenience) now **panics** on non-finite (regexp.MustCompile convention) — last clamp path removed. Fixes the Go YAML+Excel loaders. Tests at builder/YAML/Excel/`RationalFromFloat` levels; benchmark before/after = noise (check construction is off the per-frame hot path). BREAKING (Go). Architecture confirmed: float→rational scaling is binding-side by necessity (FFI takes int pair); canonical normalization/wire-shape/arithmetic delegate to the proven core.
