...
**⚡ CI-SPEED + PROOF-GATE EXHAUSTIVENESS ✅ MERGED 2026-06-29 (#122/#123)** — **#122** throughput lane ~12min→~4.5min: the build-tree cache now also captures `haskell-shim/dist-newstyle` (the cabal foreign-lib build — 276 MAlonzo .o/.hi; omitting it made cabal recompile them ~47s EVERY run; `build/libaletheia-ffi.so` is `cp`-copied from it, not a symlink) across all 3 workflows; + benchmark-lane ccache / cpp-deps (distinct `-bench` keys) / pip / `paths-ignore` / cabal-update reorder. **ghcup cache DROPPED as infeasible** — the runner's `~/.ghcup` is a symlink to a 6.2 GB `/usr/local/.ghcup`, so caching it evicts the real caches under the 10 GB budget. **#123** `check-properties` (the hand-maintained `proofModules`) was NOT exhaustive — 2 proof modules (`DBC/TextFormatter/Properties`, `Protocol/Adequacy/StreamingWarm`, the lemma making `warm-cache-agreement` unconditional) were type-checked only by the `iwyu --all` backstop; added both + a drift-proof `check-proof-coverage` gate (in run_ci's `AGDA_SHAKE_TARGETS`) asserting `closure(proofModules)∪closure(build)` covers every `src/**.agda`. Lessons → `memory/project_ci_speed_optimization.md`: ghcup-is-a-symlink; shellcheck/actionlint local-gate-parity (install shellcheck so local actionlint runs SC checks); cache-effectiveness = cache-SIZE telemetry. Detail: CHANGELOG + git #122/#123.

**🔬 r25 ADVERSARIAL 4-BINDING API REVIEW — B6 decimal-SSOT arc SHIPPED (#119/#120); remaining B6e UI-converters / B7 / B8 (CI-caching PR ✅ #122)** (spec `.archive/reviews/r25/FINAL_report.md`; focused PRs, each SHA-verified green). **Shipped #80-120**: B1-B4 + Dependabot + the 3-point sequence (#103-110) + throughput **Lever A** #111 (diff-scoped `mutation testing` lane, fails SAFE to full) + **B5b** decode-lockstep #113 + the **B6a–d wire-float arc** #114-117 (all 4 bindings reject wire floats in lockstep) + the **B6 decimal-SSOT** arc: **#119** Agda kernel `aletheia_parse_decimal` + **#120** every binding parses decimals through it (heuristics `from_double`/`from_f64`/`float_to_rational`/`PhysicalValue` **deleted**), numeric API params exact (`int|Fraction` / `Rational`), **float AND bool rejected at every exact-numeric boundary** (Python via a shared `to_exact_fraction` SSOT in `_helpers/rational.py`), **all-text Excel** (a number cell is rejected with row+column+value+reason), RTS-gated+vocal loaders, and a **demo gate** that runs every `examples/demo/*.py`. #120 review: 6 Copilot (bool ×2→SSOT, doc-import ×4) + 4 self-caught doc-truth (stale `floatToRational`/`from_double` docstrings, whole-number float signal-values→`int`) fixed; mutation lane repaired (the new demo gate aborted mutmut baseline collection → `--ignore` it; pristine 828/0/1). **THE FLOAT PRINCIPLE (user, 2026-06-27)**: no float anywhere internal — internal OR UI-input; a rational degrades to float ONLY at print-out; a decimal is a `DecRat` (exact, denom 2^a·5^b), never a `float64`. **Remaining**: kill the UI float-input converters (locked DecRat-FFI design + the still-accepted Python DBC-dict `factor`/`offset` float — folded here → `memory/project_b6e_decrat_ffi.md`), B7 comment-truth, B8 dead-code/DRY (CI-caching PR ✅ shipped #122 — see the top entry). **Standing**: attribution (`Co-Authored-By: Claude Opus 4.8` + PR Claude Code footer — `memory/feedback_commit_workflow_signed.md`), Lever B pipeline-PRs, the dribble, ultracode. Detail: `memory/project_r25_binding_review.md` + `memory/project_mutation_to_zero.md` + CHANGELOG + git #80-120.
