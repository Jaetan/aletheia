...
**🦀 RUST PARITY R1 (typed DBC document model) ✅ COMPLETE 2026-06-17** (`main` `85bbf22`; #53/#54/#55, 11 rows) — full typed `Dbc` family + `BA_*` attribute vocabulary, deserialize + serialize vs the core's canonical JSON, parse/format/validate + mux-query/lookup; design mirrors Go/Python (hand-mapped serde_json; i64 int/hex bounds vs Rational float; `message_by_id(CanId)` std/ext). Copilot caught a real extended-CAN-id cross-binding bug (#53). Detail: `memory/project_rust_parity_r1.md`.
