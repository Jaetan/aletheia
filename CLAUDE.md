...
**R22 A-1.1 batches 7-25g (2026-05-18 → 2026-05-20)** — batches 7-9 landed on `review-r21` pre-merge; batches 10-25g on `review-r22` post-merge.  Per-batch narrative + commit hashes consolidated in `memory/project_review_round21.md`; summary:

- **b7-b18** (mid-density tier, 60 files / ~480 candidates): lessons — `feedback_qualified_access_empty_using.md` (b10), `feedback_public_reexport_blindspot.md` (b14, scanner v2.4 `public`-line skip), Assign.agda R22-PRE-CLOSE side-observation (b15).
- **b19-b24** (5 per batch, ~640 candidates total): scanner evolved v2.4 → v2.5 (b20 WSL restart) → v2.6 (b23 multi-line `using\n  (` fix); persisted to `~/.local/bin/` at b24.  Lessons: `feedback_grep_E_word_boundary.md` (b19), `feedback_unicode_brackets_scanner.md` (b20), `feedback_scanner_fn_audit_heavy_imports.md` (b21+b22), `feedback_scanner_fn_mixfix_with_args.md` (b23).
- **b25-series** (b25b → b25n, 13 batches 2026-05-19 → 2026-05-21; commits `b65bae5` / `458f3ba` / `2d73f6e` / `744901a` / `f042b31` / `41bad7c` / `f2ed4f1` / `6b50d50` / `eb6ba83` / `4a67777` / `6520001` / `d1a2d69` / `69c032b`): ~86 files / ~516 dead removed.  Series climbs through Format subtree → Aggregator/Dispatcher/Attribute siblings → deep long-tail → top long-tail → rank-5/rank-4 tiers.  FP rate trajectory: 3.3% → 26.9% → 25.5% → 34.0% (b25h heavy-DSL peak) → 20.0% (b25i recovery) → 56.4% (b25j) → 45.8% (b25k) → 75.4% (b25l) → 63.3% (b25m) → 77.1% (b25n).  **FOUR new FN-class lessons captured early in series**: b25e `feedback_scanner_fn_module_qualified_access.md`, b25g `feedback_scanner_fn_mixfix_section.md` + `feedback_scanner_fn_module_path_segment.md`, b25h `feedback_scanner_fn_mixfix_free_combinator.md`.  **No new FN classes after b25h** — workflow stabilised but diminishing returns clear by b25n (77% FP).  User flagged "boring" mid-session, captured in `feedback_review_process_revisit.md`.  Per-batch narrative + commit hashes + per-file FP/FN inventory in `memory/project_review_round21.md`.

- **`tools/prune_unused_imports.py`** (built 2026-05-20 → 2026-05-21; commits `a3b580c` / `2af4620` / `06d2ac1` / `df58123` / `b1d3d34` / `5788a12` / `403555b`): replaces the regex-scanner workflow with agda-driven brute-force.  For each `using` name and each `renaming` pair, attempts removal, runs `agda --type-check`, keeps the removal iff exit 0 + no new semantic-change warnings + (for public blocks) all direct consumers still type-check.  Handles every edge case the scanner couldn't (mixfix syntax sugar, sections, free combinators, module-qualified access, multi-line `using`, renaming, public re-exports).  4-way parallel concurrency with **topological-level batching** to avoid `.agdai` races between workers; crash-safe `.prune-bak` sidecars; `--restore-backups` for recovery.  CI-gate-ready: `--check-only` mode exits 1 on dry-run findings (matches gofmt/prettier convention).  Lives in-repo at `tools/prune_unused_imports.py` (committed; no `~/.local/bin/` shadow copy).  Two bugs discovered AND fixed mid-development: (a) `PatternShadowsConstructor` silent semantic change — removing data-type constructors from `using` clauses reinterprets pattern matches as variable bindings (warnings on **stdout** not stderr; commit `06d2ac1`); (b) `--include-public` per-file check insufficient for public re-exports — consumer-aware check builds reverse dep graph + type-checks direct importers per removal attempt (commit `df58123`).  Third issue: parallel-worker `.agdai` race condition (44% baseline-failure rate at first; commit `b1d3d34` introduces topological-level batching that eliminates it).  Lessons consolidated in `memory/feedback_agda_import_pruning_safety.md`.

**Manual workflow cumulative (pre-tool)**: ~200 files / ~1908 candidates via the scanner-based b25 series.  Superseded 2026-05-20 by `tools/prune_unused_imports.py`.  Scanner FN class catalog (sections / module-qualified / free-combinator / path-segment / mixfix-with-args / unicode-brackets / public re-exports / `renaming` clauses) consolidated in `memory/feedback_agda_import_pruning_safety.md` + the per-class `feedback_scanner_fn_*.md` files.  Both regex scanner (`tools/scan_dead_imports.py`, in-repo, with `scan_dead_imports.ignore` FP suppression) and precise agda-driven tool (`tools/prune_unused_imports.py`) are the canonical successors.

**Gates fresh at `69c032b`** (R22 A-1.1 batch 25n landing, 2026-05-20): build clean **2m05s** / check-properties clean **9m54s** "All proof modules type-checked successfully!" / count-modules **263** (unchanged across b19-b25n).  Per-batch gate timings b25b-n: build avg ~2.5 min, check-properties avg ~11 min.  Binding tests NOT re-run for b25-series — dead-import pruning is MAlonzo-neutral.  Last green binding posture at `dd8bb81`: pytest 870p+1s (6.27s), Go -race ok 7.343s, C++ ctest 10/10 (22.78s).  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**AGDA-A-1.1 status: ✅ COMPLETE** — tool-driven sweep #4 (`56ac3d6`, 2026-05-22, 15h wall) removed **532 dead names across 90 files**, 0 baseline failures, project-wide `check-properties` PASS.  Race-free via topological-level batching (`b1d3d34`); 3-tier safety net.  Combined with manual b25 series (~605 dead): **~1100+ total dead names removed in the R22 campaign**.  Full narrative + sweep-#2/#3 failure-mode diagnosis in `memory/project_review_round21.md` (R22 carry-over section).

**Sanity-gate infrastructure** (`403555b` + `d503d75` + `2c01026`): pre-commit `tools/scan_dead_imports.py` (regex, advisory; FP suppression via committed `tools/scan_dead_imports.ignore`, 312 entries / 146 files) + pre-push step 9 `tools/prune_unused_imports.py --check-only --no-topo` on `git diff main...HEAD` (strict, blocks push; 30-file sanity threshold + `ALETHEIA_PRUNE_GATE_NOLIMIT` override).  `BASE_STEPS` 29 → 30.  Exit codes match gofmt/prettier convention.  Fresh-clone docs updated (BUILDING.md step 8 + CONTRIBUTING.md + CI_LOCAL.md); all dev tools in-repo at `tools/` only (`~/.local/bin/` shadow copies retired `13a62fb`).

**AGDA-D-12.1 status: ✅ COMPLETE** — closure at `d51cdb1` (2026-05-22) shipped all 3 REQUIRED follow-ups: new `LogEvent.endstream.uncached_atom` (warn) emitted per `CompleteWarning` by all 3 bindings' `end_stream`, alongside the existing aggregate `stream.ended numWarnings`; RUNBOOK § End-of-stream diagnostics entry; PROTOCOL § End-of-stream Warnings spec (wire shape + 7-step evolution rule for new kinds).  Cross-binding parity tests bumped 15 → 16 with sanity-floor asserts mirroring existing `dbc.parsed` floor.  Prose count drift fixed across 5 sites.  In C++, `log_end_stream_summary` helper extracted to keep `end_stream` under clang-tidy's cognitive-complexity 25 threshold.  In-source DEFER block at `StreamingWarm.agda:391-405` flipped to a CLOSED historical record + CHANGELOG entry under `[Added] § Cross-binding`.  Gates fresh at `d51cdb1`: build 2m06s · check-runbook 16/16 · pytest **872p+1s** (+2 new vs `56ac3d6` baseline) · basedpyright/pylint clean · Go test -race PASS 8.3s · C++ ctest 10/10 · clang-format clean.

**Assign.agda b15 marker ✅ RETIRED** at `57ad862` (2026-05-22, comment-only) — b15's 18 `ModuleDoesntExport` warnings already cleared as a side-effect of later A-1.1 work; detail in `memory/project_review_round21.md`.

**Next pre-close items**: (1) review-process meta-review (user directive 2026-05-20; `memory/feedback_review_process_revisit.md`).  AGDA-A-1.1 + AGDA-D-12.1 + b15 all closed.  **Push gap**: local `main` 162+ commits ahead of `origin/main` — R20+R21 merges + R22 work held local per `feedback_review_branch_workflow.md`.
