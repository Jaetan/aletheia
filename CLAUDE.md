...
**🦀 RUST PARITY R3c (Excel loader) + CROSS-BINDING EXCEL COLUMN-HANDLING UNIFICATION ✅ COMPLETE 2026-06-19** (`main` `986423f`, #65) — new **`aletheia-excel`** crate (`rust/excel/`, mirrors `go/excel/`): `load_checks_from_excel` / `load_dbc_from_excel` / `create_template` (calamine + rust_xlsxwriter + zip), strict typed coercion on calamine cells, presence-optional `Extended`/mux/`Unit`, 64 MiB + saturating-add ZIP-bomb hardening. Matrix gate now resolves a single-colon `pkg:symbol` entry vs `rust/<pkg>/src` → `excel_check_loader` implemented (rust **30/40**, **Slice R3 complete**). A user-directed design review unified Excel column handling across ALL 4 bindings: lookup by header name, all columns read, **absent column ≡ empty cell**, and **strict coercion** — a number stored as TEXT is rejected for a numeric field (Message ID still takes hex). BREAKING — Go: `LoadDbc` drops the required-`Extended` + rejects text-numbers; C++: type-aware via `XLValueType` + reads all columns. Locked by a gated demo-workbook DBC test + a number-as-text rejection test per binding. Empirical (probed): calamine gives `Float` for whole numbers; excelize native number = `CellTypeUnset`/`Number`. Copilot 5 fixes. Detail: CHANGELOG `[Unreleased]` + git #65 + `memory/project_excel_column_handling.md`.
...
**🦀 RUST PARITY R1+R2+R3a+R3b ✅ COMPLETE 2026-06-17/18** — R1 typed `Dbc` family + `BA_*` attrs + parse/format/validate (#53/#54/#55); R2 `build_frame`/`update_frame`/`send_frames` + mux extract (#57); R3a fluent `check::signal`/`check::when` DSL → LTL + `add_checks` (#59); R3b `load_checks_from_yaml` behind a default-on `yaml` feature, schema-portable with the peers + shared `Rational::from_f64` + 64 MiB/symlink hardening (#62); doc UPDs #56/#58/#60/#63. Idiomatic divergence from Go/Python where it offers the same feature (no client DBC cache; `impl Into<Rational>`; `u64` ms). Detail: `memory/project_rust_parity_r1.md` + git + CHANGELOG.
