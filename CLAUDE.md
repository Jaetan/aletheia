...
**Cumulative R21/R22 A-1.1 closure (manual workflow): ~200 files / ~1908 candidates** via the scanner-based workflow.  Manual workflow superseded mid-session 2026-05-20 by `tools/prune_unused_imports.py` (agda-driven); tool-driven sweep #4 (commit `56ac3d6`) is the canonical post-prune state — 90 files / 532 dead names removed, project-wide `check-properties` PASS.  The original regex scanner `~/.local/bin/scan_aletheia_dead_imports.py` has been retired and removed; replaced by the in-repo `tools/scan_dead_imports.py` (committed) with an ignore-file mechanism at `tools/scan_dead_imports.ignore` (312 known-FP entries post-sweep-#4).  **Scanner FP rate climbs with depth into proof tree, stabilises in long-tail then ticks up on heavy-DSL-mixfix surfaces** (b25b 3.3% → b25e 26.9% → b25f 25.5% → b25g 25.5% → b25h 34.0% → b25i 20.0%): mixfix `_≡_` at theorem signatures + every `_X_` operator at heavy-mixfix parser surfaces are highest-recurrence FP forms.  **Known scanner FPs**: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉`/`-[1+_]` outside ident class; (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` body tokens); (3) `public` re-exports (v2.4 line-skip).  **Known FN classes**: heavy `using (...)` clauses, `import M as N` module aliases, multi-line `using\n  (`, `_X_` mixfix-with-args (`∣_∣`, `ifᵀ_then_else_`, `-[1+_]`, `_≈ᵇ_`, `_≡_`, `_<|>_`, `_∷_`, `_+_`, `_≤ᵇ_`, `_⊎_`, `_*>_`, `_>>=_`, `_>>=ₑ_`), **`module X` literal with qualified `X.field` body** (b25e), **`_X_` mixfix sections `(_X y)` / `(x X_)`** (b25g — `_∈?_`), **single-type module-path tail segments** (b25g — `ValueTable`/`SignalGroup`), **`_X_` as free combinator at higher-order argument site** (b25h — `_≢_` in `AllPairs _≢_ args`), `renaming (X to Y)` clauses (scanner doesn't process at all).  **Workflow**: per-name `grep -cP` audit after scanner pass + `simplify` skill quality-review agent post-prune (catches residual `renaming` + within-using-clause items); for `_X_` flagged dead, run FOUR checks: infix `a X b`, left section `(_X y)`, right section `(x X_)`, AND free combinator `_X_` literal (b25h); for single-CamelCase types from `Types` modules coexisting with submodule imports terminating in same identifier, add `.` to lookbehind to disambiguate module-path segments.
...
**Next**: wait for `tools/prune_unused_imports.py` full sweep #2 (background task `bswa2vei5`, launched post-fix 2026-05-21 with `-j 4 --include-public --final-check`) — estimated 10-15 h wall clock.  On completion, the tool's removals (with consumer-aware safety) become the canonical post-prune state.  **AGDA-D-12.1 follow-ups REQUIRED** unchanged: per-warning enumerant + cross-binding parity + check-runbook + PROTOCOL.md (in-source block at `Adequacy/StreamingWarm.agda:392-406`).  **Pre-close blocker** unchanged: 18 `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` (see in-source `R22-PRE-CLOSE` block).  **Review-process revisit REQUIRED** before R23 (user directive 2026-05-20): meta-review the review process itself; detail in `memory/feedback_review_process_revisit.md`.  **Push gap**: local `main` 152 commits ahead of `origin/main` — R20+R21 merges held local per `feedback_review_branch_workflow.md`.
