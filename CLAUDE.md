...
**Latest (2026-04-18, in progress):** AGENTS.md review round 17 — decomposed into 7 commits (C1–C7). **C1 shipped** as `db2d90c` (13 files, +354/−52): Agda per-file items 1, 2, 4, 5, 6 + system items 13, 14, 15. Item #3 (stdlib-mandated `{{NonZero}}` instance args) resolved in-source as `DEFER-stdlib-mandate` comment blocks in three modules with AGENTS.md Cat 29 text updated accordingly. New build guards: Shakefile phonies `check-ffi-exports` + `regen-ffi-exports` paired with `haskell-shim/ffi-exports.snapshot`; Shakefile phony `check-stdlib-version`. PROTOCOL.md got a "Streaming Semantics: Soundness vs. Completeness" section clarifying that `pipeline-adequate` is offline-conditional on `AllObserved`. **C1-fixup queued** with three user-approved carve-outs: (8a) remove dead `_-_` from `Evaluation.agda:13`, (9b) full harmonisation of error-message formats (adopt `InContext` wrapping uniformly across all 5 error ADTs + unified quoting), (11a) replace local `+-assoc-cancelʳ` in `Rational.agda:186` with stdlib lemma. C2–C7 scope recorded in `.r17-plan.md`. R17 benchmark audit (15 runs via two-batch protocol) confirmed no real regression vs R16 baseline; apparent B1 negatives were session-level WSL2/thermal noise, normalised by B2. See `.session-state.md` for detailed position + resume instructions.
