...
**✅ AGENT-MEMORY-CITATION GATE ✅ MERGED 2026-07-16 (#196 + UPD #195).** `tools/check_no_memory_citations.py` (run_ci + pre-commit-fast) fails on 3 structured store-pointer shapes (`[[<prefix>_…]]` / `memory/<name>.md` / `<prefix>_<name>.md`) in every tracked non-binary file — source AND user-facing docs; CHANGELOG/PROJECT_STATUS **gated**, AI-infra (CLAUDE/AGENTS/DEFERRED_ITEMS) + `docs/development/E2_*` backlog **exempt**. Closes the arc's "extend the citation check to source" open idea. Caught **11** dead pointers on first run (source `.yaml`/`.gitignore` + product docs + CHANGELOG history), all stripped pointer-out-point-kept. [[feedback_no_memory_citations_in_source]] · [[reference_python314_except_syntax]] (PEP 758). Detail: CHANGELOG + git #195/#196.

**✅ GATE-SOUNDNESS + IWYU + COMMENT-QUALITY ARC ✅ MERGED 2026-07-16 (#190–#194).** The post-E.2 gate-soundness follow-ups (§G.2, gate audit, §G.1, comment-quality, doc tracking-IDs), all squash-merged to `main`:
- **§G.2 (#190, `183898c6`):** `DBC.Properties.*` reserved for proofs; deciders moved to a new `DBC.Decidable.*` namespace, making `check-no-properties-in-runtime` an EXACT assertion over the foreign-library cabal closure (441 modules, ZERO `DBC.Properties.*`). 287→290 modules.
- **Gate audit (#191, `f759d944`):** USER RULE born here — *a gate that doesn't fail on an issue HAS A BUG; failing IS its function.* Swept every Shake `check-*` + `tools/check_*.py` + both git hooks; **8 gates + 2 hooks could PASS while checking nothing** (missing dir → `return 0`; discarded grep's exit-2; empty snapshot → "0 entries"; dead `pre-commit` mode; hooks "skipping" → allow). All fixed + regression tests; exit 2 = could-not-check vs 1 = violation. SSOT [[feedback_gate_pass_is_absence_of_output]].
- **§G.1 iwyu (#192, `68b88e36`):** the `.agdai` import gate reported a tree holding **20 dead imports** as clean — a name counted USED as soon as elaboration baked it into the module's terms, even with no source token. Fix: such a name counts only when elaboration SEARCHED THE SCOPE for it (instance / copy-delegation); overloaded names keep the trusting rule (highlighting can't see their term-position uses). Validated vs the recompile oracle 20/20; guard fixtures proven to have TEETH. [[feedback_fixture_matrix_coverage_is_the_gate]].
- **Comment-quality pass (#193, `2597d04c`):** comments state goals + rules, not history. 14 verified-incorrect comments fixed — 3 stated FALSE things about the verification (e.g. the universal round-trip theorem minus its `WellFormedTextDBCAgg` hypothesis). All plan-scaffolding + agent-memory-store citations stripped from EVERY source language (Agda/Py/Go/C++/Rust/tools/Shakefile); line-cites → symbolic. 85 files, documentation-only, −102 lines. [[feedback_no_memory_citations_in_source]].
- **Doc tracking-IDs (#194):** 3 provenance PR-numbers dropped from the workflow docs; product-facing doc surface otherwise clean; the E2_* proof-strategy docs' deletion-trigger recorded in DEFERRED_ITEMS.md (delete them once the E.2 proof lands). Module count 290 (unchanged; comment/doc-only). Detail: PROJECT_STATUS + git #190–#194.

**✅ E.2 ROUTE (b) — always-strict `format_dbc_text` ✅ MERGED (#181/#183/#187 + review #188).** Returns `.dbc` text that provably re-parses to the input DBC, or a typed `handler_text_roundtrip_failed` refusal — never silently-lossy; machine-checked (`formatDBCTextResult-sound`), ×4 bindings. Routes (a)/(c) deferred (lossless emission; stronger validator). [[project_e2_reexamination]] + docs/development/E2_ROUTE_B.md.

**🏷️ 3.0.0 RELEASED (#171, tag `v3.0.0`)** — first major since v2.0.0 (GPG-signed tag; signed dist cosign+Rekor; breaking ×4 bindings). Earned a build-integrity fix: version-scoped fail-loud `.so` select (the `find|head-1` locate could ship a STALE prior-version `.so`) + dist stale-tree prune. [[feedback_release_stale_dist_tree]]. Detail: PROJECT_STATUS + git #171.

**🔀 MERGE-QUEUE ARC ✅ (#157–#168) — CLOSED** — CLI single-parse, kernel dual-route `Handlers/LoadDBC` + typed `input_bound_exceeded`, EOS extract-once ×4, Rust FFI symbol cache, MockBackend error-on-exhaustion ×4. Binding-review program + follow-up queue CLOSED. [[feedback_unkillable_mutant_is_design_signal]] · [[feedback_mutation_cache_hygiene]] · [[feedback_pr_open_protocol]] · [[feedback_cross_binding_consistency_as_fp]] · [[project_r25_binding_review]]. Detail: git #157–#168.

**🎯 BYTE-EXACT PARSER POSITIONS ✅ + SHAKE-INSTALL / FRESHNESS GATE ✅ (#155/#156)** — furthest-failure positions on the watermark encoding (~100 proof modules restated at the `proj₂` outcome level, bindings green zero edits, hot path untouched); `shake install` repaired + deployed-kernel freshness gate (`check_install_freshness.py`). [[feedback_conflict_blocks_pull_request_ci]] · [[project_v2_parser_position_design]]. Detail: CHANGELOG + git #155/#156.

**🎯 VALIDATE FIRST-TOUCH ✅ (#147–#153)** — `aletheia validate` on a broken DBC lists the full issue set (errors AND warnings) and exits 1 ×3 CLIs, locked by a cross-CLI harness (`docs/CLI_SCENARIOS.yaml` + `test_cli_parity.py`); kernel structured `issues` envelope + positioned parse errors + typed `AttributeRefinementFailed`. [[feedback_stacked_pr_retarget_before_parent_merge]] · [[feedback_never_unsigned_commits]] · [[project_validate_cli_first_touch]] · [[project_cli_integration_harness]]. Detail: git #147-#153.
