...
**🦀 RUST PARITY R1 (typed DBC document model) ✅ COMPLETE 2026-06-17** (`main` `85bbf22`; PRs #53/#54/#55) — full typed `Dbc`/`DbcMessage`/`DbcSignal` family + `BA_*` attribute vocabulary (`Attribute`/`AttrType`/`AttrValue`/`AttrScope`/`AttrTarget`), deserialized from + serialized to the core's canonical JSON; `Client::parse_dbc`/`parse_dbc_text`/`format_dbc`/`format_dbc_text`/`validate_dbc` + mux-query/lookup helpers. All **11 R1 FEATURE_MATRIX rows → implemented** (rust now **22/40**): read side #53 (7), tier-2 attrs #54 (1), serialize side #55 (3). Design mirrors Go/Python (hand-mapped serde_json; i64 int/hex bounds vs Rational float; `message_by_id(CanId)` disambiguates std/ext). Copilot: a real extended-CAN-id cross-binding bug fixed (#53) + strict-decode/`json!`-clone cleanups (#55). Next: **R2** (frame construction). Detail: `memory/project_rust_parity_r1.md`.
