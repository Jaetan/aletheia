...
**Current Status**: ✅ 148 of 149 Agda modules use `--safe --without-K`; 1 allowlisted Unsafe module (B.3.d substrate, see breakdown below)

### Module Safety Flag Breakdown

**By flag combination** (149 total):
...
- **1 module**: `--without-K` only (no `--safe`) — `Aletheia/DBC/TextParser/Properties/Substrate/Unsafe.agda`. Hosts the two `String ↔ List Char` bridging axioms (`toList∘fromList`, `fromList∘toList`) needed to compose the B.3.d universal roundtrip theorem.  Mirrors stdlib's `Data.String.Unsafe` (proven by `trustMe` under `--with-K`); structurally unprovable in `--safe --without-K` because the Agda String primitives reduce only on closed terms.  Allowlisted by name in `Shakefile.hs` `check-invariants`; any other Unsafe module or `^postulate` line is rejected.

**148 of 149 modules use `--safe`**. No modules require `--sized-types`.
...
**Phase B.3.d Layer 2 — in progress (2026-04-24).** Commit 2a (`9adbc46`) lifted `Identifier` from bare `String` to `{ name : String; valid : T (validIdentifierᵇ (toList name)) }` (T3-fixed; 5–10% Signal Extraction regression accepted — see `project_identifier_eq_revisit.md` for revisit angles). Commit 2b (this commit) ships `parseIdentifier-roundtrip` — the first per-primitive theorem at `List Char` level. Commit 2c next: remaining primitives (ByteOrder digit, sign flag, mux marker, StringLit, AttrScope, AttrType/AttrValue tags, SignalPresence). DecRat-side already closed via `fromℚ?-after-toℚ`.
...
- **Phase B.3.d status** — **pre-gate ✅; layer 1 ✅; layer 2 in_progress (commits 2a `9adbc46` + 2b this commit ✅, 2c pending); layers 3–4 pending.** Target is the universal `∀ d → parseText (formatText d) ≡ inj₂ d` (not corpus-shape / per-fixture).  Proof decomposition in PARITY_PLAN.md §B.3.d: (1) `List Char` substrate ✅, (2) per-primitive parse/emit lemmas — Identifier done (2a+2b), remaining primitives (2c) next, (3) per-line-construct lemmas, (4) top-level aggregator induction.
- **Phase B.3.d Layer 2 Commit 2b (this commit, 4 files).**  Ships `parseIdentifier-roundtrip` — the first per-primitive theorem at `List Char` level.  Refactors: `mkIdentFromCharsUnsafe` from `with ... in eq` to `ifᵀ`-based (with-abstraction failed on the `Identifier.valid` dependency; see `feedback_ifT_witness_pattern.md`); `buildIdent` split into `fromMaybeIdent (mkIdentFromCharsUnsafe ...)` for external rewriting.  New `Properties/Primitives.agda` (253 lines): 5 probes + main theorem, composed via `bind-just-step` following `DecRatParse/Properties.parseDecRat-roundtrip-+suc` template.
...
- **Verification:** 760 Python tests + 1 expected-skip; 6 C++ ctest cases; Go tests `-race` clean.  pyright `aletheia/` 0/0/0; pylint `aletheia/` / `tests/` / `conftest.py` all 10.00/10; ctest 6/6; go vet + gofmt clean; clang-format clean on touched files; Agda full build + `check-invariants` (postulates and Unsafe modules limited to allowlisted B.3.d substrate) + `check-ffi-exports` / `check-stdlib-version` / `check-erasure` / `check-properties` all green.  149 Agda modules (+1 from 148 at pre-gate close: `DBC/TextParser/Properties/Substrate/Unsafe.agda` — the layer-1 bridging-axioms module under Option 3a).
