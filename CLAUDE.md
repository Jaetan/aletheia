...
**Cluster Y** (this commit) — **Smart-fallback Rational renderer (cross-binding)** (GO-D-19.1 promoted from cluster R Phase-6 framing per user redirect 2026-05-15). All three pretty-printers now classify on the reduced denominator: terminating decimals (denom divides `2^a · 5^b` after GCD-reduction) keep the existing `:g` / `%g` rendering; non-terminating Rationals emit literal `N/D` from the reduced form. **Per-binding scope**: Go `formatRational` + `gcdInt64` + `absInt64` private helpers added in `enrich.go`, 9 call sites in `formatPredicate` flipped; C++ `format_value(const Rational&)` body in `enrich.cpp` replaced with the smart dispatch (consumer-side `format_predicate` automatically picks up via overload resolution), `<numeric>` and `<cstdlib>` includes added; Python `_format_rational` helper added in `_enrichment.py`, `_format_predicate` flipped from `_coerce_to_float(...):g` to `_format_rational(...)`. **Reduction matters**: Go and C++ Rational direct-construction does NOT enforce lowest-terms; renderer GCD-reduces internally so `Rational{2, 6}` → `"1/3"` not `"2/6"`. Python `Fraction` is always lowest-terms by construction. **15 new tests across 3 bindings** (7 Go + 5 C++ + 4 Python). Wire JSON shape unchanged (`{"numerator": N, "denominator": D}` per R19P2 cluster 17) — display-path-only refinement. No benchmark needed (pretty-printer is off the streaming hot path). CHANGELOG ships the `Changed` entry under `[2.0.0] — Unreleased`. New memory `memory/project_smart_rational_renderer.md` records the design. Module count unchanged at 248 (no Agda touched).
