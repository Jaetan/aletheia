...
**✅ AGENT-MEMORY-CITATION GATE ✅ MERGED 2026-07-16 (#196 + UPD #195).** `tools/check_no_memory_citations.py` (run_ci + pre-commit-fast) fails on 3 structured store-pointer shapes (`[[<prefix>_…]]` / `memory/<name>.md` / `<prefix>_<name>.md`) in every tracked non-binary file — source AND user-facing docs; CHANGELOG/PROJECT_STATUS **gated**, AI-infra (CLAUDE/AGENTS/DEFERRED_ITEMS) **exempt** (the `E2_*` backlog exemption of the time was retired when E.2 closed — no E2_-named files remain). Closes the arc's "extend the citation check to source" open idea. Caught **11** dead pointers on first run (source `.yaml`/`.gitignore` + product docs + CHANGELOG history), all stripped pointer-out-point-kept. [[feedback_no_memory_citations_in_source]] · [[reference_python314_except_syntax]] (PEP 758). Detail: CHANGELOG + git #195/#196.

**✅ GATE-SOUNDNESS + IWYU + COMMENT-QUALITY ARC ✅ MERGED 2026-07-16 (#190–#194).** §G.2 (`DBC.Properties`=proofs / deciders→`DBC.Decidable`; exact cabal-closure gate; 287→290 modules) · gate audit (USER RULE *a gate that can't fail on an issue HAS A BUG; failing IS its function*; 8 gates + 2 hooks could PASS while checking nothing, all fixed + regression tests → [[feedback_gate_pass_is_absence_of_output]]) · §G.1 iwyu (elaboration-baked QName ≠ used import; 20 dead imports the gate called clean, removed; validated vs recompile oracle 20/20 → [[feedback_fixture_matrix_coverage_is_the_gate]]) · comment-quality (14 verified-incorrect comments fixed; plan-scaffolding + memory-store citations stripped from EVERY source language; 85 files → [[feedback_no_memory_citations_in_source]]) · doc tracking-IDs #194. Detail: PROJECT_STATUS + MEMORY.md + git #190–#194.
