...
**R22 A-1.1 batches 7-24 (2026-05-18 → 2026-05-19)** — batches 7-9 landed on `review-r21` pre-merge; batches 10-24 on `review-r22` post-merge.  Per-batch narrative + commit hashes consolidated in `memory/project_review_round21.md`; summary:
...
- **b24** `97a14a6` (5f / ~123c — `Properties/Attributes/Assign/{Node,Signal,Message,EnvVar,Rel}.agda` sibling cluster sharing a near-identical Combinators / DBC.Types / Lexer / Emitter / DecRatParse.Properties / Common import template with one ATgt ctor varying per file: ATgtNode / ATgtSignal / ATgtMessage / ATgtEnvVar / (ATgtNodeMsg + ATgtNodeSig for Rel which uses both arms); kept 2 mixfix FPs per b23 rule — `_×_` in Node L23 + `_>>=_` in Node/Signal/Message/EnvVar L29-34; FN audit of LIVE Combinators names clean post-prune; scanner persisted to `~/.local/bin/scan_aletheia_dead_imports.py` for WSL-resilience).

**Cumulative R21/R22 A-1.1 closure: 110 files / ~1318 candidates** (past original 864 estimate per b21+b22+b23 FN catches).  Scanner v2.6 at `~/.local/bin/scan_aletheia_dead_imports.py` (persistent across WSL restarts; not committed to git per WSL-resilience-via-out-of-repo-PATH rule; rebuild from `feedback_unicode_brackets_scanner.md` + b23 commit body's v2.6 multi-line fix if ever lost).  Known scanner FPs: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉`/`-[1+_]` outside ident class (per-file agda catches); (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` body tokens — context-dependent); (3) `public` re-exports (v2.4 line-skip).  **Known FN classes** (b21-b23): heavy `using (...)` clauses (combinator families, PropEq sets), `import M as N` module aliases (qualified `N.X` not tracked), multi-line `using\n  (` (v2.5 → v2.6 fix), `_X_` mixfix-with-args (`∣_∣`, `ifᵀ_then_else_`, `-[1+_]`, `_≈ᵇ_` — body usage scatters tokens around args), `module X` literal (body uses `X.field` / `open X`), `renaming (X to Y)` clauses (scanner doesn't process at all).  **Workflow**: per-name `grep -cP` audit after scanner pass + `simplify` skill quality-review agent post-prune (catches residual `renaming` + within-using-clause items).

**Gates fresh at `97a14a6`** (R22 A-1.1 batch 24 landing, 2026-05-19): build clean **3m03s** / check-properties clean **9m31s** "All proof modules type-checked successfully" / count-modules **263** (unchanged).  Faster than b23's 14m47s because b24 files are leaf modules deep in the Properties tree — fewer downstream re-checks.  Prior `3e63ca3` gates: build 3m53s / check-properties 14m47s.  Other invariant gates NOT re-run for b24 — only structural changes flip them, none here.  Binding tests NOT re-run — dead-import pruning is MAlonzo-neutral.  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 — high-density tier (R22-original + DecRat + Attributes/Assign/ sibling cluster) all CLEARED.  Top remaining per fresh v2.6 scan post-b24: `Aggregator/Dispatcher/Attribute/Assign.agda` (38; overlaps R22-PRE-CLOSE blocker — distinct file from but sibling-named to the Attributes/Assign.agda stale-re-export file), `Format/AttrDef.agda` (23), `TextParser/Attributes.agda` (22), `Format/Preamble.agda` (21), `Aggregator/Universal.agda` (20), `Format/AttrLine.agda` (20), `Format/AttrValue.agda` (19), then mid-density (17 range) + long-tail (<17).  **AGDA-D-12.1 follow-ups REQUIRED** (re-labeled 2026-05-19 at `d2326a4` per user directive; in-source block at `Adequacy/StreamingWarm.agda:392-406`): per-warning `LogEvent.endstream.uncached_atom` enumerant + cross-binding parity, `check-runbook` entry, PROTOCOL.md warnings-field write-up — three items, must land before next review round's close.  **Pre-close blocker** (b15 side-observation + `74a72fa` in-source marker): 18 pre-existing `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` — stale `using (...)` re-exports of submodule names.  Must be resolved before R22 → main merge — see in-source `R22-PRE-CLOSE` block.  **Push gap**: local `main` 152 commits ahead of `origin/main` — R20+R21 merges held local per `feedback_review_branch_workflow.md`.
