...
**A.2 — `BO_TX_BU_` message senders ✅ DONE 2026-06-11** (branch `agda/e1-isidentstart-hspace-bridge`, 8 commits `c6f9574`→`aed790f` on top of the prior-session lemma phase `8c24c14`/`31a8cb0`).  The verified DBC text round-trip now carries message senders: `parseText (formatText d) ≡ inj₂ d` holds with `BO_TX_BU_` wired as an **8th synthesized top-level section** (VAL_ analogue — lines derived from messages via `collectSenders`, stitched back at `buildDBC` via `attachSenders`, senders OUTER).  `WellFormedTextDBCAgg` is strictly **weaker** (`MessageWF.senders-empty` removed, no new obligation).  **Behavior flip:** `parse_dbc_text` now ATTACHES `BO_TX_BU_` transmitters (keyed by CAN ID) instead of dropping — `kitchen_sink.json` corpus parity snapshot regenerated (shared across Python/Go/C++).  Green-sliced (advisor: bank every hard proof green, atomic wiring LAST): PC-A composition `c6f9574` / PC-B Format DSL `06e3be4` / PC-C parser capture `c0b7c17` / PC-D emitter+slim `eb2f8c5` / PC-E TBO scaffold `c8162df` / atomic 8th-section `e1afc71` / close `aed790f`.  Heap-blowup lesson: never `rewrite` over a parser-bearing goal — `memory/feedback_no_rewrite_over_parser_goal.md`.  Module count 269→**273**.  Full detail: `memory/project_a2_botxbu_senders.md`.  **All Agda gates green** (build/check-properties 52/52/invariants/no-properties-in-runtime/erasure/fidelity/ffi-exports/bound-enforcement); full `run_ci.py` for final cross-binding validation.  E.2 NOT closed (A.2 standalone); E.2 stays HOLD at 5/9.

**`ci/pr-heavy-lanes` ✅ MERGED to main** (PR #16 squash, 2026-06-10; all gates green incl. the advisory mutation lane).  Delivered (a) the **Phase 2 heavy-lanes workflow** (`.github/workflows/pr-heavy-lanes.yml` — parallel repro/stability/mutation lanes; **repro REQUIRED**, stability+mutation **ADVISORY** on `synchronize` with per-job cancel-in-progress) and (b) the **mutation-to-zero campaign**: C++ **0** survivors (Mull 45/45), Python **215→1** (the 1 is a documented genuine equivalent — `dump_json`'s `ensure_ascii=False`→`None`, un-pragma-isolable on a multi-line call-arg), Go **0**.  Baselines pinned in `docs/MUTATION_BENCH.yaml`; the advisory mutation lane is now a regression gate.  No defensive code deleted for the metric (test-the-defense / simplify-redundancy / pragma-keep — `memory/feedback_mutation_no_defense_removal.md`).  Detail: `memory/project_mutation_to_zero.md`.  (Branch `agda/e1-isidentstart-hspace-bridge` was rebased onto `origin/main`; it then delivered A.2 — see the A.2 entry above.  Still unpushed/local; rebase-onto-`origin/main` again before PR-ing if main has moved.)
