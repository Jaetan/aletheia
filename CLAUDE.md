...
**R22 A-1.1 batches 7-22 (2026-05-18)** — batches 7-9 landed on `review-r21` pre-merge; batches 10-22 on `review-r22` post-merge.  Per-batch narrative + commit hashes consolidated in `memory/project_review_round21.md`; summary:
...
- **b22** `3849884` (5f / ~131c — Bridges + Refine + Common + Line + Primitives; b21 FN-audit re-applied catching 7 more FNs: `refl`/`sym`/`cong`/`AttrValue`/`AttrAssign` (Bridges), `_+_` (Primitives), `RawAttrValue` + 5 module aliases `NetworkProofs`-`EnvVarProofs` (Line); 2 scanner FPs retained: `-[1+_]` Common L89 pattern + `⌊_⌋` Primitives L405 lemma per b20 lesson; Line's 15 `parseRawAttrAssign-roundtrip-ATgt*-Rav*` + 5 module aliases were AGDA-D-15.1 split residuals from alt5 dispatchers moved to `Line/Alt5.agda`; net -54 lines).

**Cumulative R21/R22 A-1.1 closure: 100 files / ~967 candidates** (past original 864 estimate per b21 FN lesson).  Scanner v2.5 at `/tmp/scan_dead_imports.py` (no backup — rebuild from `feedback_unicode_brackets_scanner.md` notes if WSL restarts).  Known scanner FPs: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉`/`-[1+_]` outside ident class (per-file agda catches); (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` body tokens — context-dependent); (3) `public` re-exports (v2.4 line-skip).  **Known FN class** (b21+b22): heavy `using (...)` clauses (combinator families, PropEq sets) AND `import M as N` module aliases — scanner doesn't track qualified `N.X` references.  Per-name `grep -cP` audit required.

**Gates fresh at `3849884`** (R22 A-1.1 batch 22 landing, 2026-05-18): build clean **4m37s** / check-properties clean **14m56s** "All proof modules type-checked successfully" / count-modules **263** (unchanged).  b22 timings longer than b19-b21 (1m58-2m10s build, 9m03-10m27s check-properties) because b22 touched deeper-in-the-tree files (proof-side cascade rebuild).  Other invariant gates NOT re-run for b10-b22 — only structural changes flip them, none here.  Binding tests NOT re-run — dead-import pruning is MAlonzo-neutral.  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 — **high-density tier (originally 20+ dead per file) NOW CLEARED** by b22; only `Aggregator/Dispatcher/Attribute/Assign.agda` (28 per v2.5) remains, but it overlaps with the R22-PRE-CLOSE blocker so prune side-by-side with stale-re-export fix.  Mid-density (10-20) and long-tail (<10) tiers need re-scan — v2.5 estimates pre-b22 may be stale.  **AGDA-D-12.1 follow-ups now REQUIRED** (re-labeled 2026-05-19 per user directive; in-source block at `Adequacy/StreamingWarm.agda:392-407`): per-warning `LogEvent.endstream.uncached_atom` enumerant + cross-binding parity, `check-runbook` entry, PROTOCOL.md warnings-field write-up — three items, must land before next review round's close.  **Pre-close blocker** (b15 side-observation + `74a72fa` in-source marker): 18 pre-existing `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` — stale `using (...)` re-exports of submodule names.  Must be resolved before R22 → main merge — see in-source `R22-PRE-CLOSE` block.  **Push gap**: local `main` 152 commits ahead of `origin/main` — R20+R21 merges held local per `feedback_review_branch_workflow.md`.
