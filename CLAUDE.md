...
- **tests/ basedpyright-gate expansion (4, IN PROGRESS)** — user: "make the standard basedpyright gate also cover tests, benchmarks" (benchmarks/ already in `run_ci.py:631` since 2026-05-09 @ 0 errors; only tests/ missing).  tests/ **415 → 386** strict errors: `aabe860` _dbc_helpers · `fba5f68` conftest+_canonical_dbc · `7d0a93b` test_can_log · `7ad8b4a` test_response_parsers.  ~22 leaf files left (biggest: test_dbc_metadata_tier2, test_input_bounds, test_dbc_queries).  **Gate CANNOT flip until tests/=0**, then wire `tests/` into `run_ci.py:631` + update the CLAUDE.md:111 `basedpyright aletheia/` gate doc.  Per-file playbook + remaining-file list in `.session-state.md`.  877 tests green throughout.
...
**CI-speed gate optimization** (post-R23 user directive — *gates must be fast so every prospective contributor runs them, manually + often*; full detail in `memory/project_ci_speed_optimization.md`).  **~28 commits on `ci-speed`** (warm-process work + 2026-05-27 lint campaign + 2026-05-28 review-mark sweep); **NOT yet wired into `run_ci`** (the gate-flip is the one deferred decision).
