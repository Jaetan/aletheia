...
**🦀 RUST PARITY R3b (YAML check loader) ✅ COMPLETE 2026-06-18** (`main` `75d5d4f`, #62) — `load_checks_from_yaml` / `_file` parse a `checks:` YAML doc → typed `Check`s, behind a **default-on** `yaml` cargo feature (default-on so the plain `cargo test` / `clippy --all-targets` CI lane exercises it + matches peers' built-in YAML; opt out via `--no-default-features`). Schema matches Python `load_checks` / Go `LoadChecksFromYAML` (single `checks:` list; simple `signal`+`condition` or causal `when`/`then`+top-level `within_ms`; unknown keys ignored) — proven by a test loading the **shared** `go/aletheia/testdata/doc_examples/checks.yaml` fixture. New `Rational::from_f64` = shared `round(v*1e9),1e9` + gcd-reduce + NaN/Inf/overflow→Err (matches Python/C++ + the fixed Go; reuses core-delegated `to_value`). Trust-boundary hardening (64 MiB cap inline+file, symlink/non-regular rejection) mirrors Go/Python. 1 row → implemented (rust **29/40**). Copilot: 3 hardening fixes. Next: **R3c** (Excel loader crate).
...
**🦀 RUST PARITY R1+R2+R3a ✅ COMPLETE 2026-06-17/18** — R1 typed `Dbc` family + `BA_*` attrs + parse/format/validate (#53/#54/#55); R2 `build_frame`/`update_frame`/`send_frames` + mux extract (#57); R3a fluent `check::signal`/`check::when` DSL → LTL + `add_checks` (#59); doc UPDs #56/#58/#60. Idiomatic divergence from Go/Python where it offers the same feature (no client DBC cache; `impl Into<Rational>`; `u64` ms). Detail: `memory/project_rust_parity_r1.md` + git + CHANGELOG.
