...
197 total modules (`cabal run shake -- count-modules`):
- **192**: `--safe --without-K`
...
3d.4 + JSON-mirror + Path A details: single mega-commit (98 files, +1545/-1314).  Module-flag invariants post-3d.8: **197 modules total** (was 193 post-3d.5.d; +4 new across 3d.6/3d.7/3d.8, all `--safe --without-K`); 192 `--safe --without-K` + 4 `--safe --no-main` + 1 `--without-K`-only (Substrate.Unsafe).  Stream LTL +12-38% across bindings (Path A.5 Bool fast path); Signal Extraction -2-9% / Frame Building -1-7% (Path A structural cost; γ.2 incremental cost folds into the Path A profile, no fresh regression beyond what 320c5a9 already accepted; ε/ζ/η/3d.5.d/3d.6/3d.7/3d.8 are proof-only and runtime-neutral on the streaming hot path — `parseSignalLine`, `parseValueTable`, and `parseMessage` all run at DBC config time only).  Baselines NOT refreshed per user "wait and see" 2026-04-28; COMPILE-pragma escape hatch deferred (now requires explicit user approval per `feedback_no_suppression_without_approval`).  Per-commit detail in PROJECT_STATUS.md and `memory/project_b3d_universal_proof.md`.
...
**Layer 3 status post-3d.8**: BO_ block ✅ closed.  Remaining pending: 3d.5.d migrations of 3a (Preamble) + 3b (BU_, EV_, CM_) + 3c (BA_DEF_ family) onto DSL → Layer 4 (parseDBC aggregator).
