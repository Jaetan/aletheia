...
**R22 A-1.1 batches 7-20 (2026-05-18)** — batches 7-9 landed on `review-r21` pre-merge; batches 10-20 on `review-r22` post-merge. Per-batch index:
...
- `da0b881` b19 (5f / 35c / 0 FPs — Namespace + Refine + TopStmt/Def + TopStmt/Assign + MessageRoundtrip/Base; **manual-audit batch finding far more dead names than scanner v2.4 estimate** — session-state's "3-cand tier" framing was a scanner artifact; new lesson `feedback_grep_E_word_boundary.md` — `grep -E '\b'` matches backspace not word boundary, was inflating usage counts via hyphenated-identifier substring matches) → **`dfa6937` b20 (5f / 42c / 1 FP — DecRat + Aggregator/Dispatcher/Attribute/Default + Topology/Message + Aggregator/Foundations + Aggregator/Universal; ⌊_⌋ Unicode-bracket mixfix FP caught by per-file agda's `NotInScope: ⌊`; scanner v2.5 rebuilt from scratch at `/tmp/scan_dead_imports.py` after WSL restart wiped v2.4 — new lesson `feedback_unicode_brackets_scanner.md`).**

**Cumulative R21/R22 A-1.1 closure: 90 files, ~763 of 864 candidates (~88%)**. Scanner v2.5 at `/tmp/scan_dead_imports.py` (no backup — rebuild from `feedback_unicode_brackets_scanner.md` notes if WSL restarts again).  v2.5 is more aggressive than v2.4: multi-line `using (...)` parse, `-syntax` allowlist, `public`-line skip.  Known FPs: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉` outside `[A-Za-z0-9_'\-]` ident class (per-file agda catches); (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` ↔ token-bounded `if` in body); (3) `public` re-exports — addressed in v2.4/v2.5 by line-skip.  **The session-state's "3-cand tier" projection (~178 remaining) was a v2.4 underestimate** — v2.5 reveals 39 dead in `Properties/Attributes/Common.agda` alone, 28 each in `Line.agda` / `Aggregator/Dispatcher/Attribute/Assign.agda`, 27 in `Refine.agda` (post-b19), etc.

**Gates fresh at `dfa6937`** (R22 A-1.1 batch 20 landing, 2026-05-18): build clean **2m10s** / check-properties clean **10m27s** All proof modules type-checked successfully / count-modules **263** (unchanged).  Recent batch gate timings: b20 2m10s+10m27s, b19 2m01s+9m03s, b18 3m44s+12m16s, b17 2m03s+10m01s, b16 3m38s+10m53s, b15 3m56s+12m30s.  Build-time variance ~2-4m driven by which subtree was edited.  Other invariant gates NOT re-run for batches 10-20 — they only flip on structural changes, none here.  Binding tests (pytest / Go / C++) NOT re-run — dead-import pruning is MAlonzo-neutral.  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 — ~101 candidates / ~53 files remaining per scanner v2.5.  Top remaining files by density (post-b20): `Properties/Attributes/Common.agda` (39 dead), `Attributes/Line.agda` (28), `Aggregator/Dispatcher/Attribute/Assign.agda` (28), `Aggregator/Refine.agda` (27 still — only b19's 6 pruned), `Aggregator/Dispatcher/Attribute/Assign/Bridges.agda` (24), `Properties/Primitives.agda` (23), `Aggregator/Universal.agda` (8 still — only b20's 13 pruned), Network/BareInt/Frac/String per-target submodules.  Current efficiency: 5 files × 8-12 candidates / ~12-15 min build + 10-12 min check-properties.  Optional AGDA-D-12.1 follow-ups still documented in `StreamingWarm.agda:394-409` DEFER block — none blocking.  **Pre-close blocker** (added 2026-05-18, b15 side-observation + `74a72fa` in-source marker): 18 pre-existing `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` — stale `using (...)` re-exports of submodule names.  Must be resolved before R22 → main merge — see in-source `R22-PRE-CLOSE` block + `.session-state.md` Pre-close checklist.  **Push gap**: local `main` is 152 commits ahead of `origin/main` — R20 + R21 merges not yet pushed; user-gated per `feedback_review_branch_workflow.md`.
