...
**🎗️ C.2 DEC₀ REIFICATION ✅ MERGED 2026-07-24 (#243 `133a8910`; kernel-only, runtime-neutral).** Hot-path Bool predicates are self-certifying: new `Aletheia.Data.Dec0` (+ `Dec0/Rational`, `Dec0/Nat`) packages each fast-path Bool with an ERASED `Reflects` certificate; MAlonzo compiles Dec₀ to a newtype over Bool — pinned by a new `check-erasure` guard. Reified (most previously uncertified): ℚ atom comparators · `finalizesHolds/Fails` · `inBounds` · `canIdEquals` · mux selector match · bit membership/intersection + overlap family (`HasPairOverlap`) · `_≟cs₀_`. Zero proof churn (every Bool = definitional projection of its twin; all proof modules unchanged); bench-ratified identical ×4. Constraint-(b) sites keep relevant proofs deliberately (`_≡ᵇ-proc_`, `findSignal*`, `mkBoundedBitVec` — reasoning in-source; do-not-revisit map in `memory/project_c2_dec0.md`). Detail: CHANGELOG + git #243.

**📨 H.2 DETAILED EXTRACTION REASONS ✅ MERGED 2026-07-23 (#240 `84072fd7`; BREAKING binary wire, folds into pending MAJOR).** The binary extraction wire now carries the kernel's detailed error reason for every extraction error, ×4 bindings — generic per-code tables retired. Wire = Arrow-style offsets table (header gains `u32 reasonBytes`; `(nErrors+1)×u32` cumulative offsets + UTF-8 blob between errors and absent; O(1) segment starts, validation-as-a-phase). u8 code space 4→8, one distinct code per distinguishable error (`InContext` recurses). Machine-checked: `reason-parity` (binary ≡ JSON reasons via shared kernel formatter) + `extractionErrorCodeToℕ-injective` in new `CAN/Batch/Properties/ReasonParity.agda`. Per-binding real-FFI parity tests assert EXACT reason equality; fidelity pins an exact reason string e2e. Perf within WSL2 band ×4 (Python/Go zero-error fast path; bench caught a +18% Python p50). Lessons: [[feedback_wire_capability_over_yagni]] · defensive-bound review = ask what it defends (`feedback_be_the_primary_reviewer` refined). Detail: `memory/project_h2_detailed_reasons.md` + CHANGELOG + git #240.
...
**⛩️ GEOMETRY REFUSE-AT-ENTRY ✅ MERGED 2026-07-21 (#222 `eba7b8bc`; J.1+E.7+E.8 closed, BREAKING JSON wire).** Frame-capacity bounds (DLC decides classic vs FD; shared decider family `DBC.Decidable.SignalGeometry` across entry gates/validator/checker) · refuse-at-entry both routes (mod clamps + monus relocation retired; typed errors name SUBMITTED values) · BE no-wrap re-aimed pre-conversion (Motorola layouts load; closure under emission regression-tested) · E.7 deadness theorem (`DBC.Properties.GeometryGateDeadness`) · E.8 resolved by consumption. Detail: CHANGELOG + git #222.

**🔬 E.3 tightness + validator mirrors ✅ MERGED 2026-07-20 (#218-#220).** Proven tightness verdicts (`WellFormedCheck.agda` header); CHECK 24/25 on shared deciders. Detail: `memory/project_e3_tightness.md` + git #218-#220.

**📦 Distribution hardening ✅ COMPLETE 2026-07-19 (#213-#216; Phase 6 item 1 closed).** Dist-staging gate · SBOM coverage gate · bundle validation runs the installers' recipes · nfpm `.deb`/`.rpm` + keyless-signed GHCR image. First tag-triggered release = remaining full-path acceptance; then the one-time GHCR visibility flip (RELEASE.md). Detail: PROJECT_STATUS § Phase 6 + git #213-#216.
