...
**🔬 r25 ADVERSARIAL 4-BINDING API REVIEW — wire-fix arc COMPLETE; remaining B6e/B7/B8** (spec `.archive/reviews/r25/FINAL_report.md`; cadence = focused PRs, each SHA-verified green). **Shipped #80-117**: B1-B4 + Dependabot + the 3-point sequence (Point 1 cargo-doc #103; Point 2 renderer-vocal ×4 #104-107; Point 3 render-delegation ×4 #109-110, exact rationals via kernel `formatℚ`) + throughput **Lever A** #111 (the `mutation testing` lane is diff-scoped per binding, fails SAFE to the full run; verified `diff-scope → [go]`) + **B5b** decode-lockstep #113 + the **B6a–d wire-float arc** #114-117: **B6a** Rust logging parity (extraction events + doc-truth), **B6b** Go `copyMessage` genuine deep copy, **B6c** Go rational-exact wire decode (`UseNumber`), **B6d** Python+C++ reject floats on the internal wire (`decode_wire_rational` / `require_int`+`require_uint`; `parse_rational` retired) — **all 4 bindings now reject wire floats in lockstep**. **THE FLOAT PRINCIPLE (user, 2026-06-27)**: no float anywhere internal — internal OR UI-input; a rational degrades to float ONLY at print-out; a decimal is a `DecRat` (exact, denom 2^a·5^b), never a `float64`. **B6e** (design LOCKED → `memory/project_b6e_decrat_ffi.md`): kill the UI float-input converters too — expose Agda `DecRat` over FFI as the **uniform** SSOT for decimal→rational (all bindings delegate, incl. Python/Go); API takes `int | Rational`; delete the 4× `float_to_rational`/`from_double` heuristics; overflow caught in the Haskell shim (`toIntegralSized`). **Remaining**: B6e (impl), B7 comment-truth, B8 dead-code/DRY. **Standing**: attribution (commits `Co-Authored-By: Claude Opus 4.8`, PR Claude Code footer — `memory/feedback_commit_workflow_signed.md`), Lever B pipeline-PRs, the dribble, ultracode (workflows for substantive tasks — drove the B6d audit + adversarial-verify that caught the `dbc_normalize`-is-wire-not-UI blocker). Detail: `memory/project_r25_binding_review.md` + `memory/project_b6e_decrat_ffi.md` + `memory/project_mutation_to_zero.md` + CHANGELOG + git #80-117.
