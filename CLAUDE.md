...
**🧪 Mutation-lane arcs ✅ MERGED 2026-09-20/21 (#274-#278).** L.1 C++ survivors worked to a ledger of zero, L.11 index loops, L.12 type deduction (#274, [[project_cpp_mutation_surface]]) · N.1 the C++ lane as two legs and a merge, every budget a measurement (#275) · N.2 three slices per tree behind ccache keyed on the plugin and the Mull config (#276) · N.5 a lane pays only for what the diff can move (#278): `reproducible-build` and the stability bench live in `pr-build-lanes.yml` under benchmark.yml's documentation filter, the two lanes no required check reports, and each mutation lane asks `tools/mutation_scope.py` (the runner's own scope) before installing its toolchain. USER RULE reaffirmed, *a gate that cannot fail on an issue has a bug*: `check_mutation_setup` now refuses a test the mutated tree cannot satisfy. Detail: CHANGELOG [Unreleased] + [[project_shipped_arcs_index]] + git #274-#278.
...
**🏷️ v4.0.0 RELEASED 2026-07-17 (#198/#200/#201/#204, tag `v4.0.0`).** MAJOR (breaking `format_dbc_text` ×4). First self-contained multi-binding bundle: `shake dist` bundles all four wrappers + `env.*`/`install.*` + LICENSE; a `v*` tag keyless-signs (Sigstore, ambient OIDC, no key in CI), self-verifies against the workflow identity and publishes a DRAFT Release; consumer-verified end to end on a clean py3.14. Detail: PROJECT_STATUS § Phase 6 + `memory/project_r26_product_gaps.md` (release record) + git.
