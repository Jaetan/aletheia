...
**🎯 VALIDATE FIRST-TOUCH PROGRAM ✅ + r25 CLOSE + V2a ✅ 2026-07-04/05 (#147–#153)** — seven PRs. **#147** B6-logging doc-truth (PROTOCOL/LOG_EVENTS/FEATURE_MATRIX now four-binding-aware; closed r25 B6-logging — its code items had already shipped); **#148** B8 DRY sweep (cpp `parse_issue_entry` + `std::to_array` ×3 incl. the REALIZED drift: `error_code_table` declared 59/real 57, two phantom `{"",Unknown}` entries; Go `parseIssueArray`/`DispatchThen`/KeepAlive; Rust `Rational::le`/`set_extended`/`select_diag_values`); **#149** B8 efficiency (Go single-marshal `serializeDBC`→RawMessage with the probe retained + alloc-free cache-hit key; cpp excel moves; Rust EOS seen-set; Go+C++ CLI double-parse removal) → **r25 = CLOSED** (PR D/E follow-ups queued). **#150+#152 the validate first-touch fix (user HIGH PRIORITY: "the first command users run")**: kernel envelope carries the FULL issue list (warnings were silently dropped) + `TrailingInput` user-facing wording + `AttributeRefinementFailed` typed cause+name (`refineAttribute` Maybe→cause-sum; Refine.agda lemmas just→inj₂ bodies unchanged; Universal/B.3.d zero edits); 4 typed binding errors (code-gated lift; Python converter delegates to the SSOT helper after the mutation lane + Copilot independently caught the un-gated lift → [[feedback_unkillable_mutant_is_design_signal]]); 3 CLIs render "Validation FAILED" + numbered list exit 1 both modes; **CLI scenario harness** (`docs/CLI_SCENARIOS.yaml` + `test_cli_parity.py`, 40 tests, own run_ci step in the cpp lane, mutmut-ignored); matrix row `validation_rejection_issues` (44 rows). **#153 V2a** (Position+CharClass leaf modules; Error/ResponseFormat/Message closures Combinators-free; modules 275→277) preps **V2b — the B.3.c.k override** (user-ratified: furthest-failure merge + watermark-through-successes on the spike-proven E2 pair encoding → `memory/project_v2_parser_position_design.md`). Process rules earned: [[feedback_stacked_pr_retarget_before_parent_merge]] (cost #151), [[feedback_never_unsigned_commits]] (commit-tree resign surgery repaired #152). **Queue (user 2026-07-05): V2b → PR D (EOS reshape) → PR E (Rust symbol cache).** Detail: memory files + CHANGELOG + git #147-#153.
