...
189 total modules (`cabal run shake -- count-modules`):
- **184**: `--safe --without-K`
...
188 of 189 modules use `--safe`. No modules require `--sized-types`. 3d.5 Format DSL (2026-04-27) added 2 modules (`DBC/TextParser/Format.agda` + `DBC/TextParser/Format/ValueTable.agda`), both `--safe --without-K`. 3d.5.c-γ (2026-04-28) added 3 more (`DBC/CanonicalReceivers.agda` + `DBC/TextParser/Format/Receivers.agda` + `DBC/TextParser/Format/Receivers/Roundtrip.agda`), all `--safe --without-K`. Path A.4 (3d.4 + JSON-mirror, 2026-04-27) lifted the prior 47-module `--without-K`-only cluster to `--safe --without-K` by retyping `Identifier.name`, JSON `JString`, DBC AST text fields, and LTL signal names from `String` to `List Char`.
...
**Status (2026-04-28):** Layers 1–2 ✅; Layer 3 through 3d.3 ✅; **3d.4 + JSON-mirror + Path A ✅ shipped 2026-04-28 (commit `320c5a9` on branch `b3d-3d4-json-detaint`)**; **3d.5.a framework core + 3d.5.b parseValueTable gate + 3d.5.c-γ.1+γ.2+γ.3 ✅ shipped on branch `b3d-3d5-format-dsl`**.
...
3d.4 + JSON-mirror + Path A details: single mega-commit (98 files, +1545/-1314).  Module-flag invariants post-γ.3: **189 modules total** (was 186; +3 for CanonicalReceivers + Format/Receivers + Format/Receivers/Roundtrip, all `--safe --without-K`); 184 `--safe --without-K` + 4 `--safe --no-main` + 1 `--without-K`-only (Substrate.Unsafe).  Stream LTL +12-38% across bindings (Path A.5 Bool fast path); Signal Extraction -2-9% / Frame Building -1-7% (Path A structural cost; γ.2 incremental cost folds into the Path A profile, no fresh regression beyond what 320c5a9 already accepted).  Baselines NOT refreshed per user "wait and see" 2026-04-28; COMPILE-pragma escape hatch deferred (now requires explicit user approval per `feedback_no_suppression_without_approval`).  Per-commit detail in PROJECT_STATUS.md and `memory/project_b3d_universal_proof.md`.
...
2. **3d.5 Format DSL framework (~4–6w):** sub-phases (a) framework core ✅; (b) single-construct validation gate (parseValueTable) ✅; (c) pinch-point extensions in_progress — α `refined` constructor ✅, β `altSum` + `withPrefix` ✅, γ.1 `CanonicalReceivers` refinement carrier ✅, γ.2 AST retype + cascade ✅, γ.3 receivers DSL roundtrip ✅; **δ pending — 3d.3 dispatcher migration (3 dispatchers: NotMux, IsMux, SelBy/BothMux variants — spike one first per advisor)**; (d) migration of remaining 3a–3d.3 proofs onto DSL (~2-3w); (e) apply to renumbered 3d.6–3d.8 (~1w); (f) Layer 4 aggregation (~3-5d).
