...
**R22 A-1.1 batches 7-21 (2026-05-18)** — batches 7-9 landed on `review-r21` pre-merge; batches 10-21 on `review-r22` post-merge. Per-batch index:

- `df6e5e0` b7 (5f / ~50c / 2 FPs: `ifᵀ` multi-line + `All` ctor) → `e8eb147` b8 (5f / ~56c / 1 SignalPresence operator slip) → `1c0786b` b9 (5f / ~50c / scanner `module X` improvement) → `2aa58b2` b10 (5f / 62c — Protocol/Handlers + Format/AttrDef + Format/ValueDescription + Format/SignalGroup + SimplifySound/Decomposition; new lesson `feedback_qualified_access_empty_using.md`) → `4e0f12c` b11 (5f / 38of39c — Topology/SignalLine + Aggregator/Dispatcher/.../Assign + JSONParser/MessageWF + Formatter/MessageRoundtrip + LTL/Semantics/MTL; 1 FP `if_then_else_`) → `28aeece` b12 (5f / 35c / 0 FPs — Format/Preamble + Format/AttrValue + MessageRoundtrip/Standard + MessageRoundtrip/Extended + SignalExtraction/Properties).
- `d4fdb8a` b13 (5f / 31c / 0 FPs — CrossOrder + Routing + JSON/Properties + FrameProcessor/Step + StreamingWarm) → `a3d9c12` b14 (5f / 29c / **1 scanner FP** = Lexer.`parseNatural` `public` re-export consumed by 14 downstream files; scanner upgraded to v2.4 to skip `public` lines; new lesson `feedback_public_reexport_blindspot.md`) → `afd5ea7` b15 (5f / 28c / 0 FPs — MetadataRoundtrip + Formatter + DecRat/Refinement + TopStmt/Default + SignalLine/Roundtrip; side-observation logged: 18 pre-existing `ModuleDoesntExport` warnings in Assign.agda from sub-module renames) → `783fe09` b16 (5f / 23c / 0 FPs — EnvVar + RationalRenderer/Properties + JSONParser/Properties + JSON/Parse + ManyRoundtrip) → `bfe09b5` b17 (5f / 20c / 0 FPs — PrefixHead + Dispatcher + Receivers/Roundtrip + Comments + StartBit; **all 4-candidate files exhausted**) → `4e02d27` b18 (5f / 15c / 0 FPs — Limits + Validity/Theorem + ValueTables/ValueTable + Preamble/Newline + Batch/Completeness; first batch into 3-candidate tier).
- `da0b881` b19 (5f / 35c / 0 FPs — Namespace + Refine + TopStmt/Def + TopStmt/Assign + MessageRoundtrip/Base; **manual-audit batch finding far more dead names than scanner v2.4 estimate** — session-state's "3-cand tier" framing was a scanner artifact; new lesson `feedback_grep_E_word_boundary.md` — `grep -E '\b'` matches backspace not word boundary, was inflating usage counts via hyphenated-identifier substring matches) → `dfa6937` b20 (5f / 42c / 1 FP — DecRat + Aggregator/Dispatcher/Attribute/Default + Topology/Message + Aggregator/Foundations + Aggregator/Universal; ⌊_⌋ Unicode-bracket mixfix FP caught by per-file agda's `NotInScope: ⌊`; scanner v2.5 rebuilt from scratch at `/tmp/scan_dead_imports.py` after WSL restart wiped v2.4 — new lesson `feedback_unicode_brackets_scanner.md`) → **`3acda42` b21 (5f / ~73c — Foundations + BareInt + Frac + String + Network; Network.agda standout at 30 net = 15 scanner-confirmed + 15 FN-found via per-name `grep -cP` audit on heavy `using (...)` clauses; 1 FP `-[1+_]` Unicode-bracket mixfix in pattern position; `_≈ᵇ_` converted to qualified-access-via-empty-using; new lesson `feedback_scanner_fn_audit_heavy_imports.md` — when a `using` clause has 10+ names and scanner flags ≤ 30%, per-name grep audit is required).**

**Cumulative R21/R22 A-1.1 closure: 95 files, ~836 of 864 candidates (~97%)**. Scanner v2.5 at `/tmp/scan_dead_imports.py` (no backup — rebuild from `feedback_unicode_brackets_scanner.md` notes if WSL restarts again).  v2.5 is more aggressive than v2.4: multi-line `using (...)` parse, `-syntax` allowlist, `public`-line skip.  Known FPs: (1) Unicode-bracket mixfix `⌊_⌋`/`⌈_⌉` outside `[A-Za-z0-9_'\-]` ident class (per-file agda catches; b21 hit on `-[1+_]` in pattern position); (2) mixfix-with-token-collision (`+_` vs `++`, `if_then_else_` ↔ token-bounded `if` in body); (3) `public` re-exports — addressed in v2.4/v2.5 by line-skip.  **Known FN class** (new b21): heavy `using (...)` clauses with 10+ names from combinator families leak dead names past v2.5 — Network.agda's 15 names from `Aletheia.Parser.Combinators` had only 1 scanner-flagged but 11 actually dead.  Per-name `grep -cP` audit required.

**Gates fresh at `3acda42`** (R22 A-1.1 batch 21 landing, 2026-05-18): build clean **1m58s** / check-properties clean **9m30s** All proof modules type-checked successfully / count-modules **263** (unchanged).  Recent batch gate timings: b21 1m58s+9m30s, b20 2m10s+10m27s, b19 2m01s+9m03s, b18 3m44s+12m16s, b17 2m03s+10m01s, b16 3m38s+10m53s.  Build-time variance ~2-4m driven by which subtree was edited.  Other invariant gates NOT re-run for batches 10-21 — they only flip on structural changes, none here.  Binding tests (pytest / Go / C++) NOT re-run — dead-import pruning is MAlonzo-neutral.  Prior `5e01ce3` benchmark posture (R20 R6-B7.4 baseline) remains the last refreshed empirical baseline.

**Next**: continue AGDA-A-1.1 — ~28 candidates remaining per scanner v2.5 estimate, but b21 demonstrated heavy-import FN rate is significant; actual remaining likely higher post-per-name-audit.  Top remaining files by density (post-b21): `Properties/Attributes/Common.agda` (39 scanner-flagged), `Attributes/Line.agda` (28), `Aggregator/Dispatcher/Attribute/Assign.agda` (28 — overlaps with R22-PRE-CLOSE blocker file), `Aggregator/Refine.agda` (27 still — only b19's 6 pruned), `Aggregator/Dispatcher/Attribute/Assign/Bridges.agda` (24), `Properties/Primitives.agda` (23).  Current efficiency: 5 files × 10-30 candidates / ~10-12 min build + 9-10 min check-properties.  Optional AGDA-D-12.1 follow-ups still documented in `StreamingWarm.agda:394-409` DEFER block — none blocking.  **Pre-close blocker** (added 2026-05-18, b15 side-observation + `74a72fa` in-source marker): 18 pre-existing `ModuleDoesntExport` warnings in `DBC/TextParser/Properties/Attributes/Assign.agda:35-65` — stale `using (...)` re-exports of submodule names.  Must be resolved before R22 → main merge — see in-source `R22-PRE-CLOSE` block + `.session-state.md` Pre-close checklist.  **Push gap**: local `main` is 152 commits ahead of `origin/main` — R20 + R21 merges not yet pushed; user-gated per `feedback_review_branch_workflow.md`.
