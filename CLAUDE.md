...
**AGDA-A-1.1 status: ✅ COMPLETE** — tool-driven sweep #4 (`56ac3d6`, 2026-05-22, 15h wall clock) removed **532 dead names across 90 files** with 0 baseline failures and project-wide `check-properties` PASS.  Race-free via topological-level batching (commit `b1d3d34`); 3-tier safety net (local check + warning-count gate + consumer re-check) validated every removal.  Two prior tool-driven sweeps had earlier failure modes diagnosed and fixed: sweep #2 unsafe (reverted, `PatternShadowsConstructor` + `--include-public` bugs fixed in `06d2ac1` / `df58123`); sweep #3 partial (273 removals; 81 files race-skipped, retry mitigation in `5788a12`).  Sweep #4 supersedes both as the canonical post-prune state.  Top facade finds: `Main.agda` 92/106, `Properties/Attributes.agda` 91/91 (100%), `FrameProcessor/Properties.agda` 69/116, `TextParser/Properties.agda` 38/38 (100%) — three facades alone accounted for 41% of removals.  Combined with manual b25 series (~605 dead): **~1100+ total dead names removed in the R22 campaign**.

**Sanity-gate infrastructure** (commits `403555b` + `d503d75` + `2c01026`): pre-commit hook calls `tools/scan_dead_imports.py` (regex, advisory only; FP suppression via committed `tools/scan_dead_imports.ignore` — 312 entries / 146 files post-sweep-#4); pre-push step 9 of `tools/run_ci.py` calls `tools/prune_unused_imports.py --check-only --no-topo` on `git diff main...HEAD` files (strict, blocks push; 30-file sanity threshold with `ALETHEIA_PRUNE_GATE_NOLIMIT` override).  `BASE_STEPS` 29 → 30.  Exit codes now match gofmt / prettier convention (dry-run findings → exit 1).  Fresh-clone docs updated to surface `install_hooks.py` (BUILDING.md step 8 + CONTRIBUTING.md sub-section + CI_LOCAL.md refresh).  All dev tools in-repo at `tools/` only (`~/.local/bin/` shadow copies retired in `13a62fb` per repo-standalone directive).

**Next pre-close items**: (1) `Assign.agda` 18 `ModuleDoesntExport` warnings; (2) AGDA-D-12.1 REQUIRED follow-ups (per-warning enumerant + cross-binding parity + check-runbook + PROTOCOL.md, in-source block at `Adequacy/StreamingWarm.agda:392-406`); (3) review-process meta-review (user directive 2026-05-20; detail in `memory/feedback_review_process_revisit.md`).  **Push gap**: local `main` 160+ commits ahead of `origin/main` — R20+R21 merges + R22 work held local per `feedback_review_branch_workflow.md`.
