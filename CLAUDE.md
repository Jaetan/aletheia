...
**🎯 V2b BYTE-EXACT PARSER POSITIONS ✅ + SHAKE-INSTALL / FRESHNESS GATE ✅ 2026-07-05 (#155/#156)** — two PRs. **#156 V2b** (task #105 ✅): byte-exact furthest-failure parser positions on the **E2 watermark encoding** (`Parser A = Position → List Char → Position × Maybe (ParseResult A)` — the failure watermark rides proj₁; proj₂ discards stuck `maxₚ` terms definitionally so pre-existing proof pins still close by `refl`; `<|>` both-fail merges via lexicographic `maxₚ`, `many` swallows element failures but KEEPS their watermark). Ten combinator primitives reshaped; **~100 proof modules restated to the `proj₂` outcome level** via agent waves (parser-result rewrites → simultaneous `with`-abstractions; Bool-level rewrites stay), ZERO agent failures; positioned runtime errors flow out (`ParseFailure : Position`, `TrailingInput : Position → Position` = watermark+stmt-start, `InvalidJSON : Position`; ResponseFormat extras emit line/column + statement_line/column). Bindings green **ZERO edits** (py/go/cpp/rust + parity 40×3; new byte-exact pin: `midfile_bad` L40 C22 = the corrupted DLC byte). B.3.d universal roundtrip unchanged; Copilot per-char `advancePosition` bind-once fix folded; MAlonzo ordinal `_70→_72` (sed in shim+fidelity-test+snapshot); modules stay 277. Paired bench (DBC-text COLD path only): 200-msg +3.2%, 1000-msg +5.1% — May ledger was 9× stale; hot streaming path untouched. **#155**: `shake install` fixed (Shakefile `cmd` word-split the `python -c` program string → SyntaxError, dead since 2026-02-08 before it could write `_install_config.py`; fixed by passing `-c` as a list element) + a **deployed-kernel freshness gate** (`tools/check_install_freshness.py`: GNU build-id identity when both `.so`s carry one, else SHA-256; a gitignored `.install-receipt` records prefix/library/config so the gate reads them instead of guessing PREFIX — user: "you don't know how users install it"); Copilot joint `_same_build` fix folded. Process rule earned: [[feedback_conflict_blocks_pull_request_ci]] (a CONFLICTING PR silently blocks its `pull_request` heavy CI — resolve by merging main IN, before expecting gates to run). **Queue (user 2026-07-06): CLI single-parse #106 (patch-ready) → #107 (kernel dual-route factor + C2 bounds) → PR D (EOS reshape) → PR E (Rust symbol cache).** Detail: [[project_v2_parser_position_design]] + CHANGELOG + git #155/#156.

**🎯 VALIDATE FIRST-TOUCH ✅ + r25 CLOSE + V2a ✅ 2026-07-04/05 (#147–#153)** — `aletheia validate` on a broken DBC now lists the full issue set (errors AND warnings — warnings used to be dropped) and exits 1 across all 3 CLIs, locked by a 40-test cross-CLI scenario harness (`docs/CLI_SCENARIOS.yaml` + `test_cli_parity.py`). Kernel #150: structured `issues` envelope + positioned parse errors + typed `AttributeRefinementFailed` cause+name (Refine.agda lemma restatements only; B.3.d unchanged). Bindings #152: 4 typed validation errors (code-gated lift; Python delegates to the SSOT helper). Closed r25 (#147 B6-logging, #148 B8 DRY incl. the phantom `error_code_table` drift, #149 B8 efficiency). #153 V2a: Position+CharClass leaf modules, modules 275→277. Rules earned: [[feedback_stacked_pr_retarget_before_parent_merge]] (cost #151), [[feedback_never_unsigned_commits]]. Detail: [[project_validate_cli_first_touch]] + [[project_cli_integration_harness]] + CHANGELOG + git #147-#153.
