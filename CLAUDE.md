...
**Active branch (2026-05-09): `review-r18`** — R18 review round in flight. Forked from `main` at `b4528ee`; live commit count via `git rev-list --count b4528ee..HEAD`. 17 review agents → ~610 findings → 17 clusters ranked FIX-early / FIX-middle / DEFER-end-of-round per `memory/feedback_review_round_dispositions.md`. **Rounds 1-10 + clusters 1, 2, 3, 4, 5, 6, 7 closed**; **110 hard findings closed cumulatively** (cluster 7 contributes 3: PY-B-14g.1, GO-B-14.7, CPP-B-14.8 — mutation-testing infrastructure across all 3 bindings + CLI overhaul of `tools/run_ci.py`; +1 from cluster 7 second follow-up `32932bd` — `violations.bench_cli` W0632 unbalanced-unpack real-bug fix surfaced by promoting `python/benchmarks/` into the pylint gate).  **1 DEFER-end cluster open**: cluster 17 (CANId proof-field irrelevance migration).  **End-of-round follow-up**: promote `python/benchmarks/` into the basedpyright gate (~325 errors; same asymmetry the pylint side just closed; recorded in `memory/project_review_round18.md` § End-of-round follow-ups).  Per-cluster commit hashes + scope detail + gate logs live in `memory/project_review_round18.md` and PROJECT_STATUS.md.
...
**End-of-round follow-up deferred** (recorded in `memory/project_review_round18.md` § End-of-round follow-ups + TaskCreate task #9): promote `python/benchmarks/` into the **basedpyright** gate — `basedpyright benchmarks/` reports 325 errors (mostly `reportUnknownArgumentType` / `reportUnknownVariableType` from loose `dict` types: `DBCDefinition` vs raw dict, `Signal` builder return types, etc.).  Substantially heavier than the pylint side (~30-90 min/file × 8 files); closed at end-of-round per user direction 2026-05-09.
