...
- **tests/ basedpyright-gate expansion (IN PROGRESS)** — bring each tests/ file to ruff+pylint+basedpyright-0, then wire `tests/` into `run_ci.py:631` (gate flips only at tests/=0).  tests/ basedpyright **415 → 219**: shared helpers + 2 leaf (`aabe860`/`fba5f68`/`7d0a93b`/`7ad8b4a`), then the **ruff-clean DBC batch (2026-05-30)** — tier2 `456fbfa` / queries `1009245` / format_dbc `2a6cff3` / validator `e92bc95` (+promoted `SignalOverrides`) / tier1 `65ee942`.  ruff now in venv + `[dev]` (`1736554`).  **tests/ pylint = 10.00** via `ddbd10b`: adopted `[lint.isort] known-first-party=["aletheia"]` (resolves I001↔C0411 at root — ruff classified aletheia as 3rd-party; reorder 38 files; gated `aletheia/`+`tools/` stay 0) + extracted `mux_signal()` / new `tests/_stream_helpers.py` (de-dup R0801) + relocated `run_one_frame_stream` out of conftest (C0411) + hoisted a local import (C0415).  **LESSON**: per-file pylint MISSES cross-file R0801 — run FULL `pylint tests/` after pattern-shared edits.  **(2026-05-30 later) 7 files cleaned → 219→180 bp** (one commit each, full clean): `d74137b` fuzz via NEW `python/stubs/atheris/__init__.pyi` (drops `# type: ignore`, keeps `# pylint: disable=import-error` for the opt-in extra) + `62f0d0d`/`9be269d` lazy_import/private_import + `391ea01` property_hypothesis + `7e81aa6` backend_protocol (amended) + `ef6b221` cancellation + `00f109e` logging.  **Scope = "fully clean per file" incl. mark-strip (user-confirmed via AskUserQuestion).**  **8 left (~180 bp)**: types_and_conditions·14, batch·13, cross_binding·12, classify_satisfied·20, unified_client·24, canfd_mux·23, default_checks·34, input_bounds·48(+marks).  **NEW LESSONS** (in [[feedback_ruff_all_lint_campaign]]): inline DBC literals → `_dbc_helpers` builders (`ruff format` expands them → cross-file R0801; builders R0801-safe — bit me twice); run FULL `pylint tests/` per file (caught a 10.00→9.99 regression, recovered by amending backend); in-module fixture `@pytest.fixture(name=x) def x_handler()` (non-`_` name ≠ param clears bp reportUnusedFunction + pylint W0621).  877 green.  Playbook + detail: `.session-state.md` + `memory/project_ci_speed_optimization.md`.
...
**CI-speed gate optimization** (post-R23 user directive — *gates must be fast so every prospective contributor runs them, manually + often*; full detail in `memory/project_ci_speed_optimization.md`).  **~35 commits on `ci-speed`** (warm-process work + 2026-05-27 lint campaign + 2026-05-28 review-mark sweep + 2026-05-30 tests/ DBC bp-batch + known-first-party isort / tests/ pylint 10.00); **NOT yet wired into `run_ci`** (the gate-flip is the one deferred decision).
...
- **Roadmap**: (lint) `python/aletheia/` (root + `asyncio/` + `client/`) ✅ ruff(ALL) + mark-strip CLEAN; next dirs `python/tests/` (big batch), `python/benchmarks/`, `examples/`, then Agda `src/`, Go `go/`, C++ `cpp/`.  Open: the 4 pre-existing `aletheia/` R0801s (kw-arg `send_frame`, `FrameResult` build, 2× `__all__`) need a dedicated multi-file dedupe commit.  Then **wire tools/ into the gate** (separate `pylint tools/` step + tools/ in basedpyright + ruff steps).  (warm) wire `run_ci` (batch authoritative until weeks of agreement), land 5 confirmed dead imports (`++ₗ-assoc`/`concatMap`/`-_`/`sj`/`suc-injective`), steps 10-30; then merge `ci-speed`→main.
