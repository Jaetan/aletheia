...
266 total modules (`cabal run shake -- count-modules`):
- **261**: `--safe --without-K`
...
**Active branch: `ci-speed`** — forked from `main` at `f6025e8` (post-R23 merge, 2026-05-26).  Module count **266** — two new Agda modules this branch: the renderer-proof linchpin `DecRat/RationalSoundness.agda` (264→265) and `DBC/RationalRenderer/Faithful.agda` (265→266) holding the renderer faithfulness theorem (`Represents (formatℚ-chars q) q`) AND the canonical-shape theorems (reduced/trimmed/branch); prior CI-speed work was tooling (Python/Shakefile).  Live commit count: `git rev-list --count main..HEAD` per `feedback_self_referential_count_drift.md`.

**2026-06-08 — item 4 renderer-proof ✅ + IWYU gate unbroken & made accurate + dead-import cleanup (8 commits; detail `memory/project_rational_renderer_proof.md` + `memory/project_iwyu_reader_resolution.md`)**: **Renderer proof** — `ef51df1` linchpin (`toℚ-fromℚ?-sound`) → `ec5ab3d` faithfulness (`Faithful.formatℚ-chars-represents : ∀ q → Represents (formatℚ-chars q) q`; module 265→266) → `2c219ab` canonical SHAPE (`formatℚ-fraction-form`/`decimal-form`/`emitMagnitude-shape`: reduced `↥q/↧ₙq` + trimmed + decimal-vs-fraction branch, via linchpin + `fromℚ?-after-toℚ` roundtrip; constant 18 = unpinned design param) — faithfulness pins VALUE not spelling, so shape pinned separately ([[feedback_proof_pins_value_not_shape]]); step (c) collapsed the triplicated per-binding value tables to FFI-plumbing-only (NO golden — user chose "strengthen proof instead").  **IWYU gate** — the `.agdai` reader had never run (`_warm.py` committed py2 `except A, B:`); unbroke it (`bdc08cb`) and that surfaced **4 reader resolution gaps**, all "resolve the real key, don't reconstruct `modS.name`" (`bdc08cb` 3 + `9b41722` 4th): renamed-import-erased-from-signature (alias-site count, driver passes pos), re-exported record member (value QName), re-exported module `module ≡-Reasoning` (`amodName`), member from a LOCAL module alias (resolve `modS` via consumer scope).  self-test 31→35 fixtures (each gap got a reproducing fixture).  `e3ce2b3` removed the 2 genuine dead imports the now-accurate gate found.  All gates green; `ci-speed` NOT merged.  **NEXT = branch/PR-hygiene → merge.**

**2026-06-07 — pre-merge items 1–3 DONE (9 commits; full detail `memory/project_ci_speed_optimization.md` + [[project_object_sweep]] + [[project_opaque_type_sweep]])**: item 1 namespace review (`06343c0`→`1287459`: `aletheia` sole public pkg; `dbc`/`codes` unified; `protocols`→`types`; Tier 2=A; over-aggressive dsl/loaders demotion REVERTED) · item 2 Fraction-pipeline (`2db0b68` drop `RationalNumber`→int at parse, restores Go/C++ parity; `4d66870` `JSONValue` + covariant `Mapping` loaders, no casts via `dict.update()` widening; Chunk C no-op — output FFI-formatted) · item 3 opaque-sweep (`69f7999`+`48017dc`: tree already clean; `tuple[str,str,str]`→`PrivateImport` NamedTuple etc.).  All gates green per commit.
