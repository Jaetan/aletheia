...
**Active branch: `ci-speed`** — forked from `main` at `f6025e8` (post-R23 merge, 2026-05-26).  Module count **266** — two new Agda modules this branch: the renderer-proof linchpin `DecRat/RationalSoundness.agda` (264→265) and the full renderer-faithfulness theorem `DBC/RationalRenderer/Faithful.agda` (265→266, `Represents (formatℚ-chars q) q`); prior CI-speed work was tooling (Python/Shakefile).  Live commit count: `git rev-list --count main..HEAD` per `feedback_self_referential_count_drift.md`.
