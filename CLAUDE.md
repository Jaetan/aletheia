...
186 total modules (`cabal run shake -- count-modules`):
- **181**: `--safe --without-K`
...
185 of 186 modules use `--safe`. No modules require `--sized-types`. 3d.5 Format DSL (2026-04-27) added 2 modules (`DBC/TextParser/Format.agda` + `DBC/TextParser/Format/ValueTable.agda`), both `--safe --without-K`. Path A.4 (3d.4 + JSON-mirror, 2026-04-27) lifted the prior 47-module `--without-K`-only cluster to `--safe --without-K` by retyping `Identifier.name`, JSON `JString`, DBC AST text fields, and LTL signal names from `String` to `List Char`.
...
**Status (2026-04-27):** Layers 1–2 ✅; Layer 3 through 3d.3 ✅; **3d.4 + JSON-mirror + Path A ✅ shipped 2026-04-28 (commit `320c5a9` on branch `b3d-3d4-json-detaint`)**; **3d.5.a framework core + 3d.5.b parseValueTable gate ✅ shipped on branch `b3d-3d5-format-dsl`**.
...
3d.4 + JSON-mirror + Path A details: single mega-commit (98 files, +1545/-1314).  Module-flag invariants post-3d.5.b: **186 modules total** (was 184; +2 for Format/Format.ValueTable, both `--safe --without-K`); 181 `--safe --without-K` + 4 `--safe --no-main` + 1 `--without-K`-only (Substrate.Unsafe).  Stream LTL +9-39% across bindings (cache hot path); Signal Extraction / Frame Building -3-9% (Path A structural cost: `String → List Char` representation in MAlonzo `Data.Text` → `[Char]`).  Baselines NOT refreshed per user "wait and see" 2026-04-28; COMPILE-pragma escape hatch deferred (now requires explicit user approval per `feedback_no_suppression_without_approval`).  Per-commit detail in PROJECT_STATUS.md and `memory/project_b3d_universal_proof.md`.
...
2. **3d.5 Format DSL framework (~4–6w):** sub-phases (a) framework core ✅; (b) single-construct validation gate (parseValueTable) ✅; (c) pinch-point extensions pending (`caseFmt` for sum-type dispatch, `iso` refinement for `T (predicateᵇ value)` records, asymmetric strip for `parseReceiverList∘strip`); (d) migration of existing 3a–3d.3 proofs onto DSL (~2-3w); (e) apply to renumbered 3d.6–3d.8 (~1w); (f) Layer 4 aggregation (~3-5d).
