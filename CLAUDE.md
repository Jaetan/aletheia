...
**Current Status**: ✅ All 141 Agda modules use `--safe --without-K`

### Module Safety Flag Breakdown

**By flag combination** (141 total):
- **137 modules**: `--safe --without-K` (standard safe modules)
- **4 modules**: `--safe --without-K --no-main` (Main.agda, Main/JSON.agda, Main/Binary.agda, Parser/Combinators.agda)

**All 141 modules use `--safe`**. No modules require `--sized-types`.
...
- `--safe` ensures no undefined behavior (like Rust's borrow checker)
  ...
  - Used in all 141 Aletheia modules
