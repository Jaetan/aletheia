...
**Next: Phase B.3.d Layer 2.** Per-primitive parse/emit lemmas at `List Char` level, fully `--safe`.  DecRat-side already closed via `fromℚ?-after-toℚ`; remaining: `showℕ-dec-chars`, identifier emitters, string-literal emitters (`quoteStringLit-chars`), enum-tag emitters (ByteOrder, Signed/Unsigned, SignalPresence, attribute-type tags).
...
- **Phase B.3.d status** — **pre-gate ✅ complete; layer 1 ✅ complete (this commit); layers 2–4 ⏳ in_progress.** Target is the universal `∀ d → parseText (formatText d) ≡ inj₂ d` (not corpus-shape / per-fixture).  Proof decomposition in PARITY_PLAN.md §B.3.d: (1) `List Char` substrate ✅ (2 bridging axioms in Substrate.Unsafe + formatter/parser refactor), (2) per-primitive parse/emit lemmas (next), (3) per-line-construct lemmas, (4) top-level aggregator induction.
