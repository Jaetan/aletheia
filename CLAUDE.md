...
**Status (2026-04-29):** Layers 1–2 ✅; Layer 3 through 3d.3 ✅; **3d.4 + JSON-mirror + Path A ✅ shipped 2026-04-28 (commit `320c5a9` on branch `b3d-3d4-json-detaint`)**; **3d.5.a framework core + 3d.5.b parseValueTable gate + 3d.5.c-γ.1+γ.2+γ.3+ε+ζ+η + 3d.5.d parseValueTable production migration ✅ shipped on branch `b3d-3d5-format-dsl`**; **3d.6 + 3d.7 + 3d.8 ✅ shipped 2026-04-29 (commits `89e04ee` + `42228df` + `f25ca5a` on same branch)** — full BO_ block roundtrip closes Layer 3 BO_ block; **3d.5.d-BU_ ✅ shipped 2026-04-29 (commit `abc7d0f` same branch)** — 73% strict-LOC reduction; **3d.5.d-EV_ ✅ shipped 2026-04-29 (commit `21a3146` same branch)** — 94% strict-LOC reduction; **3d.5.d-CM_ ✅ shipped 2026-04-29 (commit `7c27100` same branch)** — 88.6% strict-LOC reduction (heaviest Layer-3 construct, 2,533L pre-migration); **3d.5.d-3a Preamble ✅ shipped 2026-04-29 (commit `c5b34fb` same branch)** — 76% combined production-side reduction across VERSION/BS_/NS_ (Namespace alone 91%, was 864-file-LOC bind-chain); **3d.5.d-3c-A ✅ shipped 2026-04-29 (commit `27aaa8c` same branch)** — 56% combined / 82% Properties-side reduction across BA_DEF_/BA_DEF_REL_ (Type.agda deleted entirely; Def.agda 1155→355 strict-LOC).
...
3d.5.d-CM_ (`7c27100`): production migration of `parseComment` (CM_ comment line, heaviest Layer-3 construct at 2,533 file-LOC pre-migration) onto Format DSL.  η-style: `parseComment = parse commentFmt >>= λ result → many parseNewline >>= λ _ → buildCommentP (proj₁ result) (proj₁ (proj₂ result))`.  New module `Format/Comments.agda` (581 file-LOC, 328 strict-LOC): `commentFmt : Format (RawCommentTarget × List Char × ⊤)` via `withPrefix "CM_"` + `withWS` + 4-deep left-associated `altSum` over `(((buNodeFmt <|> boMsgFmt) <|> sgSigFmt) <|> evVarFmt) <|> netFmt` (5-way `RawCommentTarget` dispatch with `netFmt = literal []` for CTNetwork) + `stringLit` + `wsOpt;";"` + `newlineFmt`.  **Raw-ADT-in-Format pattern** (3d.8 `messageHeaderFmt` precedent, generalised here to a 5-constructor ADT): `RawCommentTarget` 5-constructor mirrors `CommentTarget` but carries raw ℕ for CAN-IDs; wrapper `buildCommentP` lifts via `buildCANId`, failing on out-of-range IDs — preserves pre-3d.5.d backtrack-then-fail semantics (an out-of-range CAN-ID under the old parser caused `parseStringLit`-after-backtrack to fail on the leftover keyword via `<|>`'s reset-on-nothing semantics; under the migrated wrapper, `buildCommentP` returns `nothing` directly).  altSum disjointness for the 4 inj₂ branches closes by `(λ _ → refl)` (closed-Char head comparison through `parseCharsSeq`: every keyword leaf's `string` matcher fails on input prefixed with `'"'` from `quoteStringLit-chars`).  `Properties/Comments/Comment.agda` 1,901 → 216 strict-LOC = **88.6% production-side reduction** (file-LOC 2,533 → 363 = 85.7% reduction; combined Format + Properties + TextParser = 580 strict-LOC).  **Bridge `emit-commentFmt-eq-emitComment-chars`** per-target case-split: CTNetwork by `refl`; CTNode/CTMessage/CTEnvVar each via one `shift-trail-space = ++ₗ-assoc xs (' ' ∷ []) ys` step wrapped in a closed-prefix `cong` lambda; CTSignal via two ++ₗ-assoc steps — bridges DSL's left-associated `(name ++ ' ' ∷ []) ++ rest` to formatter's right-associated `name ++ ' ' ∷ rest` form.  **`shift-trail-space` ++ₗ-assoc bridge pattern** (load-bearing for future migrations): when a per-target leaf in a Format pair-tower bakes in a trailing space (`withWSAfter`), the DSL produces `(emit f a ++ ' ' ∷ []) ++ rest` which doesn't reduce by `refl` against the formatter's `emit f a ++ ' ' ∷ rest`.  One ++ₗ-assoc step per offending leaf, wrapped in a closed-prefix cong, bridges the gap.  Slim `parseComment-roundtrip` via 3-step `bind-just-step` chain (parse-line success via `parseComment-format-roundtrip` → `manyHelper-parseNewline-exhaust` → `buildCommentP-roundtrip` 5-way dispatch).  CAN-ID cases use `with buildCANId (rawCanIdℕ cid) | buildCANId-rawCanIdℕ cid ; just .cid | refl = refl` for K-elim avoidance.  `buildCANId-rawCanIdℕ` retained from 3d.3b: extended-CANId case uses **pointwise `subst`** (not `rewrite`) per the documented 3d.3b heap-blowup root cause (`memory/project_b3d_cm_heap_blowup.md`).  `CommentTargetStop` precondition migrates upstream to `Format/Comments`; Properties re-exports for source compatibility.  Module count 199 → 200 (+ Format/Comments; `--safe --without-K`).  `cabal run shake -- check-properties` 11m53s clean ("All proof modules type-checked successfully!"); agda Main.agda clean; `cabal run shake -- build` 1m49s clean.  No semantic shift on streaming hot path — `parseComment` runs at DBC config time only.

3d.5.d-3a Preamble (`c5b34fb`): production migration of `parseVersion` / `parseBitTiming` / `parseNamespace` (the three preamble constructs VERSION / BS_ / NS_) onto the Format DSL — fifth 3d.5.d construct group to land, all three under one PR via η-style wrap: `parseVersion = parse versionFmt >>= many parseNewline >>= pure`, `parseBitTiming = parse bitTimingFmt >>= many parseNewline >>= pure tt`, `parseNamespace = parse nsFmt >>= pure tt` (NS_'s body Format already captures the trailing blank line via discard-iso over `many nsLineFmt`).  New module `Format/Preamble.agda` (~670 file-LOC, **418 strict-LOC**) houses the three formats + universal `parseVersion-format-roundtrip` / `parseBitTiming-format-roundtrip` / `parseNamespace-format-roundtrip` lemmas + bridges (`emit-versionFmt-eq-emitVersion-chars-prefix` / `emit-bitTimingFmt-eq-emitBitTiming-chars-prefix` / `emit-nsFmt-eq-emitNamespace-chars`).  NS_ uses the **discard-iso pattern** over `many nsLineFmt`: inner pair tower's underlying carrier is `⊤ × List NSLineShape`, the iso projects to `⊤` for the user-facing API while emitting the canonical 26-element body (25 specific keyword identifiers + 1 trailing blank line).  `NSLineShape = ns-blank | ns-keyword Identifier`; one body line dispatches via `altSum (blankLineFmt) (keywordLineFmt)`.  Termination certificate `parse-nsLineFmt-fail-on-stop` supplied for the `[]-fails` constructor of `EmitsOKMany` — closes by char-class disjointness on the `c` head of an `isNSLineStart c = isHSpace c ∨ isNewlineStart c`-stopped suffix (`∨-false-split` → both branches fail definitionally).  The 25 canonical keyword IDs are constructed via `mkKwId : (kw : String) → {valid : T (validIdentifierᵇ (toList kw))} → Identifier` using the load-bearing reduction canary from pre-3d.5.d's `_Scratch.agda` (typechecks iff `validIdentifierᵇ` reduces to `true` on closed keyword strings).  **Format DSL extensions** (`Format.agda`): (a) **`wsCanonTab`** — canonical tab emit (`'\t' ∷ []`), parser one-or-more isHSpace; mirror of `ws` for tab-indented lines; used by NS_ keyword-line indent.  (b) **`nonNewlineRun`** — empty canonical emit (`[]`), parser zero-or-more non-newline; used by BS_ opaque-tail consumer (formatter writes `BS_:\n\n`; parser consumes any non-newline tail before the LF).  **Spec preservation via `withWSCanonOne` (advisor-caught spec drift, fixed pre-commit)**: the NS_ header `:` separator slot uses the new sugar combinator `withWSCanonOne` (parser zero-or-more, canonical single space) — matching the OLD `parseNamespace` accepting `"NS_:..."` (no whitespace required).  An initial `withWS` (one-or-more, mandatory) draft was caught by the advisor as a silent spec narrowing — `nsFmt` would have rejected `"NS_:..."` while old code accepted it.  Type-equivalent fix (both EmitsOK obligations are `SuffixStops isHSpace`; both bridges close by `refl`).  Lesson captured in `feedback_format_dsl_ws_family_discipline.md`: when migrating a parser to Format DSL, audit each whitespace slot for permissiveness — the four `withWS*` family members (`withWS`/`withWSOpt`/`withWSCanonOne`/`withWSCanonTab`) have distinct parser semantics that silently change parser permissiveness (bridge proofs only catch emit shape, not parse acceptance).  **Migrated modules**: `Properties/Preamble/Version.agda` 166 → 79 strict-LOC = 52% reduction; `Properties/Preamble/BitTiming.agda` 161 → 80 strict-LOC = 50%; `Properties/Preamble/Namespace.agda` 570 → 52 strict-LOC = **91% reduction** (heaviest 3a, was hand-written 864-file-LOC bind-chain through 25-keyword inductive `manyHelper-parseNSLine-body` + per-branch `parseNSLine-blank` / `parseNSLine-keyword` lemmas + length bounds).  Combined: 897 → 211 strict-LOC = **76% production-side reduction**.  Helpers added: `Lexer.isNonNewline : Char → Bool` (dual of `isNewlineStart`); `Primitives.parseWS-one-tab` (mirror of `parseWS-one-space` for tab-led input); `Primitives.map-nothing` (`_<$>_` analogue of `bind-nothing`, used by `parse-blankLine-fail-on-stop`'s `<|>` fall-through case in the termination certificate).  **Where-clause vs module-level helpers** (load-bearing): initial draft had `nsLineFmt`'s iso forward/backward functions as where-bound; under elaboration these did NOT reduce definitionally at consumer call sites, blocking `EmitsOK-keyword-line` (the goal stayed `Identifier.name kw != Identifier.name kw ++ emit (withWSOpt newlineFmt) tt`).  Fix: hoist `ns-line-fwd` / `ns-line-bwd` / `ns-line-inv` to module-level `private` definitions.  Same fix applied to `versionFmt` and `keywordLineFmt`'s pair-projection helpers (replaced with inline `proj₁` / `proj₂` lambdas).  **EmitsOK shape mismatch via `++`-association**: `emit (withWSOpt newlineFmt) tt` stays abstract under Agda elaboration, so the goal `(Identifier.name kw ++ emit (withWSOpt newlineFmt) tt) ++ rest` doesn't reduce against a precondition of shape `SuffixStops isHSpace (Identifier.name kw ++ '\n' ∷ rest)`.  Fix: change precondition shape to `SuffixStops isHSpace ((Identifier.name kw ++ '\n' ∷ []) ++ rest)` in three places (`EmitsOK-keyword-line`, `build-EmitsOKMany-canonical-keywords`, `ns-keyword-ids-head-stops`) — generalises to any future migration where a Format leaf has an abstract emit that needs to bridge a precondition.  Module count 200 → 201 (+ Format/Preamble; `--safe --without-K`).  `cabal run shake -- check-properties` 7m32s clean ("All proof modules type-checked successfully!"); agda Main.agda clean; `cabal run shake -- build` 1m52s clean.  No semantic shift on streaming hot path — `parseVersion` / `parseBitTiming` / `parseNamespace` all run at DBC config time only.

3d.5.d-3c-A (`27aaa8c`): production migration of `parseAttrDef` (BA_DEF_, attribute-definition for standard scopes Network/Node/Message/Signal/EnvVar) and `parseAttrDefRel` (BA_DEF_REL_, relational scopes NodeMsg/NodeSig) onto the Format DSL — sixth 3d.5.d construct group; first half of 3c (BA_DEF_ family ~6,000L combined; 3c-B for Default + Assign/* + Line follows).  η-style: `parseAttrDef = parse attrDefFmt >>= λ result → many parseNewline >>= λ _ → pure (liftStdAttrDef result)`; `parseAttrDefRel` mirror with `liftRelAttrDef`.  New module `Format/AttrDef.agda` (851 file-LOC, **503 strict-LOC**): `attrTypeFmt : Format RawAttrType` (5-way `altSum` for INT/FLOAT/STRING/HEX/ENUM; `RawAttrType` carries refinement payloads — `IntDecRat` for INT/HEX bounds, `DecRat` for FLOAT, cons-required `List Identifier` for ENUM); `stdScopeFmt : Format RawStdScope` (5-way `altSum` Network/Node/Message/Signal/EnvVar); `relScopeFmt : Format RawRelScope` (2-way NodeMsg/NodeSig); `attrDefFmt : Format (RawStdScope × Identifier × RawAttrType × ⊤)` (10-tuple, std scopes bake trailing space via `withPrefix "BU_" ws` etc); `attrDefRelFmt : Format (RawRelScope × Identifier × RawAttrType × ⊤)` (11-tuple, **rel scopes use explicit `withWS` between scope keyword and string-lit** — mirrors parser's explicit `parseWS`; std-scope keywords bake the trailing space, rel-scope keywords don't).  Universal `parseAttrDef-format-roundtrip` / `parseAttrDefRel-format-roundtrip` one-liners over `roundtrip` calls + `build-emits-ok` builders with **concrete-shape EmitsOK preconditions** (bake `(' ' ∷ rest)` / `(quoteStringLit-chars name ++ inner) ++ outer-suffix` into signature so SuffixStops witnesses close by `∷-stop refl`).  **Format DSL extensions** (`Format.agda`): (a) **`intDecRat : Format IntDecRat`** — emit `showInt-chars ∘ intDecRatToℤ`, parse `parseIntDecRat`; EmitsOK requires `SuffixStops isDigit suffix × ('.' ≢ headOr suffix '_')` to disambiguate from DecRat.  (b) **`natDecRat : Format NatDecRat`** — analogous, parser `parseNatDecRat`.  Both used for INT/HEX/ENUM bounds slots in `attrTypeFmt`.  **Cycle break** (`Attributes/Foundations.agda`, NEW, 57L/24 strict): extract 4 cycle-relevant scope parsers (`parseStandardScope`, `parseRelScope`, `parseOptionalStandardScope`, `parseStringType`) into a Foundations module to break Attributes → Format → Properties.Primitives → Attributes (mirror of 3d.5.c-ε's Topology split).  `Properties/Attributes/Type.agda` **DELETED entirely** (was 1177 file-LOC / 869 strict-LOC) — per-tag `parseAttrTypeDecl-roundtrip-AT*` lemmas subsumed by universal `parseAttrTypeDecl-format-roundtrip`.  `Properties/Attributes/Def.agda` REWRITTEN (1428 → 462 file-LOC, **1155 → 355 strict-LOC = 69% reduction**): slim wrappers; bridges via **5-case dispatch (per scope)** using `cong (λ z → ...) (emit-attrTypeFmt-eq-emitAttrType-chars rt)` instead of brittle 25-case enumeration (5 scopes × 5 attr-types).  **`concatMap-foldr-bridge`** structural induction lemma for the ENUM case (stdlib's `concatMap = concat ∘ map` doesn't fold to inline `foldr` definitionally).  **Orphan drop in `Properties/Primitives.agda`** (1144 → 983 file-LOC, 161 lines): post-3c-A the per-tag scope-tag / rel-scope / ATString roundtrips have zero internal callers — dropped `parseOptionalStandardScope-AS{Node,Message,Signal,EnvVar,Network}-roundtrip`, `parseRelScopeWS` helper + 2 roundtrips, `parseStringType-roundtrip`; corresponding re-exports + unused `AttrScope`/`AttrType` imports also removed.  **Properties-side reduction**: 1155 + 869 → 355 + 0 = **2024 → 355 strict-LOC = 82%**.  **Combined** (with new Format DSL cost): 2024 → 355 + 503 + 24 = **882 strict-LOC = 56%**.  Module count 201 → 202 (+ Format/AttrDef + Attributes/Foundations; - Properties/Attributes/Type.agda; net +1; all changes `--safe --without-K`).  `cabal run shake -- check-properties` 15m30s clean ("All proof modules type-checked successfully!"); agda Main.agda clean; `cabal run shake -- build` 1m48s clean.  No semantic shift on streaming hot path — `parseAttrDef` / `parseAttrDefRel` run at DBC config time only.

3d.4 + JSON-mirror + Path A details: single mega-commit (98 files, +1545/-1314).  Module-flag invariants post-3d.5.d-3c-A: **202 modules total** (was 201 post-3d.5.d-3a Preamble; net +1 — added Format/AttrDef + Attributes/Foundations, deleted Properties/Attributes/Type.agda); 197 `--safe --without-K` + 4 `--safe --no-main` + 1 `--without-K`-only (Substrate.Unsafe).  Stream LTL +12-38% across bindings (Path A.5 Bool fast path); Signal Extraction -2-9% / Frame Building -1-7% (Path A structural cost; γ.2 incremental cost folds into the Path A profile, no fresh regression beyond what 320c5a9 already accepted; ε/ζ/η/3d.5.d/3d.6/3d.7/3d.8/3d.5.d-BU_/3d.5.d-EV_/3d.5.d-CM_/3d.5.d-3a/3d.5.d-3c-A are proof-only and runtime-neutral on the streaming hot path — `parseSignalLine`, `parseValueTable`, `parseMessage`, `parseBU`, `parseEnvVar`, `parseComment`, `parseVersion`, `parseBitTiming`, `parseNamespace`, `parseAttrDef`, and `parseAttrDefRel` all run at DBC config time only).  Baselines NOT refreshed per user "wait and see" 2026-04-28; COMPILE-pragma escape hatch deferred (now requires explicit user approval per `feedback_no_suppression_without_approval`).  Per-commit detail in PROJECT_STATUS.md and `memory/project_b3d_universal_proof.md`.
