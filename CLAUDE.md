...
**📦 DISTRIBUTION HARDENING ✅ COMPLETE 2026-07-19 (#213 `10f3f547` · #214 `d92fe6b1` · #215 `6b1770eb` · #216 `37843e5a`; PROJECT_STATUS Phase 6 item 1 closed).** Four slices, each with empirically proven teeth: **always-on staging gate** `check-dist-staging` (Shakefile stageBinding pathspecs must resolve incl. `:(exclude)` inner globs — git archive passes dead excludes SILENTLY; packaging scripts tracked + `bash -n`/`fish -n`, fish installed in CI, FAST tier) · **SBOM bills the bundled binding sources** (manifest parsers over the STAGED tree, zero external SBOM tooling; `check_sbom_coverage` fails dist on any missing dependency + always-on `--parse-only` rot tripwire; go.sum `h1:` dirhash rides a property, never a SHA-256 hashes entry) · **real-workflow bundle validation** (`bundle_validate` executes the installers' own captured recipes; one consumer per compiled binding through a real LTL scenario; gates publish; first `schedule:` workflow re-validates weekly — HEAD dist + published-Release replay; `--self-test` corruption teeth, all-skipped = COULD-NOT-CHECK) · **native packages + GHCR image** (nfpm one-manifest `.deb`/`.rpm`, payload = the bundle at `/opt/aletheia` byte-identity-proven; rpm deps on the libgmp SONAME (resolves across rpm distros); self-verify loop over EVERY signed artifact; Dockerfile bundle/worktree skew fixed + BuildKit verify stages — image cannot build unless every binding builds against its own bytes; digest keyless-signed; user's one-time GHCR visibility flip documented). First tag-triggered release = the full-path acceptance test. Acceptance run of `bundle-validation.yml` green both jobs (dispatch 2026-07-19). Detail: PROJECT_STATUS § Phase 6 + CHANGELOG + git #213-#216.

**🔒 WIRE-CODE SSOT + COMPLETENESS GATES ✅ MERGED 2026-07-19 (#211 `862d3840`; DEFERRED_ITEMS §G.3+§H.2 closed).** `docs/WIRE_CODES.yaml` pins the kernel's two wire vocabularies (validation issue codes + error codes); `tools/check_wire_codes.py` (run_ci step `check-wire-codes`) anchors YAML↔kernel formatter arms (reciprocal set equality + declaration order, fail-closed on vacuous input); per-binding completeness tests ×4 anchor each binding's vocabulary to the YAML (Rust: named-variant decode + lossless `Error::Core` passthrough, typed lifts SSOT-checked); `test_error_code_sync.py` retired. H.2: Rust `IssueCode`/`Error` now `#[non_exhaustive]` (BREAKING Rust-only; `Unknown(String)` stays as the version-skew channel; `as_str()` skew idiom documented; `IssueSeverity`/`Verdict`/LTL AST stay exhaustive by design). **USER RULE 2026-07-19**: prose about code carries no hard-coded counts and no line-number references — describe structure qualitatively or name the SSOT; measurements (benchmarks, run evidence), spec constants, versions/dates, and machine-re-derived numbers keep their digits. `memory/feedback_self_referential_count_drift.md` (rewritten) + AGENTS/docs.md cat 14. Detail: CHANGELOG + DEFERRED_ITEMS §G.3/§H.2 + git #211.
