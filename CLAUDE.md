...
**Latest (2026-04-21):** Phase A (feature matrix + structural gates) + Phase B.1 (DBC metadata Tier 1) + Phase B.1.x (DBC metadata Tier 2 + signal receivers + message senders) landed, plus pylint-gate hardening. Work follows the [cross-binding parity roadmap](docs/development/PARITY_PLAN.md) locked after R17.
...
- **Verification:** 738 Python tests + 1 expected-skip + 111 markdown-docs fence runs; 292 C++ tests across 5 runtime ctest binaries + `static_tests`; 240 Go tests `-race` clean; **1270 total**. pyright `aletheia/` 0/0/0; pylint `aletheia/` / `tests/` / `conftest.py` all 10.00/10; ctest 6/6; go vet + gofmt clean on both `go/aletheia` and `go/excel` modules; clang-format + clang-tidy clean on B.1.x-touched files; Agda full build + `check-ffi-exports` / `check-stdlib-version` / `check-erasure` all green.
