...
- `[x]` ✅ CLOSED cluster 3 — **UR-3.1** `tools/check_reproducible_build.py` runs two clean `cabal run shake -- build` invocations, compares sha256 of `build/libaletheia-ffi.so`, fails with diagnostic on drift.  Same-host repro empirically verified (sha256 `e095fb1c...` × 2, ~6.5 min wall).  `--test-fail` flag exercises the failure path in seconds (forward-revert verified).  Opt-in into `tools/run_ci.py` via `ALETHEIA_REPRO_CHECK=1`.
- `[x]` ✅ CLOSED cluster 3 — **UR-3.2** Shake `dist` rule records pre-strip + post-strip SHA-256 in `dist/aletheia/MANIFEST.txt`, generates CycloneDX 1.5 SBOM at `dist/aletheia/aletheia-sbom.cdx.json`, signs tarball at `dist/aletheia.tar.gz.sig` (cosign keypair, `keys/cosign.pub` committed), produces sidecar `dist/aletheia.tar.gz.sha256`, and includes the GHC libHS*.so version manifest inline in MANIFEST.txt + the SBOM.  Tarball-level repro verified (`sha256 24b0142c...` × 2 dist runs).
- `[x]` ✅ CLOSED cluster 3 — **UR-3.3** `-ffile-prefix-map=${CMAKE_SOURCE_DIR}=.` added to `aletheia-cpp` library target; `--ghc-options=-optc-ffile-prefix-map=$REPO_ROOT=.` injected by the Shake cabal build invocation.  Defense-in-depth — empirical check showed the .so already had no source-path leakage (only GHC libdir, which patchelf rewrites to `$ORIGIN`), but the flags prevent regression if anyone enables `-g` debug info downstream.
...
- `[ ]` **CICD-1.3** `tools/check_action_pins.py` missing.
- `[ ]` **CICD-1.4** `tools/check_workflow_permissions.py` missing.
...
- `[x]` ✅ CLOSED cluster 3 — **CICD-2.1** dist rule now caches the `find -name '*.so*'` result once into `soFiles` and reuses it for patchelf + strip + per-file hashing.
- `[ ]` **CICD-3.1** No `permissions:` precedent set; `Shakefile.hs phony "install-python"` `pip3 install -e .` would inherit ambient `GITHUB_TOKEN` scope when run under CI.  DEFER — `install-python` is a developer convenience target, not a CI-invoked one.  Track for follow-up if/when CI ever runs install paths.
- `[x]` ✅ CLOSED cluster 3 — **CICD-4.1** dist rule now records `sourceHash` (pre-copy) and `copyHash` (post-copy) for `libaletheia-ffi.so` and asserts equality before continuing.  Hash-verify step exists between unpackaged build artifact and packaged dist.
- `[x]` ✅ CLOSED cluster 3 — **CICD-5.1** `docs/development/RELEASE.md` covers the cat-5 checklist (release flow, sign+verify, reproducible-build verification, key rotation, troubleshooting).  Cross-references `DISTRIBUTION.md` for the integration-side concerns.
- `[x]` ✅ CLOSED cluster 3 — **CICD-5.2** SHA-256 recorded in `dist/aletheia/MANIFEST.txt` (pre-strip + post-strip + each shipped libHS*.so) plus sidecar `dist/aletheia.tar.gz.sha256` consumable by `sha256sum -c`.
- `[x]` ✅ CLOSED cluster 3 — **CICD-5.3** CycloneDX 1.5 SBOM at `dist/aletheia/aletheia-sbom.cdx.json` enumerates toolchain (GHC + cabal + Agda + stdlib) + main artifact + each GHC runtime dep with sha256.  Generator at `tools/sbom_generate.py` (no external `syft`/`cyclonedx-cli` dep).
- `[x]` ✅ CLOSED cluster 3 — **CICD-5.4** Tarball signed at `dist/aletheia.tar.gz.sig` via cosign keypair (`keys/cosign.pub` committed; `~/.config/aletheia/cosign.key` private).  Skip-with-warning when cosign / key missing.  Default `--tlog-upload=false` for dev iteration; `ALETHEIA_COSIGN_TLOG=1` enables Rekor for releases.
- `[x]` ✅ CLOSED cluster 3 follow-up — **CICD-5.5** `Dockerfile.runtime` base image now pinned by SHA-256 digest (`python:3.13-slim@sha256:a0779d7c12fc20be6ec6b4ddc901a4fd7657b8a6bc9def9d3fde89ed5efe0a3d`, OCI multi-arch index resolved 2026-05-08).  `phony "docker"` builds with both `aletheia:latest` (moving) AND `aletheia:<git-short-sha>` (immutable, derived from `git rev-parse --short HEAD`); consumers can pin to a specific commit's image.  Refresh procedure documented in `Dockerfile.runtime`.
- `[x]` ✅ CLOSED cluster 3 — **CICD-5.6** `dist/aletheia/MANIFEST.txt` captures GHC + cabal + Agda + stdlib pin alongside the per-file hashes.  In-tarball; survives extraction.
- `[x]` ✅ CLOSED cluster 3 follow-up — **CICD-5.7** `phony "dist"` now writes `dist/aletheia/README.txt` before tar invocation.  Content: artifact entry-point — file inventory + quick-start gcc command + verify-then-trust order (cosign sig → sha256sum -c → MANIFEST.txt hashes) + cross-refs to `DISTRIBUTION.md`/`RELEASE.md` + git commit + commit-time build-date.  Deterministic content (no wall-clock) so it doesn't break tarball reproducibility.
...
- `[x]` ✅ CLOSED cluster 6 — AGDA-A-16.1 `benchmarks/stability/<short-sha>/` directory created + populated by `tools/stability_run.py` per `ALETHEIA_STABILITY_CHECK=1` lane.  Layout: `python.json` / `go.json` / `cpp.json` per-binding verdicts, `aletheia-ffi.hp` heap profile, `summary.json`.
- `[x]` ✅ CLOSED cluster 6 — AGDA-A-16.2 `+RTS -hT` capture wired via `ALETHEIA_RTS_OPTS=-hT` env-var path through `python/aletheia/client/_ffi.py` `RTSState.acquire` (now uses `hs_init_with_rtsopts` so any RTS flag passes through regardless of `.so` link-time `-rtsopts`).  Time profile (`-p`) deferred — requires profiling-build rebuild; `docs/operations/STABILITY.md` documents the rebuild path.
- `[x]` ✅ CLOSED cluster 6 — AGDA-A-16.3 `docs/operations/STABILITY.md` (NEW) documents the full procedure: env-var contract, threshold model, per-binding sub-checks, forward-revert verification, CI wiring.
...
- `[x]` ✅ CLOSED cluster 17 AGDA-B-22.1 **[CAN/Frame.agda:39-41] `CANId.Standard`/`Extended` proof fields migrated to `.(…)` irrelevance.** Per G-A4, the proof exists solely to enforce a precondition; `.(…)` makes `_≟-CANId_` collapse to `≟ ℕ` (no `T-irrelevant`/`cong` plumbing), and MAlonzo erases the per-CANId proof cell — `T_CANId_8 = C_Standard_12 Integer | C_Extended_16 Integer` (was `Integer AgdaAny`). Cascade: `Prelude.ifᵀ-witness` accepts irrelevant `f` and `pf`; `IntDecRat.isInt` and `NatDecRat.isNat` migrated to `.(…)` (cascade — `cong (mkX _) (T-irrelevant ...)` simplifies to `refl`); `MetadataRoundtrip.agda` + `MessageRoundtrip/{Standard,Extended}.agda` helper signatures take `.(pf : T (...))`; `Comments/Comment.agda` adds local `T-materialize` + `<ᵇ⇒<-irr` to bridge stdlib `<ᵇ⇒<` (which requires relevant input); `check-erasure` Shake gate updated to assert single-Integer ctor shape. Original "Medium effort" estimate was wrong — cascade reaches `Prelude` + 5 proof files + 2 record-type definitions; user-approved bite-the-bullet propagation 2026-05-09.
...
- `[x]` ✅ CLOSED cluster 17 (by-product of AGDA-B-22.1) AGDA-D-11.3 [CAN/Frame.agda:39-41 vs CAN/DLC.agda:71-75] `CANId` proof field NON-erased, `DLC.bounded` `@0`-erased — close-related-domain-types asymmetry. **Resolved at the runtime layer**: post-cluster-17, both CANId proof fields and DLC.bounded compile to no runtime cell in MAlonzo (CANId via `.(…)` irrelevance, DLC.bounded via `@0` erasure — distinct mechanisms, identical compiled form). The mechanism asymmetry remains (Agda's irrelevance and erasure are orthogonal: `.(…)` is definitionally-irrelevant for type-checking + erased; `@0` is purely runtime-erased) but the user-visible asymmetry the finding flagged ("close-related-domain-types") is gone — both fields are equally invisible at runtime. CANId picked `.(…)` over `@0` because data constructors lack η-equality and `@0` would have left `_≟-CANId_` requiring relevant proof access; DLC's record has η so `@0` works there. Future unification (both to `.(…)`) is a separate opportunity, deferred.
...
- `[x]` ✅ CLOSED cluster 6 — GO-B-27.6 / 27.7 / 27.8 / 27.9 covered by `go/benchmarks/stability/main.go`: (a) FD count delta via `/proc/self/fd` (anon_inode filtered), (b) goroutine count delta via `runtime.NumGoroutine`, (c) RSS delta via `runtime/metrics /memory/classes/heap/objects:bytes`, (d) StablePtr accounting via `aletheia.StablePtrCount()` (new public API, atomic counter incremented in `FFIBackend.Init` and decremented in `FFIBackend.Close`).  Hard-zero gates on (a)/(b)/(d), soft-threshold on (c).  Forward-revert verified.
...
- `[ ]` CPP-B-26.1-26.10 — `collect_matching_signals` O(n*m); `std::map` insertion log-cost; `emplace` vs `try_emplace`; `last_frames_.insert_or_assign` payload copy; `cache_` `std::map` heterogeneous lookup; `error_code_table` linear scan (cold path); `parse_issue_code` 21-clause if-chain (cold path); `cache_full` warn rate-limit absent; `SignalValue` default-construct asserts in debug.  **Stability benchmark sub-checks ✅ CLOSED cluster 6** (RSS / FD / threads / malloc_info covered by `cpp/benchmarks/stability_bench.cpp`); remaining hot-path findings stay open as cat 26 backlog.
...
- `[x]` ✅ CLOSED cluster 6 — PY-B-25.3a / 25.3b / 25.3c / 25.3d covered by `python/benchmarks/stability.py`: (a) RSS via `psutil.Process().memory_info().rss`, (b) FD count via `/proc/self/fd` (anon_inode filtered), (c) ctypes-handle accounting via `RTSState.refcount` post-final-close + per-cycle `_state is None` invariant assertion, (d) logger-handler count via `len(logging.getLogger("aletheia").handlers)`.  Hard-zero gates on (b)/(c)/(d), soft-threshold on (a).  `psutil` added to `[dev]` extras.  Forward-revert verified.
...
- `[ ]` DOC-B-19.3 No `tools/check_reproducible_build.py`.
...
11. **Long-run resource leakage R18 sub-checks across all 3 bindings** — AGDA-A-16.1-3, GO-B-27.6-9, CPP-B-26.* (long-run sub-checks portion), PY-B-25.3a-d. **✅ CLOSED cluster 6** — closes 12 findings.  Per-binding stability harnesses (`python/benchmarks/stability.py` / `go/benchmarks/stability/main.go` / `cpp/benchmarks/stability_bench.cpp`) exercising 1M frames in cycles × frames pattern + GHC RTS heap profile via `ALETHEIA_RTS_OPTS=-hT` env-var path through `RTSState.acquire`'s switch from `hs_init` to `hs_init_with_rtsopts`.  Static gate `tools/check_stability_bench.py` parses `docs/STABILITY_BENCH.yaml` and verifies markers in harness sources (Shake `phony "check-stability-bench"`, run_ci.py step 12 always-on).  Dynamic gate `tools/stability_run.py` runs harnesses + archives to `benchmarks/stability/<short-sha>/` (run_ci.py step 29 opt-in via `ALETHEIA_STABILITY_CHECK=1`).  Two-tier threshold model: hard-zero on real-resource leaks (FD / goroutine / StablePtr / ctypes / logger handlers), soft on RSS / malloc_info.  Forward-revert verified for all three bindings: per-cycle FD leak fires hard-zero gate with precise diagnostic.  `docs/operations/STABILITY.md` documents the procedure.
...
17. **CANId proof-field irrelevance migration** — AGDA-B-22.1. **✅ CLOSED cluster 17 2026-05-09**. `CAN/Frame.agda:39-41` migrated `T (n <ᵇ max)` → `.(_ : T (...))` per G-A4. `_≟-CANId_` collapses to `≟ ℕ` + `yes refl`. MAlonzo erases the proof cell: `T_CANId_8 = C_Standard_12 Integer | C_Extended_16 Integer` (was `Integer AgdaAny`). Cascade required: `Prelude.ifᵀ-witness` accepts irrelevant `f`/`pf`; `IntDecRat.isInt` and `NatDecRat.isNat` `.(…)`-migrated (`cong (mkX _) (T-irrelevant ...)` → `refl` via record η); `MetadataRoundtrip.agda` + `MessageRoundtrip/{Standard,Extended}.agda` helper signatures take `.(pf : T (...))`; `Comments/Comment.agda` adds local `T-materialize` + `<ᵇ⇒<-irr` to bridge stdlib `<ᵇ⇒<`; `check-erasure` Shake gate updated. Closes 2 findings: AGDA-B-22.1 + AGDA-D-11.3 (close-related-domain-types asymmetry by-product — runtime cell now equally absent for CANId and DLC). Cumulative across Rounds 1-10 + clusters 1, 2, 3, 4, 5, 6, 7, 17: **112 hard findings closed**.
