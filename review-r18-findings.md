...
- `[ ]` **CICD-5.5** `phony "docker"` (`Shakefile.hs`) tags `aletheia:latest` (moving tag); no commit-SHA-derived tag, no digest pin.  DEFER — Docker image is not currently part of the release lane, only `dist/aletheia.tar.gz` is.  Track for follow-up if Docker becomes a release artifact.
...
- `[ ]` **CICD-5.7** `phony "dist"` writes "Usage" hint to stdout only; no `dist/aletheia/README` shipped inside the tarball.  DEFER — MANIFEST.txt + SBOM (in-tarball) cover the audit trail; integration guidance lives in `docs/development/DISTRIBUTION.md`.  A standalone in-tarball README would duplicate, not add.
...
3. **UR-3: Create `tools/check_reproducible_build.py` + Shake `dist` SHA256 recording + SBOM + signing** — ✅ CLOSED cluster 3.  Single bundled commit + bash-to-Python migration sweep (per user direction 2026-05-08): `tools/check_reproducible_build.py` + `tools/sbom_generate.py` + extended `Shakefile.hs dist` (sha256 chain, MANIFEST, SBOM call, cosign sign-blob, reproducible tar+gzip) + `cpp/CMakeLists.txt` `-ffile-prefix-map` + `keys/cosign.pub` + `docs/development/RELEASE.md` + opt-in `tools/run_ci.py` step 22.  All six existing `tools/*.sh` scripts migrated to Python (≥ 3.13.7) — `check_changelog.py` / `check_gate_claim.py` / `check_action_pins.py` / `check_workflow_permissions.py` / `install_hooks.py` / `run_ci.py`.  Closes UR-3.1-3, CICD-5.1/2/3/4/6, CICD-2.1, CICD-4.1.  Two CICD-5 sub-items deferred (5.5 docker tag pinning, 5.7 in-tarball README) — both not load-bearing for the current release lane.
