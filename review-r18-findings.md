...
- `[ ]` AGDA-A-2.1 [src/Aletheia/DBC/TextParser/Properties/Comments/Comment.agda:118-119] inline literal `2048` should reference `standard-can-id-max` constant.
- `[ ]` AGDA-A-2.2 [src/Aletheia/DBC/TextParser/Properties/Comments/Comment.agda:127] prose comment `(n < 2048 < 2^31)` should reference `standard-can-id-max` and `extFlagBit`.
...
- `[ ]` AGDA-A-4.1 [src/Aletheia/DBC/TextParser/Format/ValueDescription.agda:129-130] open-ended TODO with no link to memory file or follow-up issue.
...
- `[ ]` AGDA-D-11.1 [Formatter/WellFormed.agda:82 + TextParser/Properties/Aggregator/Universal.agda:132] **Two distinct `WellFormedDBC` records under same name in different modules with different fields** (1 vs 8 fields). Importing the wrong one silently weakens preconditions.
- `[ ]` AGDA-D-11.2 [Formatter/WellFormed.agda:82] JSON-side `WellFormedDBC` doesn't carry `unresolved-empty`/`msg-ids-unique`/attribute-WF/SG-WF; a DBC failing CHECK 18/CHECK 23 satisfies it but isn't text-roundtrip-invertible.
...
- `[ ]` AGDA-D-15.4 [Formatter/WellFormed.agda:82 vs Aggregator/Universal.agda:132] same `WellFormedDBC` record name in two places (cf. AGDA-D-11.1).
...
- `[ ]` AGDA-D-19.6 [Substrate/Unsafe.agda:116-121] `WellFormedDBC d` precondition for `parseText-on-formatText`: JSON-parser path produces witnesses by construction (type b); `formatDBCText` FFI command does NOT enforce — accepts any DBC JSON, would silently fail to roundtrip on malformed input. Mixed discharge.
...
- `[ ]` AGDA-D-GA20.4 [Aggregator/Universal.agda:132] `WellFormedDBC` type defined alongside the universal theorem; should split.
...
- `[ ]` CPP-B-11.3 [json_serialize.cpp:299-309 vs json_parse.cpp:584-593] **Serializer emits `unresolvedValueDescs`; parser doesn't read it. JSON-roundtrip drops Track E.8 data.**
...
- `[ ]` PY-S-15.1 **[asyncio/__init__.py:15] Docstring example shows `AletheiaClient(dbc=dbc_path)` — neither sync nor async `__init__` accepts `dbc=` kwarg. Example does not run.**
...
- `[ ]` DOC-A-1.13 **[CANCELLATION.md:150] Code uses `AletheiaClient(dbc=dbc_path, checks=load_checks())` — neither sync nor async `__init__` accepts `dbc=`/`checks=` kwargs.**
- `[ ]` DOC-A-1.14 [CANCELLATION.md:302] References `AsyncAletheiaClient` — not a defined symbol.
- `[ ]` DOC-A-1.15 [CANCELLATION.md:324] Cites matrix rows `lazy_frame_iter`, `cancellation` — actual IDs are `lazy_streaming_batch`, `cancellation_contract`.
...
- `[ ]` DOC-B-15.1-15.6 **`iter_can_log` 4-tuple unpack across QUICKSTART.md / TUTORIAL.md:258 / PITCH.md:190 / PYTHON_API.md:33,44,771,879 / COOKBOOK.md:181 / PYTHON_API.md:869 — ALL would raise `ValueError` at runtime if executed.**
...
4. **CICD: bootstrap `.github/workflows/` + `.github/dependabot.yml` + 3 audit scripts + `actionlint`** — closes CICD-1.1-5, CICD-3.1, CICD-5.1.  Three deferred enforcement / orchestration sub-items also land here:
   - **Sub-item (a)** — `tools/check-changelog.sh` script that diffs the public surface of `python/aletheia/` (anything not `_`-prefixed), `go/aletheia/` (any exported), and `cpp/include/aletheia/` against the previous commit and fails when API drift exists without a matching `CHANGELOG.md` entry under the active version section.  Deferred from cluster 8 / Round 2.
   - **Sub-item (b)** — gate-claim-integrity gate that fails commits whose message contains a "all N gates clean" / "gates green" assertion without an attached gate-output artifact (committed under `tools/ci-output/<commit-prefix>.log` or similar).  Deferred from the Round 1 follow-up — directly addresses the failure mode where `ae9fe67` claimed gates clean but the `.so` mtime predated the source edits.
