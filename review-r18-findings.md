...
- `[x]` ✅ CLOSED Round 7 AGDA-D-15.4 [Formatter/WellFormed.agda:82 vs Aggregator/Universal.agda:132] same `WellFormedDBC` record name in two places (cf. AGDA-D-11.1).  Closed by the same rename as AGDA-D-11.1: text-side renamed to `WellFormedTextDBCAgg` in new module `Aletheia.DBC.TextParser.WellFormed`.
