...
def main() -> int:
    ...
    if rc.returncode != 0:
        sys.stderr.write("check-action-pins: not inside a git repo\n")
        return 2
    repo_root = Path(rc.stdout.strip())

    workflows_dir = repo_root / ".github" / "workflows"
    if not workflows_dir.is_dir():
        print(f"check-action-pins: {workflows_dir} does not exist; skipping")
        return 0
    ...
    for workflow in sorted(workflows_dir.glob("*.y*ml")):
        for line in workflow.read_text(encoding="utf-8").splitlines():
            ...
            if repo.startswith("actions/") or repo.startswith("github/"):
                if BRANCH_REF_RE.match(ref):
                    violations.append(
                        f"  {workflow.relative_to(repo_root)}: {repo}@{ref} "
                        f"(branch ref disallowed even for github-owned)"
                    )
                continue
            ...
            if not SHA_RE.match(ref):
                violations.append(
                    f"  {workflow.relative_to(repo_root)}: {repo}@{ref} "
                    f"(third-party — must be SHA-pinned, not tag/branch)"
                )
