...
# Slice {number} of {total}: the mutation configuration of {config}, with the
# files of the other slices held out so this build carries this slice's
# mutants alone.  The merge unions a tree's slices and refuses a census that
# is not the recorded one, which is what a file missing from every slice --
# and so mutated by all of them, under identifiers that then arrive twice --
# fails on.
"""
...
def slice_config_text(
    ...
    they are written; the slice adds the files it does not claim.  Text rather
    than a file, because this content is what decides whether the tree holding
    it is stale: nothing else in the build knows an object depends on it.
    """
    config = _load_config(config_path)
    excludes = held_out_patterns(config_path)
    config["excludePaths"] = excludes + [hold_out_pattern(path) for path in held_out]
    header = _SLICE_HEADER.format(number=number, total=total, config=config_path.name)
    return header + yaml.safe_dump(config, sort_keys=False)


def write_slice_config(
    config_path: Path, dest: Path, held_out: Iterable[RelPath], number: int, total: int
) -> None:
    """Write one slice's Mull configuration beside its build tree."""
    dest.write_text(slice_config_text(config_path, held_out, number, total), encoding="utf-8")
