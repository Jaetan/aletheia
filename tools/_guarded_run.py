...
Usage: ``python tools/_guarded_run.py <watch-fd> <grace-seconds> <command>...``

The caller starts this script as the leader of a new process group and passes
it the read end of a pipe whose write end only the caller holds and never
writes.  The read end therefore reports end-of-file exactly when the caller
closes the write end or dies, SIGKILL included, since the kernel closes a dead
process's descriptors.  At end-of-file the script sends SIGINT to its group,
...
def _stop_on_end_of_file(
    watch_fd: int, grace: float, child: subprocess.Popen[bytes], stopping: threading.Event
) -> None:
    """Block until ``watch_fd`` reads end-of-file, then stop the group."""
...
def main(argv: list[str]) -> int:
    """Run ``argv[2:]``, stopping its group when descriptor ``argv[0]`` reads end-of-file.
    ...
    with subprocess.Popen(argv[2:]) as child:
        ...
        watcher = threading.Thread(
            target=_stop_on_end_of_file, args=(watch_fd, grace, child, stopping), daemon=True
        )
        watcher.start()
