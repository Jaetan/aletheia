...
- `[Design-held]` AGDA-C-6.4 [src/Aletheia/DBC/Formatter/MessageRoundtrip/{Standard,Extended}.agda] 72-line mirror twins — **adjudicated design-held** (2026-05-12, cluster 14 / AGDA-C-6.4): the two 71-LOC files are byte-identical apart from {`Standard`↔`Extended` ctor, `standard-can-id-max`↔`extended-can-id-max` bound, `-std`↔`-ext` suffix}.  Parameterising over a `(max : ℕ, ctor : (n : ℕ) → .(T (n <ᵇ max)) → CANId)` pair blocks the proof body `ifᵀ-witness _ _ pf`: that one-liner only type-checks because Agda automatically reduces `messageFields (mkMessage (Standard rawId pf) …)` (or `Extended …`) by pattern-matching on the concrete ctor — emitting `"extended": false` (or `true`) — which then lets `parseCANId`'s `with lookupBool "extended" obj` step into the matching branch and `ifᵀ-witness` extract the witness against the matching bound.  Abstracting over `ctor` halts that reduction at `messageFields (mkMessage (ctor n pf) …)`; restoring it costs (a) an extra `extended-flag : Bool` parameter, (b) a witness `ctor-emits : ∀ n .(pf : T (n <ᵇ max)) → canIdExtended (ctor n pf) ≡ extended-flag`, and (c) a `rewrite ctor-emits …` plumbing step at every callsite — net machinery roughly matches the lines saved.  Same cascade-of-witnesses pattern as AGDA-C-6.1 InContext lift (also design-held).  At 142 LOC across both files this is well below the `feedback_step_back_when_proofs_balloon.md` ~500-1000 LOC threshold; the textual mirror reflects a genuine binary structural distinction in the parser/formatter/CANId data type (two branches via `lookupBool "extended"`, two ctors via `Standard`/`Extended`).  Cross-binding parity unaffected — kernel-side proof artifact only.
