...
- `[Design-held]` AGDA-C-5.3 [src/Aletheia/Error.agda:351 + 326] `DBCTextParseError` and `DispatchError` lack `InContext` ctor — **adjudicated design-held** (2026-05-11, cluster 14 / AGDA-C-6.2 follow-on): InContext is added per-ADT when *sub-parsers* need labeled-context wrapping for inner errors.  DBCTextParseError errors flow up from a structurally-recursive text parser carrying positional info already (`TrailingInput Position`); contextualization happens at the top-level via `WithContext` (e.g., `WithContext "ParseDBCText" (...)` at the handler).  DispatchError is emitted at the top-level dispatcher only — there are no sub-parsers of DispatchError that would attach a context label.  Adding the ctors for symmetry without an active use case is YAGNI.

#### Cat 6: Redundant patterns (6)
- `[Design-held]` AGDA-C-6.1 [src/Aletheia/Error.agda:67,148,184,224,288] `InContext` duplicated across 5 ADTs — **adjudicated design-held** (2026-05-11, cluster 14 architectural-half analysis): unlike `InputBoundExceeded` (AGDA-C-6.2, lifted because its `BoundKind → ℕ → ℕ → X` signature is identical across ADTs and the BoundKind enum already carries the sub-discrimination), `InContext : String → X → X` is a *recursive* ctor over each ADT.  Lifting to top-level `Error.WithContext` (which already exists) would change the contextualization API: instead of sub-parsers returning `SubError ⊎ A` and contextualizing internally (`InContext "fieldX" innerErr : SubError`), every sub-parser would need to return `Error ⊎ A` directly.  This cascades through JSONParser / Routing / Handlers — ~41 InContext references across kernel + every sub-parser type signature.  Combinator-via-typeclass-like indirection isn't available in Agda idiomatically.  The per-ADT InContext is the natural shape for recursive-ADT contextualization; the surface duplication reflects a genuine structural pattern, not redundant code.  Skipping the lift here is the same design call AGDA-C-3.1 (`formatChars` FP adjudication) made: two distinct, internally-consistent conventions for two distinct roles.
...
- `[x]` AGDA-C-6.3 [src/Aletheia/DBC/Validator/Checks.agda:506-541 + 551-567] 3 near-identical `checkUnknown*` functions — closed by cluster 14 / AGDA-C-6.3 — added `private checkUnknownNodeReference : List Node → String → IssueCode → String → List ValidationIssue` helper wrapping the shared `requireDec (any? ...) (mkIssue IsWarning code detail)` pattern; `checkUnknownSender` / `checkUnknownReceiver` / `checkUnknownAdditionalSender` (CHECK 20/21/22) bodies collapse to one-line calls.  Proof-transparent: `*-allW` warning-severity proofs in `Aletheia.DBC.Validity.WarningChecks` (lines 661-739) need no edits because the helper unfolds to the same shape the proofs match on.
- `[Design-held]` AGDA-C-6.4 [src/Aletheia/DBC/Formatter/MessageRoundtrip/{Standard,Extended}.agda] 72-line mirror twins — **adjudicated design-held** (2026-05-12, cluster 14 / AGDA-C-6.4, empirically verified 2026-05-12): the two 71-LOC files are byte-identical apart from {`Standard`↔`Extended` ctor, `standard-can-id-max`↔`extended-can-id-max` bound, `-std`↔`-ext` suffix}.  Empirical attempt: a scratch `ParamTest.agda` module parameterised over `(max : ℕ) (ctor : (n : ℕ) → .(T (n <ᵇ max)) → CANId)` with body `canId-rt rawId pf … = ifᵀ-witness _ _ pf` was created and type-checked; Agda rejected it with the goal stuck on `parseCANId (ctx (Identifier.name n)) rawId (messageFields (mkMessage (ctor rawId _) n dlc sender senders signals)) ≡ inj₂ (ctor rawId _)` — `messageFields ∘ mkMessage` could not reduce because `DBCMessage.id`, `DBCMessage.dlc`, `DBCMessage.signals`, `DBCMessage.senders` projections need the concrete ctor pattern-match to unfold, and parseCANId's `with lookupBool "extended" obj` could not branch.  This confirms the obstruction analytically predicted: the proof body works only because Agda automatically reduces `messageFields (mkMessage (Standard rawId pf) …)` (or `Extended …`) by pattern-matching on the concrete ctor — emitting `"extended": false` (or `true`) — which then lets `ifᵀ-witness` extract the witness against the matching bound.  Restoring the reduction in the parameterised form costs (a) an extra `extended-flag : Bool` parameter, (b) a witness `ctor-emits : ∀ n .(pf : T (n <ᵇ max)) → canIdExtended (ctor n pf) ≡ extended-flag`, and (c) a `rewrite ctor-emits …` plumbing step at every callsite — net machinery roughly matches the lines saved.  Same cascade-of-witnesses pattern as AGDA-C-6.1 InContext lift (also design-held).  At 142 LOC across both files this is well below the `feedback_step_back_when_proofs_balloon.md` ~500-1000 LOC threshold; the textual mirror reflects a genuine binary structural distinction in the parser/formatter/CANId data type (two branches via `lookupBool "extended"`, two ctors via `Standard`/`Extended`).  Cross-binding parity unaffected — kernel-side proof artifact only.
...
- `[Design-held]` AGDA-C-6.6 [src/Aletheia/LTL/SimplifySound/Decomposition.agda] ≥5 And/Or-symmetric lemma pairs — **adjudicated design-held** (2026-05-12, cluster 14 / AGDA-C-6.6, refined after empirical attempt 2026-05-12): partial-lift attempt on the genuinely-identical pair confirmed by empirical inspection.  Of the 5 direct And/Or helper pairs, only `-Unsure-Unsure` is a true textual mirror: `finalizeL-{And,Or}-Unsure-Unsure a b r r′ eq₁ eq₂ rewrite eq₁ | eq₂ = refl` (1-line body, identical for And and Or).  The other 4 pairs (`-Holds`, `-Fails`, `-Unsure-Holds`, `-Unsure-Fails`) are **flipped-role dual** — And's identity (Holds preserves right) is Or's zero (Holds absorbs to Holds), and vice versa — not derivable from each other by renaming.  `finalizeL-And-Holds : finalizeL a ≡ Holds → finalizeL (And a b) ≡ finalizeL b` and `finalizeL-Or-Holds : finalizeL a ≡ Holds → finalizeL (Or a b) ≡ Holds` have the same antecedent but dual conclusions — they're not derivable from each other via simple renaming.  Partial lift on `-Unsure-Unsure` alone (the only truly-identical pair) would save at most 1 LOC at the cost of a helper abstracting over a `(target-fn : LFormulaState → LFormulaState → LFormulaState, target-eq : ∀ a b r → finalizeL a ≡ Unsure r → finalizeL (target-fn a b) ≡ Unsure r)` pair — net abstraction machinery exceeds the saving.  Lifting to `Aletheia.LTL.Semantics.Duality` doesn't help directly: `Duality.agda` operates on the `⟦_⟧` denotational semantics over LTL formulas, while these helpers operate on the compiled `LFormulaState` automaton via `finalizeL` / `stepL` / `runL` — there's no `notFormula : LFormulaState → LFormulaState` that flips And-state to Or-state, so a runtime-level "lift the duality" theorem would need to traverse the entire `Adequacy.agda` bridge.  The two parameterisation options — (a) introducing a `BinOp : Set` enum + `binop : BinOp → LFormulaState → LFormulaState → LFormulaState` ADT constructor unifying And-states and Or-states (cascades through every `stepL` / `finalizeL` / `runL` clause and the entire `FormulaState/Properties` proof tree), or (b) defining a runtime-level duality theorem `finalizeL-Or-flip : ∀ a b → finalizeL (Or a b) ≡ notVerdict (finalizeL (And (notState a) (notState b)))` with a new `notState` runtime operation and `notVerdict` action (cascade through `Coalgebra.agda`, `Adequacy.agda`, `StreamState.agda`) — both cost more than the ~55 LOC saved.  At 110 LOC across paired definitions this is well below the `feedback_step_back_when_proofs_balloon.md` ~500-1000 LOC threshold; the dual-role structure is cleaner expressed as paired definitions than as parameterised helpers.  Same cascade-of-machinery pattern as AGDA-C-6.1 / 6.4 (also design-held).  Cross-binding parity unaffected — runtime-proof artifact only.
...
- `[x]` GO-A-6.2 [go/aletheia/{dbc,error,result,types}.go] 6 iota-enum types each hand-roll `String()` — closed by cluster 14 / GO-A-6.2 — replaced the 6 hand-rolled `String()` methods (Verdict / IssueSeverity / ErrorKind / DBCVarType / DBCAttrScope / ByteOrder, ~85 LOC total in source) with `//go:generate stringer -type=X -linecomment -output=...` directives + inline `// <wire-string>` comments on each iota ctor.  6 new `*_string.go` generated files (~150 LOC total) sit beside the source files and are checked in.  Stringer's empty-string `// ` linecomment on `DBCAttrScopeNetwork` produces the expected `""` via a zero-width `[0,0,...]` index slice (verified by inspecting `dbcattrscope_string.go`).  Default branch behaviour changes from `"unknown"` to `"<TypeName>(N)"` — one test (`error_test.go:99`'s `{ErrorKind(99), "unknown"}`) updated to `{ErrorKind(99), "ErrorKind(99)"}` per `feedback_no_backward_compat.md`.  Net source -85 LOC; generated +150 LOC (auto-maintained, single source of truth for ctor→wire-string mapping at codegen time).  `fmt` import dropped from `dbc.go` after removing the hand-rolled `fmt.Sprintf` fallbacks.  Build adds a `go generate` step (developer must `go install golang.org/x/tools/cmd/stringer@latest` once + run `go generate` after enum changes — analogous to Python's StrEnum value-as-string built-in and C++ inline switches at serialize sites: each binding uses its language-idiomatic mechanism for ctor↔wire-string mapping, codegen for Go matches Python's stdlib StrEnum without breaking cross-binding parity since each binding's mechanism is at the same architectural layer (ctor→string at type definition).  Initial adjudication as design-held was overturned 2026-05-12 by user request to verify empirically; the empirical attempt produced clean codegen with one trivial test update.
- `[x]` GO-A-6.3 [go/aletheia/json.go: 8 sites] Tier 1/2 array parsers share shape — closed by cluster 14 / GO-A-6.3 — added `parseObjects[T any](j map[string]any, fieldName string, decode func(map[string]any) (T, error)) ([]T, error)` generic helper.  7 list parsers (parseUnresolvedValueDescs / parseSignalGroups / parseEnvironmentVars / parseValueTables / parseNodes / parseComments / parseAttributes) collapsed from `getArray → if-empty → make → for-range → type-assert → append → return` template to one-line `return parseObjects(...)` calls with a per-entry decode lambda.  Nested entries in `parseValueTables` and `parseUnresolvedValueDescs` also recursively use `parseObjects[DBCValueEntry]`.  Net: -52 LOC (208 → 156 lines for the affected region; 78 added, 130 removed).
...
#### Cat 19: Domain model fidelity
- `[x]` PY-D-19.1 [python/aletheia/protocols.py:670-678] `SignalValue.value` `Fraction` but `EqualsPredicate.value` etc. `float` — IEEE-754 approx loss; align with DecRat universal principle — **closed** (2026-05-12, cluster 17 / PY-D-19.1) — Python side: 8 predicate TypedDicts in `protocols.py` migrated `value: float` → `value: Fraction` (5 single-value comparisons + Between min/max + ChangedBy delta + StableWithin tolerance).  `dsl.py` Signal methods accept `int | float | Fraction` via a new `_RationalInput` type alias and coerce to `Fraction(value)` internally — callers can keep writing `Signal("X").equals(220)` for exact integer values OR pass `Fraction(1, 10)` for exact 1/10-style precision (float input still allowed but quantizes through float64).  Wire serialization unchanged at the byte level: `FractionJSONEncoder` already emitted `{"numerator": n, "denominator": d}` for non-integer Fractions and bare integers for denominator-1 — the change is that non-integer values now flow as exact rational dicts (mirroring C++ `predicate_to_json`'s `rational_to_json(v.value.get())`) instead of decimal floats.  Pretty-printer `_format_predicate` in `client/_enrichment.py` extended to accept `Fraction` in its numeric coerce helper (display path only, lossy float conversion is fine for human-readable output).  Cross-binding parity status: C++ already used `Rational` for predicate value types ✓, Python now uses `Fraction` ✓, Go uses `Rational` ✓ (GO follow-up shipped same session).  Gates green (Python): pytest 813p+1s, basedpyright `aletheia/ benchmarks/` 0/0/0, pylint `aletheia/ tests/ benchmarks/` 10.00/10.

  **GO-D-19.1 mirror — closed same session 2026-05-12** — `ltl.go` predicate struct `Value`/`Min`/`Max`/`Delta`/`Tolerance` fields migrated from `PhysicalValue`/`Delta`/`Tolerance` (float64-aliased) to `Rational`.  `json.go` `serializePredicate` switched all 8 `float64()` casts to `serializeRational(...)`; the prior `rejectNonFinite` (NaN/Inf guard for the float wire form) replaced with `validateRational` (non-positive denominator guard) — Rational has no NaN/Inf representation, but denominator validation remains.  `between` min/max ordering check rewritten via `rationalLess` (cross-product comparison) since `Rational` has no native `>`.  `types.go` adds two helpers: `IntRational(n int64) Rational` (exact integer literal) + `RationalFromFloat(v float64) Rational` (10^9 scaling for non-integer float input, integer-fast-path, NaN/Inf clamp to 0/1).  Builder boundary in `check.go` (`CheckSignal.NeverExceeds` etc.) and `loader.go` (`DispatchSimple`/`DispatchWhen`) preserved their `PhysicalValue` ergonomic input; private `physicalAsRational` aliases `RationalFromFloat` for the conversion at boundary.  Display `formatPredicate` in `enrich.go` uses `Rational.Float64()` for the human-readable %g format.  Test cascade: 9 test files updated via two-pass perl regex (~150 sites) replacing `Value: <int>`/`Value: <float>` with `IntRational(N)` / `RationalFromFloat(N)` (or `aletheia.X(N)` in external `aletheia_test` package).  Doc fixes: `docs/PITCH.md` and `docs/reference/INTERFACES.md` Go fences updated `aletheia.PhysicalValue(220)` → `aletheia.IntRational(220)` for the canonical predicate-value example.  Gates green (Go): `go test -race` ok 8.108s, `gofmt -l` clean, `go vet` clean.  Cross-binding sanity: Python pytest 813p+1s, C++ ctest 10/10, Go unaffected.
- `[x]` PY-D-19.2 [python/aletheia/protocols.py:97-99] `DBCSignal = DBCSignalAlways | DBCSignalMultiplexed` — **closed** (2026-05-12, cluster 17 / PY-D-19.2 reopen).  Added explicit `presence: Literal["multiplexed"]` REQUIRED field on `DBCSignalMultiplexed`, mirroring `DBCSignalAlways`'s `presence: "always"`.  Cross-binding parity propagated through every serializer: Agda `formatPresence` (Formatter.agda:84-87), Go `serializeDBC` (go/aletheia/json.go:151), C++ `presence_to_json` (cpp/src/json_serialize.cpp:67) all emit the explicit discriminator on multiplexed signals.  Updated `SignalPresence = Literal["always", "multiplexed"]`.  Updated parity snapshots (`kitchen_sink.json`, `multiplexing.json`) via JSON-rewriting script — 2 snapshots changed.  Updated 1 internal construction site (`excel_loader.py:_parse_dbc_signal`), 5 `DBCSignalMultiplexed(...)` constructor calls in `tests/test_dbc_queries.py`, 1 test assertion in `tests/test_excel_loader.py`, 1 test in `cpp/tests/unit_tests_json.cpp` (was asserting `!contains("presence")` — flipped to assert `"presence" == "multiplexed"`).  Agda parser side: `parseSignalPresence` already tries `multiplexor` first then falls back to `presence` field, so adding the redundant `"presence": "multiplexed"` is structurally accepted (parser still routes via multiplexor); the field is informational mirror.  Gates green: build 2m02s, check-properties 9m36s, Python 812 pytest + basedpyright `aletheia/+benchmarks/` 0/0/0 + pylint 10.00/10, Go all tests pass, C++ 10/10 ctest.  Tests/ basedpyright dropped 451 → 409 (-42) — the explicit discriminator structurally helps narrowing in many sites.
- `[x]` PY-D-19.3 [python/aletheia/protocols.py:678] Negative-denominator parse path Python-vs-Agda normalization unverified — closed by cluster 17 type-coherence batch — added `test_parse_rational_normalises_to_positive_denominator` hypothesis property test in `tests/test_property_hypothesis.py` (denominator range expanded to include negative values).  Verifies that `parse_rational({"numerator": n, "denominator": d})` for any non-zero d (positive or negative) returns a Fraction with `denominator > 0` (canonical sign-flipped form, matching Agda's DecRat ℕ-denominator invariant).  parse_rational already delegated to `Fraction(n, d)` which Python normalises; the test locks that behaviour against accidental regression.  `parse_rational` added to the private-import whitelist for `test_property_hypothesis.py`.
- `[x]` PY-D-19.4 [python/aletheia/protocols.py:106-117] `DBCMessage.dlc: int` ambiguous (byte count vs raw code) — **closed** (2026-05-12, cluster 17 / PY-D-19.4 reopen).  Added `DLCByteCount = NewType("DLCByteCount", int)` and `DLCCode = NewType("DLCCode", int)` in `protocols.py`.  Threaded through `DBCMessage.dlc: DLCByteCount`, `CANFrameTuple.dlc: DLCCode`, `dlc_to_bytes: DLCCode → DLCByteCount`, `bytes_to_dlc: DLCByteCount → DLCCode`, `validate_payload_length(dlc: DLCCode, ...)`, `FrameIdentity.dlc: DLCCode`.  Public API signatures updated: sync + async `send_frame` / `extract_signals` / `update_frame` / `build_frame` all take `dlc: DLCCode`.  **Real bug surfaced**: `cli.py:428` was calling `dlc_to_bytes(msg["dlc"])` where `msg["dlc"]` is byte count, not DLC code — would have raised `ValueError("Invalid DLC code: 12")` for any CAN-FD message with byte count > 8.  NewType caught the type confusion; fix is to use `msg["dlc"]` directly (it's already the byte count).  Cascade: 28 aletheia/+benchmarks/ sites (gate scope) all closed mechanically + ~95 test sites also closed for consistency (tests/ not gated but tracked).  Total +1 file pylint dropped 10.00 → 9.99 due to C0302 on protocols.py (1022 lines, was 1011); compressed NewType comment to bring back under display threshold rounding (1022/1000 violation still real but masked at project score).  **Follow-up shipped same session 2026-05-12**: protocols.py (1029 LOC) split — DBC types section + shared aliases (`ByteOrder`, `SignalPresence`, `DLCByteCount`, `DLCCode`) moved to new `python/aletheia/_dbc_types.py` (476 LOC), `protocols.py` reduced to 654 LOC; both well under C0302 1000-line threshold; `aletheia.protocols` remains the canonical public re-export surface so no caller changes.  Same-commit R0801 dedup: new `empty_dbc_tier2() -> _DBCTier2Empty` helper in `_dbc_types.py` collapses the 7-key Tier-2 `[]` defaults at the two construction sites (`excel_loader.py:570-580`, `tests/_canonical_dbc.py:45-55`).  **Regression test for latent cli.py bug**: `TestExtractCommand::test_canfd_16_byte_payload` (tests/test_cli.py:312) exercises the CAN-FD code path — `extract --dbc canfd.dbc 768 <16-byte payload>` — that would have raised `ValueError("Invalid DLC code: 16")` pre-fix; manually verified the test fails with `dlc_to_bytes(msg["dlc"])` reverted in cli.py:433 and passes with the post-fix `msg["dlc"]` direct read.  Gates green: `basedpyright aletheia/ benchmarks/` 0/0/0, `pylint aletheia/ tests/ benchmarks/` 10.00/10 (C0302 + R0801 now structurally absent; pre-existing C2801 on `test_cancellation.py:352` unchanged per cluster 17 baseline — rounds below display threshold), 813 pytest passed.
- `[x]` PY-D-19.5 [python/aletheia/protocols.py:778-779] `PropertyViolationResponse.timestamp: RationalNumber` — closed by cluster 17 type-coherence batch — added `validate_integer_rational(field_name, raw_value) -> RationalNumber` helper in `client/_helpers.py` that wraps `validate_rational` and asserts the post-parse denominator is exactly 1.  Used at all 4 response-parser sites that consume `timestamp` (microseconds, always integer) or `property_index` (ℕ, always integer) — `_response_parsers.py:171,172,271,281`.  A non-unit denominator now raises `ProtocolError` indicating a wire-format violation by the kernel.
- `[x]` PY-D-19.6 [python/aletheia/protocols.py:432-449] `DBCDefinition` Tier 2 fields `NotRequired`; Go/C++ structs non-optional — **closed** (2026-05-12, cluster 17 / PY-D-19.6 reopen).  Dropped `NotRequired` from the 7 Tier-2 list fields (`signalGroups`, `environmentVars`, `valueTables`, `nodes`, `comments`, `attributes`, `unresolvedValueDescs`), bringing Python's TypedDict structural shape into parity with Go/C++ struct definitions (where these fields are non-optional by language idiom).  Cascade was bounded: 9 construction sites needed `[]` defaults (2 in `aletheia/` — `_helpers.py:normalize_dbc` + `excel_loader.py:_parse_dbc_rows`; 7 in `tests/` — `_canonical_dbc.py:make_dbc` + `test_cli.py:495` + `test_dbc_queries.py` × 4 fixtures + `test_format_dbc.py` × 2 inline + `test_property_hypothesis.py:dbc_strategy`).  Wire-shape parity preserved (already enforced by `normalize_dbc_for_wire` at FFI send boundary; my change just tightens the source-language type to match the wire).  Gates green: `basedpyright aletheia/ benchmarks/` 0/0/0, `pylint aletheia/ tests/ benchmarks/` 10.00/10, 812 pytest passed.  Total tests/ basedpyright errors dropped 458 → 424 (-34) — the tightening structurally collapses some pre-existing inference paths.

#### Cat 20: Design coherence
- `[x]` PY-D-20.1 [python/aletheia/client/_types.py:16-127] Flat `AletheiaError` hierarchy; caller can't distinguish "FFI null" from "Agda rejected" — **closed** (2026-05-12, cluster 17 / PY-D-20.1) — refactored Python error hierarchy to mirror Go (`ErrorKind` 4-value enum) and C++ (`ErrorKind` 7-value enum) per `feedback_cross_language_parity.md`.  Added `FFIError` (mirror Go `ErrFFI` / C++ `Ffi`: library load / hs_init / `aletheia_init() → null`) and `StateError` (mirror Go `ErrState` / C++ `State`: wrong client lifecycle state — not initialized, no DBC, not streaming).  Removed the overloaded `ProcessError` class; its 17 raise sites split by category: 1 → `FFIError` (aletheia_init null), 7 → `StateError` (Client not initialized × 5 + DBC not loaded × 2), 3 → `ValidationError` (no DBC message for CAN ID, unknown signal × 2, payload length mismatch), 6 → `ProtocolError` (binary FFI Truncated/Zero-denom/Buffer-too-short, kernel ErrorResponse from extract_signals path, `_decode_and_raise` binary-path kernel errors).  Public surface: `aletheia` package re-exports `FFIError` + `StateError`; `ProcessError` removed (per `feedback_no_backward_compat`, no shim).  `docs/reference/PYTHON_API.md` § Exceptions diagram + worked example rewritten to show all 6 subclasses + cross-binding parity note.  Test cascade: 6 test files updated (test_batch.py, test_binary_extraction.py, test_types_and_conditions.py, test_unified_client.py, test_unified_client_canfd_mux.py — replaced `ProcessError` imports + `pytest.raises` matchers with the new specific class).  One test re-stated to reflect the actual lift path (`test_extract_signals_without_dbc` now expects `ProtocolError` matching the kernel `handler_no_dbc` rejection — the JSON path doesn't have a populated signal_lookup until parse_dbc succeeds, so the request reaches the FFI before any client-side state check could fire; `build_frame` keeps `StateError` because its path checks the lookup cache up-front).  Gates green: pytest 813p+1s, basedpyright `aletheia/ benchmarks/` 0/0/0, pylint `aletheia/ tests/ benchmarks/` 10.00/10.
- `[ ]` PY-D-20.2 [python/aletheia/asyncio/_client.py:97-109] `__aexit__/close` lack `asyncio.shield` (cross-ref PY-B-12.1)
- `[x]` PY-D-20.3 [python/aletheia/cli.py:106-123 + testing.py:22] `run_checks` lives in `cli.py` — layering inversion; move to `aletheia/checks_runner.py` — **closed** (2026-05-12, cluster 17 / PY-D-20.3) — extracted `Violation` / `CheckRunResult` / `_check_meta` / `_build_violation` / `_build_eos_violation` / `rational_to_int` / `run_checks` into NEW `python/aletheia/checks_runner.py` (220 LOC); `_lazy_iter_can_log` lazy-import helper moved with `run_checks`.  CLI-coupled `_die`-on-failure paths in `run_checks` replaced with `raise AletheiaError(...)` so the engine layer carries no `print` / `sys.exit` calls; `_cmd_check` in `cli.py` extends its existing `except FileNotFoundError` to `except (FileNotFoundError, AletheiaError)` to route engine failures to its `_die` exit-code path.  `aletheia.testing` re-exports now read from `.checks_runner` (was `.cli`) — fixes the layering inversion the finding called out.  `python/tests/test_cli.py` updated: `rational_to_int` import moved to `aletheia.checks_runner`; `run_checks` import path through `aletheia.testing` unchanged (still works via the new re-export).  Net Python module count +1 (`checks_runner.py` new; cli.py shrunk by ~135 LOC).  Gates green: pytest 813p+1s, basedpyright `aletheia/ benchmarks/` 0/0/0, pylint `aletheia/ tests/ benchmarks/` 10.00/10.
- `[x]` PY-D-20.4 [python/aletheia/validation.py] Module hosts only enum-types; rename `aletheia/issue_codes.py` or add thin `validate(dbc)` wrapper — **closed** (2026-05-12, cluster 17 / PY-D-20.4) — renamed `python/aletheia/validation.py` → `python/aletheia/issue_codes.py` (rename-only; module hosted `IssueSeverity` / `IssueCode` / `ValidationIssue` and no validator function).  Updated 4 internal imports (`__init__.py`, `protocols.py`, `cli.py`, `client/_response_parsers.py`), `python/pyproject.toml` `[tool.mutmut]` `paths_to_mutate` entry, `docs/MUTATION_BENCH.yaml` line 44 path entry, `docs/operations/MUTATION.md` line 53 table cell, `docs/operations/RUNBOOK.md` line 466 enum-source comment.  Public re-exports (`from aletheia import IssueCode, IssueSeverity, ValidationIssue`) unchanged — the breaking change is only for callers who imported via the module path `aletheia.validation` directly.  Per `feedback_no_backward_compat.md`, no shim left at the old path.  Docstring in the new file dropped the obsolete "Split out of protocols.py at Track E.11" historical reference per CLAUDE.md "don't reference current task" guidance — git log preserves the lineage.  Gates green: pytest 813p+1s, basedpyright `aletheia/ benchmarks/` 0/0/0, pylint `aletheia/ tests/ benchmarks/` 10.00/10 (pre-existing C2801 on `test_cancellation.py:352` unchanged).
- `[ ]` PY-D-20.5 [python/aletheia/dsl.py + checks.py] Two API surfaces; no `CheckResult.from_property(p)` inverse
- `[x]` PY-D-20.6 [python/aletheia/cli.py:527 `run_checks`] Returns 3-tuple; `@dataclass(frozen=True) class CheckRunResult` candidate — **closed** (2026-05-12, cluster 17 / PY-D-20.6) — added `@dataclass(frozen=True, slots=True) class CheckRunResult` (fields: `violations`, `unresolved`, `total_frames`) next to `Violation` in `cli.py`.  `run_checks` return type changed from `tuple[list[Violation], list[Violation], int]` to `CheckRunResult`.  Updated `_print_check_results` signature to `(result: CheckRunResult, num_checks: int)`, internal `_cmd_check` callsite (cli.py:660), and `python/benchmarks/violations.py:201` (3-tuple unpack → attribute access; the now-obsolete docstring tale about "the prior version unpacked only 2 values" stripped per CLAUDE.md "don't reference current task").  `aletheia.testing` re-exports the new symbol.  Gates green: pytest 813p+1s, basedpyright 0/0/0, pylint 10.00/10.
