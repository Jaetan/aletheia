...
- `[Design-held]` GO-A-6.2 [go/aletheia/{dbc,error,result,types}.go] 6 iota-enum types each hand-roll `String()` — **adjudicated design-held** (2026-05-12, cluster 14 / GO-A-6.2): The 6 hand-rolled `String()` methods (Verdict / IssueSeverity / ErrorKind / DBCVarType / DBCAttrScope / ByteOrder, each ~8-15 LOC) are the language-idiomatic Go approach; Python uses `class X(str, Enum)` (StrEnum, where the enum value IS the string) and C++ inlines the mapping at serialize sites (`json_serialize.cpp`).  Forcing Go to `go generate` + `stringer` would (a) add a build-time dependency on `golang.org/x/tools/cmd/stringer`, (b) introduce generated `*_string.go` files, (c) break cross-binding "language-idiomatic per binding" parity since neither Python nor C++ uses codegen for this, and (d) require per-enum `-linecomment` directives because each enum has its own wire-format convention (lowercase ctor name vs. DBC keyword vs. snake_case wire code; `Verdict.Holds → "holds"`, `DBCAttrScope.Network → ""`, `ByteOrder.LittleEndian → "little_endian"` — `stringer`'s default emit would be `"Holds"`, `"DBCAttrScopeNetwork"`, `"LittleEndian"`).  The default branches also carry meaningful fallbacks (`"unknown"`, `"DBCVarType(N)"`) that codegen doesn't provide.  Same design call as AGDA-C-3.1 (FP — `formatChars` follows distinct convention for distinct role): two valid conventions, one per language, both idiomatic.
