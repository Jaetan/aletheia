...
- `[Design-held]` PY-D-19.2 [python/aletheia/protocols.py:97-99] `DBCSignal = DBCSignalAlways | DBCSignalMultiplexed` — **adjudicated design-held** (2026-05-12, cluster 17 / PY-D-19.2, empirically verified): `DBCSignalMultiplexed` already discriminates against `DBCSignalAlways` via its REQUIRED `multiplexor: str` field (absent on `DBCSignalAlways`).  basedpyright narrows on `s.get("multiplexor") is not None` correctly — verified by the clean type-check status of `dbc_queries.py`'s `any_present_signals` / `multiplexed_signals` / `multiplexor_names` / `mux_values` / `signals_for_mux_value` helpers, all of which rely on key-presence narrowing.  Empirical attempt at adding `presence: Literal["multiplexed"]` REQUIRED + synthesising in `normalize_signal` (commit aborted 2026-05-12) broke 2 cross-binding parity snapshots (`kitchen_sink.json`, `multiplexing.json`): the Agda kernel's `formatPresence` (Formatter.agda:80-83) emits `"presence": "always"` for Always but only `"multiplexor"` + `"multiplex_values"` for multiplexed (no `presence`), and Go's `serializeDBC` (go/aletheia/json.go:147-156) matches that asymmetry.  Synthesising on the Python side without propagating to Agda + Go + C++ + the snapshot oracle would break `feedback_cross_binding_wire_symmetry.md`.  Adding the explicit redundant `"presence": "multiplexed"` to all 3 bindings + Agda + the 4 cross-binding parity snapshots is the alternative — substantial scope for a typing improvement that's already provided by key-presence narrowing.  Same flavour of "asymmetry already provides the discriminator naturally" as AGDA-C-6.1 (InContext lift design-held — recursive structure provides the contextualisation API).
