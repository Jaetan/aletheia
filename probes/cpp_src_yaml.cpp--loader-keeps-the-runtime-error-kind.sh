...
yamllib=$(find cpp/build/_deps -maxdepth 2 -name 'libyaml-cpp.a' | head -1)
xlsx=$(find cpp/build -maxdepth 3 -name 'libOpenXLSX.a' | head -1)
[ -f "$lib" ] && [ -n "$yamllib" ] && [ -n "$xlsx" ] || exit 2
...
CPP
clang++-22 -std=c++23 -Icpp/include "$scratch/t.cpp" "$lib" "$yamllib" "$xlsx" -ldl -lpthread -o "$scratch/t" > "$scratch/compile.log" 2>&1 || { tail -5 "$scratch/compile.log"; exit 1; }
