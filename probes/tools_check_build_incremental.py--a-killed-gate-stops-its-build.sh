...
# build that outlives the gate keeps Shake's lock and relinks the library
# with the killed run's marker in it. The probe runs the gate's own build step
# with a stand-in `cabal` first on PATH: the stand-in takes a lock and leaves a
# sleeping grandchild holding it, the shape of `cabal` running `shake`. The
# probe SIGKILLs the process running the build step and reads the grandchild
# and the lock. Every file it writes is under its scratch directory.
...
gate = subprocess.Popen(
    [sys.executable, "-c", "from tools.check_build_incremental import _build; _build()"],
