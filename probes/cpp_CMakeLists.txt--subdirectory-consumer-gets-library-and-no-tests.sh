...
# is missing, a test target leaks into the consumer, or the standalone tree
# registers no tests. Reuses the standalone tree's fetched dependencies.
set -u
cd "$(dirname "$0")/.." || exit 2
here=$(pwd)
...
cmake -S "$scratch" -B "$scratch/build" -DCMAKE_CXX_COMPILER=clang++-22 \
    -DFETCHCONTENT_BASE_DIR="$here/cpp/build/_deps" > "$scratch/configure.log" 2>&1 || { tail -5 "$scratch/configure.log"; exit 1; }
...
[ -d "$here/cpp/build/_deps/catch2-src" ] && [ ! -e "$scratch/build/aletheia-cpp/_deps/catch2-build" ] || true
