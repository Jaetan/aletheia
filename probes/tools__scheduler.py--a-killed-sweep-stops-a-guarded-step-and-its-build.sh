...
# Claim: when the sweep dies, SIGKILL included, a step that runs its own build
# under a guard, as the sweep's `build` step does when it is the staleness
# gate, stops with it, build included, and the lock that build held is free
# within 5 seconds. Each guard reaches only its own process group, so the
# inner build stops only if the outer guard's interrupt or kill reaches the
# gate's own guard in turn. The probe runs two lanes in parallel, one of them
# the staleness gate's build step over a stand-in `cabal` first on PATH: the stand-in takes a lock and leaves a sleeping
...
sweep = subprocess.Popen(
    [
        sys.executable,
        "-c",
        "import sys; from tools._scheduler import Step, run_lanes; "
        + "run_lanes([[Step('build', [sys.executable, '-c', "
        + "'from tools.check_build_incremental import _build; _build()'])], "
