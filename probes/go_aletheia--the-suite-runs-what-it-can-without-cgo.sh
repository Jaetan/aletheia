...
#
# Probes the test files of both packages in go/, the binding and the
# command-line interface.
# Claim: with cgo off both suites build, run the tests that need no kernel, and
# pass. The build without cgo is one the binding supports and its own
...
# interface and skip, and the rest run.
#
...
if [ "$ran" -ne "$expected" ]; then
	echo "without cgo the two suites run $ran tests, and this probe records $expected"
	echo "  a tag added or removed changes this; check which file moved and update the number"
	exit 1
fi
echo "PASS: without cgo both suites pass, running $ran tests of the ones that need no kernel"
