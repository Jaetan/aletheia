...
# Claim: --frames and --runs accept only a positive integer, and a refused
# value is rejected before the mode check, every preflight and the results
# clear, so a file matching the mode's glob survives the refusal; a valid
# count reaches the mode check. Non-zero exit: a bad count was accepted, the
# refusal cleared a result, or a good count was refused.
set -u
cd "$(dirname "$0")/.." || exit 2
dir=$(mktemp -d) || exit 2
trap 'rm -rf "$dir"' EXIT
