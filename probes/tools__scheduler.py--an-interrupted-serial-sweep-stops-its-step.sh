...
# ends a serial sweep and the step it is running within 5 seconds, and frees
# the lock that step held. The sweep runs its `build` step this way, alone and
# on the calling thread, where the interrupt lands inside the wait on the step.
# The probe runs one serial lane, a step running a stand-in
