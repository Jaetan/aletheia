...
# with one named exception: the record's unresolvedValueDescs (also the wire
# key) is unresolved_value_descriptions in C++. The Agda record is read from
...
agda=$(sed -n '/^record DBC : Set where/,/^$/p' src/Aletheia/DBC/Types.agda \
    ...
    | sed -E 's/([A-Z])/_\L\1/g' | sed 's/^unresolved_value_descs$/unresolved_value_descriptions/')
