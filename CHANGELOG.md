...
- **Mutation testing is now a required (merge-blocking) check, and its CI lane is
  cached.** Previously the `mutation testing` lane in `pr-heavy-lanes.yml` ran on
  every PR but was *advisory* — so a PR could merge with mutation red (a new
  survivor / test gap). It now blocks merge: the drift gate is baseline-relative
  (fails only on a collection crash or `observed > baseline`) and the suite is
  deterministic, so it is a stable signal — and the lane already ran on every PR,
  so advisory merely discarded a signal we already paid to compute. The job is
  renamed `mutation testing (advisory)` → `mutation testing` (add it to the `main`
  branch ruleset's required checks to activate). **Caching** removes the two big
  setup costs that made cold runs ~33 min: the workflow now also runs on
  ...
  shared build-tree cache so the `.so` build (338s) is an incremental relink. The
  C++ `-fpass-plugin` mutation compile is deliberately left un-ccache'd (ccache
  cannot content-hash the Mull plugin, which would risk stale mutation objects on
  a required gate). `push: [main]` runs are exempt from `cancel-in-progress` so a
  rapid second merge cannot kill a cache-seeding run before it saves. stability
  bench stays advisory (genuinely timing-flaky).
