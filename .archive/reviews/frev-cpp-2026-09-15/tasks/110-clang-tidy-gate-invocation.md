# Task 110: the clang-tidy gate reads clean when it ran nothing (follow-up from task 097)

- status: completed
- files: `AGENTS.md`, the probe store, and this round's own claims
- pass: measure the gate's two invocations, correct the documented one, probe it
- origin: run from the repository root, `run-clang-tidy-22 -quiet -p cpp/build cpp/src/` prints "No checks enabled. Unable to run clang-tidy." and exits non-zero, because the configuration is found by walking up from the working directory and the root has none. A reader grepping that output for a finding sees none and reads the run as clean. Measured by injecting a discarded nodiscard call into `cpp/src/types.cpp`: from the root the run reports nothing, from `cpp/` the same injection is reported three times, twice as an error about the discarded value. The lint-gate line in AGENTS.md gives the command without the directory it must run from. Every claim this round made about the gate being silent was made with the root-relative form and is unfounded; re-run correctly, the gate had one real finding, an include left unused by an earlier change in this round. Correct the documented command, probe that the gate fails on the defect it exists to catch, and say plainly in the record which claims were vacuous.

## Report

Measurement, correction, probe, and a correction to this round's own record. Fix in refs/frev/110 (signed later by the dribble).

Claims and guards: the gate's claim is that it reports what the configuration tells it to report. That holds only when the configuration is in scope, and clang-tidy finds it by walking up from the working directory, not from the build tree it is pointed at. Run from the repository root, which has no configuration, the driver prints that no checks are enabled and exits non-zero, and its output carries no finding. A reader who greps that output for a finding sees a clean run.

Measured both ways with the same injection, a discarded value from a function the round had just marked, placed in a library source. From the binding's directory the gate reports it three times, twice as an error about the discarded return. From the repository root the same tree reports nothing at all.

Findings fixed. The lint-gate line in the coding standard gave the command without the directory it must run from; it now gives both, and says what the wrong directory looks like. The probe injects the same defect, requires the gate to name it, and separately requires a run with no configuration in scope to still say so rather than resemble a clean run. Disabling the two checks that catch a discarded return makes the probe red.

The correction to the record, which matters more than either. Every statement this round made about the gate being silent over the library sources was produced by the root-relative form and is unfounded: the tasks for the Excel loader comment, the wire-integer range check, the language standard, the shared scale factor, the shared renderer, the reader rename and the em-dash sweep all carry such a line. Re-run correctly, the gate had one real finding, an include the scale-factor move had left unused in the enrichment renderer, fixed in the snapshot before this one. The round-end record runs the gate the right way and its output is the audit.

```
REPORT 2026-09-15 tree refs/frev/097g fix in refs/frev/110
claims: 1 row, 1 without a guard: that the gate reports what it is configured to report, now injected and measured
1 line per line: checked, the lint-gate line and the gate's own invocation in the orchestrator read whole
2 guidelines: n/a
3 modernize: n/a
4 catalogue: checked, the working-directory rule is the tool's own configuration-discovery behaviour, confirmed by running it both ways
5 value semantics: n/a
6 raii: n/a
7 dedup: checked, the orchestrator already runs the correct form, so the standard now agrees with it rather than restating it differently
8 ground truth: finding, the documented command omitted the one thing that decides whether it does anything
9 history: checked, none
10 simpler: checked
11 comments: the standard's lint line grows from one clause to two sentences, which is the enumeration this task exists to complete
sweep: the gate runs clean over cpp/src from the binding's directory, with one finding found and fixed on the first correct run
probes: the new gate-teeth probe passes and read red with the two checks disabled
