# Task 109: the build file's recipes disagree about where they are run (follow-up from task 097)

- status: completed
- file: `cpp/CMakeLists.txt`
- pass: lenses and diff, over every command the file's comments give
- origin: the file gives five build recipes in its comments and they do not agree on the directory a reader is standing in. The clang-tidy, address, undefined, thread and mutation lanes all write a bare build directory, which resolves only from inside `cpp/`. The fuzz lane was the odd one and was corrected while giving that recipe one owner, but it was corrected to the repository root, so the file now has four of one and one of the other. Settle the file on one, correct the odd ones, and stage it by running each recipe from the directory the comment implies.

## Report

Lenses and diff pass over every command the file's comments give. Fix in refs/frev/109 (signed later by the dribble).

Claims and guards: a recipe in a comment is a claim that those lines run. The file gives five, and four write a bare build directory, which resolves only from inside the directory the file lives in. The fifth, the fuzz lane, had just been corrected to the repository root while its recipe was given one owner, so the file held four of one convention and one of the other, and a reader following two of them in a row would be standing in the wrong place for the second.

Finding fixed: the fuzz recipe is written from the same directory as its four neighbours, and its seed path with it. The three lines were run from there.

The probe reads the file's own documented commands and refuses any that names a path through the binding's directory, which is the shape a root-relative command takes here, while requiring at least one configure line to name a bare directory so the check cannot pass vacuously on a file that documents nothing. Rewriting the mutation lane to the repository root makes it name that line and fail.

The recipe probe from the previous task moves with it: it now runs the three lines from the directory the comment implies, so the convention and the commands are checked together rather than one being assumed.

```
REPORT 2026-09-15 tree refs/frev/097f fix in refs/frev/109
claims: 5 rows, 1 without a guard: the fuzz lane, whose directory disagreed with the other four; the probe now covers all five
1 line per line: checked, every comment line carrying a command read
2 guidelines: n/a, a build file
3 modernize: n/a
4 catalogue: n/a
5 value semantics: n/a
6 raii: n/a
7 dedup: checked, the recipe still has one owner and the build file still points at it
8 ground truth: finding, one recipe named a directory a reader following the others would not be in; the three lines were run from the corrected one
9 history: checked, none
10 simpler: checked
11 comments: 18 to 18, code 14 to 14 in the harness; the build file is unchanged
sweep: no mutation names either file; fifteen suites green
probes: the six naming the build file all pass, and the new convention probe read red with one lane rewritten to the root
