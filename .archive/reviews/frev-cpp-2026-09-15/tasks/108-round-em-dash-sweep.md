# Task 108: the round's own added em-dashes (follow-up from task 097)

- status: completed
- files: every file this round wrote a line into that carries an em-dash
- pass: mechanical sweep over the round's diff, then a probe
- origin: the review contract forbids an em-dash in anything the round writes, and the round wrote twenty-nine lines carrying one. Measured by diffing the round base against the current tree, taking every added line with U+2014 and checking each against the base file: none is byte-identical to a base line, so every one was written or rewritten during the round. They span twelve source and configuration files, the C++ API guide, and the limits parity gate. One of them is a runtime message rather than a comment. Replace each, and add a probe so the next round measures the rule instead of trusting it.

## Report

Mechanical sweep, then a probe. Fix in refs/frev/108 (signed later by the dribble).

Claims and guards: the contract forbids an em-dash in anything a round writes, and nothing measured it. The measurement is the round's own diff against the tree it started from, taking every added line carrying the character and keeping only those that match no line of the same file in that tree, which is what separates a line the round wrote from one a neighbouring edit re-emitted unchanged. Twenty-nine lines, none of them inherited.

Findings fixed: twenty-nine replacements, each reworded so the sentence reads without the character rather than having it swapped for another mark. Twelve lie in source and configuration comments, eight in the C++ API guide, where the reflow rewrote the paragraph that carried them, and nine in the limits parity gate, whose docstring this round rewrote. One is not a comment: the empty-library-path refusal the backend throws, which now separates cause from remedy with a colon. No test reads that message, checked by grep over the tree.

Three of the replacements pulled in a neighbouring line the round did not write, because the two form one sentence or one list and fixing half would leave a dangling mark: the sentence about which workbook cells are text, the mirror list in the gate's docstring, and the gate's exit-code list.

The probe measures the rule rather than trusting it. It compares the worktree against the anchored review base, applies the same written-versus-inherited test, and names each offender. It skips when no base is anchored, since the claim is untestable then. Teeth: putting the character back into one comment makes it name that line and fail.

```
REPORT 2026-09-15 tree refs/frev/097c fix in refs/frev/108
claims: 1 row, 1 without a guard: the round writes no em-dash, now measured by the new probe
1 line per line: checked, each of the twenty-nine lines read in its own context before replacing
2 guidelines: n/a, comments and one message string
3 modernize: n/a
4 catalogue: n/a
5 value semantics: n/a
6 raii: n/a
7 dedup: n/a
8 ground truth: checked, every replacement re-read against the sentence it sits in; the message string has no test reading it
9 history: checked, none
10 simpler: checked
11 comments: unchanged line counts, the edits are within lines
sweep: fifteen suites green, the doc harness green, the eleven fast-tier steps green, ruff clean and the limits gate green
probes: the new rule probe passes and read red with one character restored; the ten probes naming the touched files all pass
