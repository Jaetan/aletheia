# Task 001: file review of `cpp/.clang-format`
...
Full pass. Fix landed in the snapshot after tree b222b613 (signed later by the dribble; the tree carrying the fix is refs/frev/001).

Claims: 61 option keys, each a claim that every source under cpp/ conforms to it; guard for all 61 is the clang-format gate (`clang-format-22 --dry-run -Werror`, red at base if any source deviated: exit 0 recorded in base/gates2.txt). Three keys carried a second, unguarded claim, that the key name itself is one the tool still honours: AlwaysBreakTemplateDeclarations, SpaceInEmptyBlock, SpacesInParentheses are deprecated aliases in clang-format 22 (absent from the tool's own `--dump-config`, which prints BreakTemplateDeclarations, SpaceInEmptyBraces, SpacesInParens). Guard added: probes/cpp_.clang-format--keys-canonical.sh, red before the fix on exactly those three, green after.

```
REPORT 2026-09-15 tree b222b613 fix in refs/frev/001
claims: 61 rows, 3 without a guard: probe cpp_.clang-format--keys-canonical.sh added
1 line per line: checked, all 84 lines read; every key asked whether it changes the base style (17 do, 40 restate it, 3 were aliases, BasedOnStyle and Language frame the rest)
2 guidelines: n/a, not C++; the file's own tool is the standard
3 modernize: finding, three deprecated option names renamed to the clang-format 22 canonical names; gate: `--dump-config` before and after compared byte-identical with cmp, format gate still exit 0
4 catalogue: checked, AGENTS/cpp.md names the clang-format gate as a hard requirement and nothing more specific about the config; the tool's dump is the catalogue used
5 value semantics: n/a
6 raii: n/a
7 dedup: decision point, 40 keys restate the LLVM default (measured, appended to the accumulator); no in-file clone
8 ground truth: checked, the three comment claims hold against the values (Attach is the K&R brace style, Left gives `Type* name`, IncludeBlocks Preserve with SortIncludes sorts within groups)
9 history: checked, none
10 simpler: decision point, see 7
11 comments: 10 to 10, code 64 to 64
sweep: no mutation names this file; format gate exit 0 after the edit
probes: probes/cpp_.clang-format--keys-canonical.sh added, store 1 run 1 pass
