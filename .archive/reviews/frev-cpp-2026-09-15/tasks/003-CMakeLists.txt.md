...
Correction found during task 005: the first version of the subdirectory-consumer probe pointed FetchContent's base directory at the standalone tree's own _deps, which rewrote the json, yaml-cpp and OpenXLSX sub-builds and left the standalone tree unable to rebuild the benchmark target; the gate run above used `tail -1` on the build output and so did not see the failing sub-target. The probe now names the fetched source directories through FETCHCONTENT_SOURCE_DIR_* and never writes into the standalone tree; the polluted sub-builds were removed, the tree reconfigured, and the full build (exit 0, zero errors in the log) and ctest (15 of 15) re-run on the edited CMakeLists.txt, which is the evidence the report's build line now rests on.
