# Task 107: file review of `cpp/tests/test_feature_matrix_parity.cpp` (follow-up from task 097)

- status: completed
- file: `cpp/tests/test_feature_matrix_parity.cpp`
- pass: lenses and diff, over the lexical stripper plus whatever the lenses fire on
- origin: the stripper that blanks comments and literals before the whole-word symbol search reads a C++ digit separator as a character-literal delimiter, so `1'000` opens a literal that runs to the next apostrophe and every symbol in between is blanked. Measured while adding one constant to `cpp/include/aletheia/types.hpp`: with the constant present the gate reports `Dlc`, `dlc_to_bytes` and `Frame` missing from the header that declares all three, and removing the constant makes them visible again. The gate therefore fails on a correct tree, and which symbols it hides depends on how many apostrophes happen to precede them. Tell a separator from a quote, and stage the fix by showing the header resolves with the constant in place and a genuinely absent symbol still fails.

## Report

Lenses and diff pass over the lexical stripper. Fix in refs/frev/107 (signed later by the dribble).

Claims and guards: the stripper's comment says it blanks comments, string literals and character literals so a stale comment cannot satisfy the whole-word search. It also blanked arbitrary live code, because a C++ digit separator is an apostrophe and the stripper read it as the start of a character literal. The literal then ran to the next apostrophe anywhere in the file, and everything between was replaced with spaces. Nothing tested the stripper, so the claim had no guard.

How it surfaced, and the measurement: adding one constant with a digit separator to the header the matrix cites made the gate report three symbols missing that the header declares, and removing the constant made them visible again. Which symbols vanish depends on how many apostrophes happen to sit before them, so the gate fails on a correct tree and the failure moves as unrelated code changes.

Finding fixed: an apostrophe between two identifier characters whose left-hand run begins with a digit is a separator, not a quote. The run test is what tells it from an encoding prefix, since an apostrophe in a prefixed literal also has a digit immediately to its left.

The guard is a case on the stripper itself, which the file had none of. It pins both directions: a separator leaves the following symbol visible, in decimal and in hexadecimal; a plain character literal, a prefixed one, and a comment are all still blanked. Without the fix four of its assertions fail; with it the whole binary is green over its thirteen hundred assertions.

```
REPORT 2026-09-15 tree refs/frev/094 fix in refs/frev/107
claims: 1 row, 1 without a guard: what the stripper blanks and what it leaves, now pinned by a case on the stripper
1 line per line: checked, the stripper, the whole-word search and the entry loop read whole
2 guidelines: checked, the new predicate takes the text by const reference and returns a bool
3 modernize: checked, the branch reads as one condition rather than a special case inside the loop body
4 catalogue: checked, the separator grammar is the language's own: digits, optionally grouped, with an apostrophe between two of them
5 value semantics: checked, the position is passed by value and the text by const reference
6 raii: n/a
7 dedup: checked, the existing identifier-character predicate is reused rather than re-spelled
8 ground truth: checked, the stripper's comment describes what it now does
9 history: checked, none
10 simpler: checked
11 comments: 28 to 39, code 175 to 194
sweep: no mutation names this file; fifteen suites green and clang-tidy silent over the file
probes: none name this file; the new case is the guard and read red without the fix
