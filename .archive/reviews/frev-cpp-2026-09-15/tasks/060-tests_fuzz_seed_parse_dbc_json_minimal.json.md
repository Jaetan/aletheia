...
Full pass. Fix in refs/frev/060 (signed later by the dribble).

Claims and guards: the seed claims to be a shape the DBC JSON parsers accept, and driving it through both parsers the harness calls confirms it: parse_dbc_response and parse_parsed_dbc each return a value on it, and parse_success does too. The file itself is correct and unchanged.

Finding fixed: it was the whole corpus, and its messages array is empty, so the fuzzer's only starting point carries no message and no signal. Mutation does not synthesize a valid signal object from an empty array, so the nested parsing the harness exists to cover was reached only by accident. A second seed now carries one message with one signal, generated by the binding's own serializer so that every member name and shape is the wire's rather than a transcription, and it is accepted by the same parsers. Measured over 61 seconds on the same target, the two-seed corpus adds 235 new units against 36 from the minimal seed alone, with no crash.

```
REPORT 2026-09-15 tree b222b613 fix in refs/frev/060
claims: 1 row, 0 without a guard: the seed's acceptance, measured through both parsers the harness calls
1 line per line: checked, the whole 73-byte document read; a success envelope, an empty message list and an empty warning list
2 guidelines: n/a, not C++
3 modernize: n/a
4 catalogue: checked, the shape matches what serialize_parsed_dbc_response emits
5 value semantics: n/a
6 raii: n/a
7 dedup: checked, the new seed differs from this one in exactly what it adds
8 ground truth: checked, both parsers accept it; the new seed is the serializer's own output rather than hand-written
9 history: n/a
10 simpler: checked, this seed is already the smallest accepted envelope, which is why it stays
11 comments: n/a for a JSON fixture, 0 to 0, code 1 to 1
sweep: no mutation names this file; the fuzz target runs 61 seconds over the two-seed corpus with no crash
probes: none name this file; acceptance measured directly
