...
### Correction, made while working task 080

Task 080 found `format_formula` in the public enrichment header, which renders a whole formula including both predicates, the signal names and the deadline. The three when/then cases had been given a hand-rolled visitor over the formula variant instead, about forty lines of it, which point 10 rules out when a simpler implementation exists and which could drift from the variant it walks. The visitor is gone and the three cases compare the rendered formula, which is a stronger assertion than the pair of predicate names it replaces. Teeth re-proven on the when side this time, by making the loader dispatch a when-exceeds to drops-below: one case fails. The suite runs 59 cases and 192 assertions, against 186 at the round base. Measured after the correction: comments 128 to 127, code 789 to 791.
