# Task 093: file review of `.gitignore` (follow-up from task 003)

- status: completed
- file: `.gitignore`
- round base: b222b613 (2026-09-15)
- pass: full
- origin: `cpp/CMakeLists.txt` offers a thread-sanitizer lane whose documented build directory is `cpp/build-tsan`; the ignore list names build/, build-tidy/, build-asan/, build-ubsan/, build-fuzz/ and build-mutation/ but not build-tsan/, so a tsan configure leaves an untracked tree. The review of the ignore file is otherwise the full contract.

## Report

Full pass. Fix in refs/frev/093 (signed later by the dribble).

Claims and guards: the file's claims are its patterns, and each one claims to cover something. Redundancy was measured rather than read: each candidate line was deleted and git was asked which pattern then wins for a representative path.

Findings fixed, four of them.

The thread-sanitizer tree was missing. The build file tells a reader to configure three sanitizer lanes and the ignore list named two, so a thread run left an untracked tree. It is listed with its siblings.

The single-venv comment stated a rule the pattern contradicted. It said a stray venv should surface in git status rather than be silently ignored, and the bare pattern hid every venv at every depth, the stray root one included. The pattern is now anchored to the one sanctioned path, which is what the comment already said the project wants, and the on-disk gate that walks for a venv configuration file is unaffected either way. The current tree gains no untracked path from the change: the untracked set is identical before and after.

Four patterns were redundant, each proven by deleting it and watching another pattern still win. The Python section repeated the build and distribution directories the build-artifacts section already covers. The benchmarks section named the C++ build tree, which the top-level pattern covers at any depth and which had nothing to do with benchmarks. The examples section named a log glob and a single log file, both covered by the repository-wide one. Removing the example glob leaves the tracked demo trace un-ignored, which was checked directly, because the negation that re-includes it follows the pattern that hides it.

The fetched copy of Agda's own grammar sat under a comment about a lock file and now carries its own line saying what it is and that it is fetched rather than vendored.

A measurement note: the round's line-count lens has no rule for a file named `.gitignore`, so it counts every line as code. The counts below are the file's own hash lines against its pattern lines. No file under the reviewed directory is one of these, so the round's tables are unaffected.

```
REPORT 2026-09-15 tree refs/frev/106 fix in refs/frev/093
claims: 5 rows, 2 without a guard: the sanitizer trees and the venv rule, both now guarded by the new probe
1 line per line: checked, all 97 lines read and every pattern resolved against a representative path
2 guidelines: n/a, not C++
3 modernize: checked, the venv pattern is anchored so it names one path rather than any directory of that name
4 catalogue: checked, anchoring with a leading slash and a trailing slash is the documented way to name one directory
5 value semantics: n/a
6 raii: n/a
7 dedup: finding, four patterns were covered by another and are gone, each proven by deletion
8 ground truth: finding, the venv comment described a behaviour the pattern did not have
9 history: checked, none
10 simpler: checked
11 comments: 31 to 32, patterns 66 to 62
sweep: no mutation names this file; the eleven fast-tier steps pass and the venv-convention gate is clean
probes: the new build-tree and venv probe passes, and read red against the previous file, naming the missing sanitizer tree and the three hidden venv paths
