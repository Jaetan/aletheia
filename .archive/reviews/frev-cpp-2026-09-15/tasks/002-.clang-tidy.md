# Task 002: file review of `cpp/.clang-tidy`
...
Full pass. Fix in refs/frev/002 (signed later by the dribble).

Claims: 32 disabled checks each with a stated reason, WarningsAsErrors, HeaderFilterRegex, 21 naming options, 3 thresholds, StrictMode, IgnoreHeaders. Guards: the tidy gate over cpp/src (0 diagnostics before and after, base/gates2.txt and base/tidy_after_task002.txt) for every option that shapes a diagnostic in library code; new probes for the claims nothing ran: the header's reason list matches the Checks list (red before the fix on the two re-enabled checks the history block still named), every header uses #pragma once (the stated reason for two disables), and the three CheckOptions semantics. Five disables carry reasons that hold only for test code the gate never lints (measured: zero findings over cpp/src when re-enabled); decision point.

Findings fixed: (a) concurrency-mt-unsafe's reason said test-only code; re-enabled over cpp/src it fires 9 times on std::getenv and dlerror in ffi_backend.cpp, rational_renderer.cpp and cli.cpp, so the reason now names those. (b) The bugprone-argument-comment.StrictMode comment said comments are required on positional arguments; the probe shows an uncommented call is not diagnosed and only a mismatching comment is. (c) The cppcoreguidelines-init-variables.IncludeStyle comment said the option zero-initialises fundamentals and silences the check; the probe shows identical diagnostics under google and llvm, so the option and its comment are removed (effective config then differs only in that fix-it include style, which reverts to the default llvm; the gate is unchanged). (d) readability-implicit-bool-conversion.AllowIntegerConditions and AllowPointerConditions restated the defaults (probe: both conditions are diagnosed with no options set) under a comment describing the opposite; removed. (e) Three history blocks (the reinterpret_cast re-enable story, "previously disabled, now re-enabled", "tightened, prior values") removed; the reinterpret_cast one was also stale, nine such casts sit in two files, not three helpers in one. (f) CheckOptions moved from the key/value list form to the map form the tool dumps.

```
REPORT 2026-09-15 tree b222b613 fix in refs/frev/002
claims: 60 rows, 5 without a guard: 5 probes added (header list vs Checks, pragma once, StrictMode semantics, IncludeStyle inertness, implicit-bool defaults)
1 line per line: checked, all 185 lines read; every disable asked whether its reason holds for the linted surface, every option whether the comment above it describes what the option does
2 guidelines: n/a, not C++; the tool's semantics are the standard
3 modernize: finding, CheckOptions brought to map form; gate: full --dump-config compared before and after, sole difference the removed IncludeStyle reverting to its default; tidy gate over cpp/src 0 diagnostics after
4 catalogue: checked, AGENTS/cpp.md names the tidy gate and the NOLINT rule; the check inventory of clang-tidy 22 confirms all 32 disabled names or globs exist (scratch inventory of 605 checks)
5 value semantics: n/a
6 raii: n/a
7 dedup: finding, the reason list and the Checks list are a forced pair (a folded scalar cannot carry comments), so the pair is now probed rather than trusted
8 ground truth: finding, three option comments false (b, c, d above) and one disable reason false (a); checked true: 25 of 25 headers carry #pragma once, no using-namespace in src, ALETHEIA_CLANG_TIDY exists in CMakeLists.txt, anonymous namespaces in src hold what the enabled llvm-prefer-static check tolerates
9 history: finding, three blocks removed (e above)
10 simpler: checked, map form (f); decision point on the five inert disables
11 comments: 85 to 73, code 96 to 64
sweep: no mutation names this file; tidy gate over cpp/src exit 0 with 0 diagnostics after the edit
probes: 5 added under probes/cpp_.clang-tidy--*.sh, store 6 run 6 pass
