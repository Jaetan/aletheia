# Task 123: the test suites read as assertions rather than as behaviour

- status: pending
- files: every source under `cpp/tests/`, the coding standard's test section
- pass: full, across the directory
- origin: Requested. The suites are written as flat `TEST_CASE` bodies of assertions; the framework already in the tree ships `SCENARIO` / `GIVEN` / `WHEN` / `THEN` / `AND_THEN`, and no test file uses any of them. Move the suites to that phrasing so a reader sees the behaviour under test and the condition it holds under, instead of a list of checks.

## What is measured before anything is written

- the count of test cases per file and the count of `SECTION` uses, so the conversion has a denominator
- which test cases already have a natural given/when/then shape and which are parameter tables, since a table is not a scenario and stays as it is
- what the phrasing costs: `GIVEN` / `WHEN` / `THEN` expand to sections, so a body that was one pass becomes one pass per leaf, and any test whose setup is expensive or whose state is shared across what become separate leaves changes meaning silently
- the effect on the lint surface, since the disables in the tests' own configuration are counted against the current macro use

## What the task owes

- a per-file plan naming which cases convert and which do not, with the reason
- the conversion, in file-sized steps, each with the suite run before and after and the assertion count unchanged
- a guard that a converted case still fails when the behaviour it names is broken: for each converted file, one mutation of the code under test that the converted case kills
- the coding standard's test section states the phrasing the suites use, so a later test is written in it
- no change to what is asserted: this task moves phrasing, never coverage
