...
- `clang_tidy_src.txt`: the lint gate as it now runs, `run-clang-tidy-22 -quiet -p build cpp/src/ cpp/tests/`
  from `cpp/`, 45 translation units, no finding. At the base the same gate ran over `cpp/src` alone,
...
- `clang_tidy_tests_summary.txt`: the split that ruling asked for, tests from benchmarks, taken before
  anything was decided about either.
...
- `linecount.tsv`: comment and code lines per tracked file under `cpp/`. Code 17404 to 17698, comments
  4737 to 4965, ratio 0.27 to 0.28.
...
The eleven rulings landed as fourteen tasks. The library is built shared, so an installed consumer links
...
sources are inside the lint gate with a configuration of their own.
