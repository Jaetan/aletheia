...
[[nodiscard]] auto load_checks_from_excel(const std::filesystem::path& path,
...
[[nodiscard]] auto load_dbc_from_excel(const std::filesystem::path& path,
                                       std::string_view sheet = "DBC") -> Result<DbcDefinition>;
...
[[nodiscard]] auto create_excel_template(const std::filesystem::path& path) -> Result<void>;
