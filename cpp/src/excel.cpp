...
auto parse_simple_row(const CellMap& cells, int row_num) -> CheckResult {
    ...
    CheckResult result = [&]() -> CheckResult {
        if (detail::is_simple_value_condition(condition)) {
            auto value = PhysicalValue{get_number(cells, "Value", row_ctx(row_num))};
        ...
        if (detail::is_simple_range_condition(condition)) {
            ...
            auto lo = PhysicalValue{get_number(cells, "Min", row_ctx(row_num))};
            auto hi = PhysicalValue{get_number(cells, "Max", row_ctx(row_num))};
...
auto parse_when_then_row(const CellMap& cells, int row_num) -> CheckResult {
    ...
    auto when_value = PhysicalValue{get_number(cells, "When Value", row_ctx(row_num))};
    ...
    CheckResult result = [&]() -> CheckResult {
        if (then_cond == "equals") {
            auto val = PhysicalValue{get_number(cells, "Then Value", row_ctx(row_num))};
        ...
        auto lo = PhysicalValue{get_number(cells, "Then Min", row_ctx(row_num))};
        auto hi = PhysicalValue{get_number(cells, "Then Max", row_ctx(row_num))};
...
auto parse_dbc_signal(const CellMap& cells, int row_num) -> DbcSignal {
    ...
    return DbcSignal{
        ...
        .offset = RationalOffset{detail::double_to_rational(get_number(cells, "Offset", ctx_str))},
        .minimum = RationalBound{detail::double_to_rational(get_number(cells, "Min", ctx_str))},
        .maximum = RationalBound{detail::double_to_rational(get_number(cells, "Max", ctx_str))},
        .unit = Unit{unit_str},
        .presence = presence,
    };
}
