...
// Command aletheia-cli is the C++ host CLI for the Aletheia CAN signal
...
// dispatching to the real verified Agda core through the dlopen client
...
auto main(int argc, char** argv) -> int {
    try {
        ...
        const std::span<const std::string> all{args};
        return dispatch(args.front(), all.subspan(1));
    } catch (const std::exception& e) {
