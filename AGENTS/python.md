...
8. **Strong type usage** -- sub-checks:
    ...
    - (c) **A hint carries the shape of the data, as faithfully as the language allows** (user rule, 2026-09-20): `dict[str, list[str]]`, `tuple[str, int, str]`, `list[str]` and their like are refused wherever the function does not take *any* string or *any* such mapping, which is nearly everywhere. Naming the parts is the first step, not the goal: a `type` alias to `str` documents a role and checks nothing. The goal is a type whose values are exactly the values the data can take. A closed set of words is an `Enum`/`StrEnum` or a `Literal`, never a `str`; a string that must satisfy a rule is a `NewType` minted only by the function that checks the rule; a composite whose parts have distinct roles is a `NamedTuple` or a frozen dataclass with its invariant in the docstring; a JSON shape the project authors is a `TypedDict`; a capability a function needs from its argument is a `Protocol`; a value that is one of several shapes is a union of those shapes, narrowed where it is read. The test is reading the signature alone: a reader who cannot say what each part means, and what values it can hold, without opening the body has found a finding. Worked example: `tools/_ratchet.py`'s `RowKey` and `RatchetRows`, and `tools/check_changelog.py`'s `Category` enum, shared by both C++ ratchet gates.
