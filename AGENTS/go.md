...
14. **Test adequacy** -- sub-checks:
    ...
    - (g) **Mutation testing**: a mutation pass over the hot-path sources `docs/MUTATION_BENCH.yaml` lists, with the tool it names, which is `gremlins`: the tools this category named before it are unmaintained and one of them panics on a current Go, and that file records the substitution beside the baseline a run produced. Surviving mutants on operational logic (not on log-string formatting, comment-tagged unreachable branches, or pure metadata/version strings) are findings. Justify any survivor with a comment block at the source site naming why the mutant is equivalent or unreachable; an unjustified survivor is a test gap. Mutation testing runs as a lane of its own, once per pull request, its cost being what it is.
...
22. **Cross-layer alignment** -- does the Go binding correctly mirror the Agda core's semantics? All bindings (Python, C++, Go, Rust) must have identical behaviour, and any divergence is a finding. **Response contract audit**: for every client method, trace the response lifecycle: (a) what status strings can the Agda core return for this operation? (b) does the binding's parser accept all of them? (c) does the client method actually call the parser (not discard the response)? A parser that accepts `"success"` but not `"ack"` for an operation where the Agda core returns `"ack"` is a finding. A client method that calls a backend method and discards the response string with `_` is a finding: a protocol error from the Agda core would be lost. **Concrete cross-binding comparison**: for each of the 13 `Backend` interface methods, compare: (1) what the Python client does with the response, (2) what the C++ client does, (3) what the Go client does, (4) what the Rust client does. Any behavioral divergence is a finding. Python is the reference implementation, and its `_ACK_BYTES` fast path in `send_error`/`send_remote` is the gold standard for event response handling
...
24. **`nil` & zero-value discipline** -- typed nil is not interface nil: `var e *Error = nil; var i error = e` satisfies `i != nil`. `nil` map writes panic; callers must initialize with `make(map[K]V)` before first write. `nil` slice vs empty slice differ in JSON output, `null` against `[]`, and cross-binding parity requires one of them. Zero-value usability is a Go idiom: `&Client{}` must either work or fail loudly, never silently misbehave.
...
33. **Dynamic correctness analysis (sanitizers, fuzzers, property-based, cross-binding integration)** -- four sub-lanes that each cover correctness surfaces unit tests cannot reach:
    ...
    - (b) **Native Go fuzz tests** (`testing.F`): one target per parser or serializer the binding owns, under the names `FuzzParseResponse`, `FuzzMarshalCommand`, `FuzzDecodeBinaryFrame`, `FuzzParseRationalNumber` and `FuzzParseDBCJSON`. Each seeds its corpus inline with `f.Add` and asserts that no input panics; what a parser accepts and refuses is the ordinary tests' subject. The ordinary suite runs every seed and fuzzes nothing. Fuzzing is a command, one target at a time: `go test -fuzz=FuzzParseResponse -fuzztime=60s ./aletheia/`. A crash it finds is committed minimised under `go/aletheia/testdata/fuzz/<target>/`, beside the fix it guards, which is what that directory holds.
    - (c) **Property-based tests** (`testing/quick`): round-trip properties for every wire-format encode/decode pair (frame encode → decode → frame, response marshal → parse → response, command marshal → parse → command); parity invariants for every `Backend` method (the response field-set returned by `MockBackend` and `FFIBackend` must agree over generated inputs). A binding-side data type without a round-trip property is a finding under cat 14 (b) and (c) together, mock fidelity and property coverage being the two halves.
    - (d) **Cross-binding integration test**: three tests, one per binding, build identical canonical inputs in code and assert the structural invariants `docs/architecture/PROTOCOL.md` documents (field presence, value types, counts, error-code identity), so a binding's drift from the protocol shows in its own suite and parity is transitive through the document; there is no shared corpus and no pairwise diff. The Go test is `go/aletheia/cross_binding_integration_test.go`, built under `cgo && linux` and run by the ordinary suite; its counterparts are `python/tests/test_cross_binding_integration.py` and `cpp/tests/test_cross_binding_integration.cpp`. A case one binding carries and another lacks is a parity finding.
