...
-- R21 cluster 9 — AGDA-D-15.1 split (Phases 1-3 extracted):
-- Phase 1 (digit-character arithmetic + list-level lemmas),
-- Phase 2 (`many (satisfy P)` prefix reader), and
-- Phase 3 (parseNatural roundtrip + canonicalisation extractors +
-- arithmetic bindings for Phase 4) live in dedicated submodules;
-- re-exported here so downstream consumers see the same names at
-- this module.  See each submodule header for the phase organisation.
