...
-- R21 cluster 9 — AGDA-D-15.1 split (Phases 1-2 extracted):
-- Phase 1 (digit-character arithmetic + list-level lemmas) and
-- Phase 2 (`many (satisfy P)` prefix reader) live in dedicated
-- submodules; re-exported here so downstream consumers see the
-- same names at this module.  See each submodule header for the
-- phase organisation.
