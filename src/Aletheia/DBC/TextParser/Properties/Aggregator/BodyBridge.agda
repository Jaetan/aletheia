...
-- B.3.d Layer 4c — Body bridge: `formatChars-body d` (the 6-section
...
--      constructor `TX`.  6 instances, each closes by structural
--      induction + the `emitTopStmt-chars` constructor case.
--   3. Compose 1 + 2 over the 6-section concat to get
...
open import Aletheia.DBC.TextFormatter.ValueTables  using (emitValueTable-chars; emitValueTables-chars)
...
-- The whole-body bridge.  Decomposes via 5 applications of `foldr-
-- emit-app` (one per `++` boundary in `toTopStmtsTyped`), then 6
...
formatChars-body-bridge :
  ...
  → foldr (λ t acc → emitTopStmt-chars (collectDefs (DBC.attributes d)) t ++ₗ acc) []
          (toTopStmtsTyped d)
    ≡ formatChars-body d
formatChars-body-bridge d = step6
  where
    ...
    step1 :
      ...
      ≡ foldr (λ t acc → emitTopStmt-chars defs t ++ₗ acc) [] (map TVT vts)
        ++ₗ foldr (λ t acc → emitTopStmt-chars defs t ++ₗ acc) []
              (map TM msgs ++ₗ map TEV evs ++ₗ map TCM cms ++ₗ map TAT attrs ++ₗ map TSG sgs)
    step1 =
    ...
    -- Abbreviations for the 6 chunks (typed-shadow folds).
    ...
    -- After 5 applications of foldr-emit-app, the LHS reshapes into a
    -- right-associated 6-chunk concatenation.
    distrib :
        foldr (λ t acc → emitTopStmt-chars defs t ++ₗ acc) []
              (toTopStmtsTyped d)
    ...
    convert =
      trans
        (cong (λ x → x ++ₗ chunkM ++ₗ chunkEV ++ₗ chunkCM ++ₗ chunkAT ++ₗ chunkSG)
              (emit-map-TVT-eq defs vts))
