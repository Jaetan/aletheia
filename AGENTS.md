...
3. **Lint gates**: `basedpyright`, `pylint`, `go vet`, `gofmt -l`, `clang-format --dry-run -Werror`, and `run-clang-tidy-22 -quiet -p build cpp/src/` **from `cpp/`**: clang-tidy finds `.clang-tidy` by walking up from the working directory, so the same command run from the repository root enables no checks, says so on stderr, and looks clean to anything grepping its output for a finding.
