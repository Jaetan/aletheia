...
> **Note on `memory/*.md` pointers.** This file and the `AGENTS/*.md` per-language files reference many `memory/<name>.md`, `feedback_*.md`, and `project_*.md` files. These are pointers into Claude Code's private agent-memory store under `~/.claude/` (**outside this repository**) — they do not resolve in a repo checkout and are not user-facing documentation. They record the rationale/worked-example behind a rule for the agent; the load-bearing rule itself is always stated inline here.
...
**Cross-binding feature parity is mechanical, not manual.** `docs/FEATURE_MATRIX.yaml` is the authoritative declaration of every user-facing capability and its per-binding status (`implemented` / `not_applicable` / `planned`). Four structural gate tests — `python/tests/test_feature_matrix_parity.py`, `go/aletheia/feature_matrix_test.go`, `cpp/tests/test_feature_matrix_parity.cpp`, `rust/tests/feature_matrix.rs` — parse the YAML, validate the schema, and for every `implemented` row verify the stated entry resolves on its binding (Python via `importlib` + recursive `getattr`, Go via `go/ast` source parsing, C++ via header + whole-word symbol match, Rust via whole-word symbol match over `rust/src/`). They run inside each binding's default test battery, so a silent removal or rename of a public symbol fails the build the same way a broken test does. When adding, removing, or renaming a user-facing capability, updating the YAML is part of the change, not a follow-up. See `PROJECT_STATUS.md` for the roadmap and status.

**A cross-binding finding is a finding about the *whole feature*, not one binding.** When a review (or a reviewer comment) flags behavior in one binding (Python / Go / C++ / Rust), pull the *same feature in every binding* and judge the **set** before deciding. After reading **all** peers (never conclude from one — "consistent" is unprovable from a sample of one, and a bug can live in several), the disposition is one of three: **(a) peers diverge** → a parity bug; fix all in lockstep (per "no subsumption asymmetry"); **(b) peers consistent *and* correct** → FP, record the citation chain; **(c) peers consistent *but* wrong** → a real bug in *every* binding; fix all, even though nothing looks "divergent". Consistency is agreement, not correctness — all bindings can be consistently wrong (copied from each other, or all built on one flawed assumption). The "(c) wrong-by-principle" call cannot be read off peer behavior (that is circular); it is judged against the *spec* — this file, `docs/architecture/DESIGN.md` / `PROTOCOL.md`, the Agda core's intent, the wire contract — and, because it turns on architecture/technical direction, it is the maintainer's decision: **surface the cross-binding set + the principle tension with a recommendation, don't conclude "all wrong, fix all" (nor silently FP) unilaterally.** When two consistent behaviors leave a residual tie, stricter is the safer default. (Worked example: Rust's response decoders silently blanked malformed `code`/`errors`/`absent`; Go/C++/Python all reject those shapes — Rust was the lone lenient outlier, so the fix restored parity rather than creating divergence.)

**A property-defined task makes that property the acceptance criterion for every surface — not the API shape.** When the point of a task is a property (laziness/streaming, zero-copy, thread-safety, idempotency, cancellation-safety), do not let API *shape* stand in for it. (1) List every public surface that must honor it (each binding, sync + async, eager + lazy) and check each *signature* against the property — e.g. "lazy input" ⇒ the parameter is a lazy source (`impl IntoIterator` / generator / `iter.Seq`), never a materialized `Vec`/`[]T`/`span` unless genuinely forced (then say why). (2) Write docstrings as specs and read them back against the code in the same function — a self-contradiction inside one function (a `Vec<Frame>` parameter under a "never materializes / live source" docstring) is inexcusable. (3) Write at least one test that exercises the property itself (feed a non-materialized producer; assert no full buffering), not just adjacent behavior — a test that only ever passes a `Vec` cannot reveal that the signature *demands* one. (4) Never transplant a sibling method's signature/constraint without re-deriving whether its reason applies here. (Worked example: #79's async `send_frames_stream` shipped taking `Vec<Frame>` — copied from the eager `send_frames`, where materialization is harmless — under a streaming docstring; Copilot caught the contradiction, the fix made it genuinely input-lazy. See `memory/feedback_property_defined_task_acceptance.md`.)
...
**CHANGELOG for notable build/CI/tooling changes.** Beyond the public-API surface above, a change under `Shakefile.hs`, `shake.cabal`, `aletheia.agda-lib`, `haskell-shim/`, `tools/`, or `.github/workflows/` also requires a `CHANGELOG.md` entry — the project convention is that the changelog documents ALL notable changes wherever they appear, not only public-symbol moves (user directive 2026-06-15). Test files and Markdown docs under those paths are exempt (a `.md` is always documentation; the changelog is itself `.md`). Agda sources (`src/`) are deliberately NOT watched: a behavioral `src/` change reaches users through the bindings (already covered above), and most `src/` edits are proof-internal. Enforced by `tools/check_changelog.py` (a `run_ci.py` gate + the Shake `check-changelog` phony). When the diff has no observable behavior change — a routine infra tweak or a bot dependency bump — the escape hatch is a one-line `### Changed` note saying so (e.g. "internal — no behavior change"), which the reviewer verifies by inspection. Distinct from the public-API rule above, which additionally demands migration guidance; this rule has no migration-guidance dimension.

**Single Python venv.** The project uses exactly one virtual environment, at `python/.venv` (Python 3.14). Every tool resolves it there — `tools/run_ci.py`, the mutation/stability runners, `python/pyproject.toml`'s basedpyright `venvPath` — so a second venv silently diverges from what the gates run. The recurring failure mode is a repo-root `.venv` created by following a doc that forgot to `cd python` first; one such stray once masked a basedpyright misconfiguration (it shadowed the real venv). Run every Python gate via `python/.venv/bin/...`, never the system `python3` (see `memory/feedback_use_venv_for_gates.md`). Enforced by `tools/check_venv_convention.py` (a `run_ci.py` source-hygiene gate): exactly one on-disk `pyvenv.cfg`, at `python/.venv`, and no tracked doc/script may create or activate a venv outside it (every `python -m venv` target and `.venv/bin/...` access must name `python/.venv` or sit in a `cd python` scope). The `.gitignore` lists only the canonical `.venv` — not the GitHub-template `env/`/`venv/`/`ENV/` set — so a stray non-canonical venv surfaces in `git status` instead of being silently ignored. The benchmark harness pointing at a repo-root `.venv` (`benchmarks/run_all.sh`) was the worked example the gate first caught.

**Every API language is mutation-tested, every mutant is killed, and coverage holds a floor (user directive 2026-09-18).** Each binding a user calls, Python, Go, C++ and Rust, runs a mutation lane over its hot-path sources, and the target for every lane is zero survivors: a mutant a test cannot see is a defect of the code it sits in, not of the suite, and the code is removed or restructured so a test sees the change, never justified beside it in a comment. A lane's recorded baseline in `docs/MUTATION_BENCH.yaml` is its distance from that target, and where the baseline carries a ledger every row names what still stands between; a row leaves the ledger by the change that kills or removes its mutant. Beside mutation, every binding's suite covers at least 80 percent of its lines and 60 percent of its branches, measured by the binding's own coverage tool. The mutation lanes for Python, Go and C++ run in CI (`tools/mutation_run.py`); the Rust lane is pending work in the task list. The coverage lane (`tools/coverage_run.py`, the floors and the measured figures in `docs/COVERAGE_BENCH.yaml`) measures all four; where a binding's own tool has no branch metric, the record names the finest unit it counts instead, blocks for Go and regions for Rust, and says so. The per-language mutation items (cat 14(g) in each language file) are manifestations of this rule.

**A measurement a gate is cut on is reviewed on a schedule, because nothing makes it go red (user directive 2026-09-20).** A recorded number that a gate *checks* announces its own staleness by failing. A recorded number a gate is merely *cut on* does not: it goes quietly wrong while every check stays green. The worked example is the C++ mutation lane's slice weights (`docs/MUTATION_BENCH.yaml`, `mutants_by_file`), the per-file mutant counts the six CI legs are balanced by. Adding code adds mutants, which nothing refuses and nothing should, so the counts age in one direction and the legs drift out of balance with no signal at all; coverage is never at risk, because the partition is computed over every tracked source and not over that list, but the lane slowly pays for a cut it no longer has. Such a number carries three things: the measurement it was taken from, a figure some run prints saying how far it has drifted (here the merge prints what the heaviest slice would carry today under the recorded weights), and a task in the task list whose trigger is a threshold on that figure rather than a date. Re-take it when the figure crosses, when the toolchain moves, or when the surface is re-measured for another reason.

**A gate that can't fail has a bug (user directive 2026-07-15).** A gate whose whole job is to fail on a defect, but which passes when the defect is present, is broken — and a broken gate is worse than none, because it certifies safety it never checked. This is not epistemics or hygiene: *failing on the defect IS the gate's function.* When writing or changing any gate (a Shake `check-*` phony, a `tools/check_*.py`, a git hook), prove it can fail: inject a real violation into the real input and assert a non-zero exit, and pair it with the false-positive case where a sibling must NOT match. Distinguish "could not check" (exit 2 — missing input, lost matcher grip, no build to compare) from "checked and found a violation" (exit 1); an empty result must never read as compliance. Use POSIX-portable matchers (a `\b` that works in your shell may not in CI). The audit that established this found 8 gates + 2 hooks that could pass while checking nothing. SSOT `memory/feedback_gate_pass_is_absence_of_output.md`.

**A gate that runs under a wall clock streams, fails loud, and re-measures its budget (user directive 2026-09-18).** Any gate whose runtime is long enough to be killed by a clock it does not control, a CI job's `timeout-minutes`, a hook's deadline, a host reboot, is bound by three clauses, each answering a separate way the clock defeats it.

1. **It streams its progress.** Capturing a child's output, whether by `capture_output=True`, `run_capture`, or a `| tail` in a pipeline, means the log exists only if the child exits normally: a killed run leaves nothing, and while it runs there is no way to tell progress from a hang. The long commands go through `tools/_common.run_streaming`, which emits every line as the child produces it and returns the same text for the caller to parse. A child that buffers its own stdout defeats a diligent writer on this side, so the helper sets `PYTHONUNBUFFERED=1` and a new streaming site checks that its tool actually emits per line rather than per run.
2. **Every terminal state is loud, and a timeout is a failure.** A check that reports only the state it hopes for is silent through a kill, a crash and a cancellation, and silence is indistinguishable from still running. Where a gate is reported by a job that aggregates others, it passes on the single result meaning every part finished clean and refuses every other by name, rather than enumerating the ones it expects, because a kill by the clock arrives as whichever of them the platform chooses. This is the wall-clock manifestation of the rule above: a gate that cannot report the timeout cannot fail on it.
3. **A change that multiplies the work re-measures the budget in the same change.** A budget is a measurement of what the gate did when it was set, and it is not carried across a change to what the gate does. The lane records its own wall time so the next value is read off a number rather than guessed. The worked example is this rule's origin: a round that took the C++ mutation surface from 264 mutants to 971 across two build trees edited the workflow comment describing the lane's cost and left `timeout-minutes: 90` untouched, so the lane was killed at 90 minutes with its sweep still running, having logged one line in 78 minutes and lost the rest. The fix was all three clauses: per-binding lanes on their own clocks, reported by one aggregating check that refuses any lane result short of success, with the long commands streaming.
...
Cat 1 (Dead code) is the worked example: it graduated when a strict pre-push dead-import gate was wired (commit `403555b`); the residual work was historical cleanup wearing review clothes. The gate is now the single scope-aware `.agdai` IWYU tool (`tools/iwyu.py --check`, run_ci step 9 + its `--self-test` at step 10); the recompile/regex tools it replaced are retired. Either way, the new-code regression class is caught at push time, not at review time.
...
3. **Lint gates**: `basedpyright`, `pylint`, `go vet`, `gofmt -l`, `clang-format --style=file:cpp/.clang-format --dry-run -Werror` over **every** tracked C++ source (two live outside `cpp/` and a listing taken from `cpp/` never saw them), and `run-clang-tidy-23 -quiet -p build cpp/src/ cpp/tests/ cpp/benchmarks/` **from `cpp/`** (the tests carry `cpp/tests/.clang-tidy`, which inherits the root configuration and disables only what Catch2's macros generate, each entry with the count it was measured at; the four fuzz targets compile only in the fuzz configuration, so the coverage guard excludes them and the fuzz lane owns them): clang-tidy finds `.clang-tidy` by walking up from the working directory, so the same command run from the repository root enables no checks, says so on stderr, and looks clean to anything grepping its output for a finding. CMake files are gated too, with `cmake-lint -c .cmake-format.yaml --` and the file list piped in: the config option takes one or more values, so without the `--` separator the tool reads the file paths as configuration files, scans nothing and exits zero.
4. **Cross-language benchmarks** (mandatory for runtime-affecting changes — see Universal Rules): `bash benchmarks/run_all.sh --frames 10000 --runs 5 --bench throughput`, compared against the most recent baseline. Two thresholds: (a) **steady-state noise floor ~2–4%** — the typical run-to-run jitter on a quiet host; (b) **±10% inter-run variance gate** — beyond this, a delta MUST be investigated, not dismissed as variance (the protocol that caught the −16.1% `log_event` regression). The aspirational gate is no regression > 3% lane-by-lane after accounting for the noise floor; in practice anything inside ±10% reads as run-to-run jitter and anything outside requires root-causing. **When reporting bench runs**, always show four columns per lane: raw current mean, baseline mean, delta %, AND current stddev — the stddev is the only thing that lets the reader judge whether a delta is signal or noise (drop baseline σ first if width forces compression, never both). Before trusting a C++-only regression, verify `cpp/build/CMakeCache.txt` has `CMAKE_BUILD_TYPE:STRING=Release` — a Debug-pinned cache silently produces −20%+. The runner and binary now both refuse Debug trees, and each bench JSON records `system.build_type`; an older stored baseline may have no `build_type` field, so double-check before comparing.
...
When citing a finding category in commit messages or round plans, use the canonical reference form `<LANG> cat <N>` (e.g. `Agda cat 16`, `Python cat 26`) or the finding-id shorthand `<LANG>-<AGENT>-<CAT>.<NUM>`. The full text of each category definition lives in the file above.
